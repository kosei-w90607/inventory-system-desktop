...
- [x] 商品コードの長さ上限: 完了（PR #111、2026-09-26）。商品 CSV の取込みで 100 文字（UTF-16 code unit、在庫照会の zod と同じ数え方）を超える商品コードを preview と commit の両方で拒否（owner 決定 2026-09-25）。
...
- [x] 在庫更新の共通関数が通常の接続を受け取れる: 完了（PR #111、2026-09-26）。`apply_stock_change` の第 1 引数を `&rusqlite::Transaction<'_>` に限った（振舞いは不変）。
- [x] DB の版がアプリより新しい場合の拒否: 完了（PR #111、2026-09-26）。論理的な書込みの前に拒否し、起動を固有の文言で止める。MSI 配布手順の docs は v1.0 gate 側に残る。
...
- [x] 設定時刻の自動バックアップの確認を画面に依らず動かす: 完了（PR #111、2026-09-26）。共通レイアウトの hook で 60 秒ごと（71 §71.8 どおり）。
- **保存と起動の守りの follow-up（PR #111 起源、小口）**: (1) `src-tauri/src/db/migration.rs` の `migrate` の doc comment の手順番号（0〜2）を 22 §3.2 の番号（1〜5）に合わせる（Final Review の Claude 側 P3）。(2) 新しすぎる backup の復元失敗が画面では固定文言「もう一度お試しください」になり、版の detail（データの版・このアプリが扱える版）が見えない。同じ backup で再試行しても必ず失敗するので、文言で「新しい版のアプリが要る」と伝える。(3) `generate_custom_code`（`product_service.rs:153`）も TX の内側を前提にする helper で、型で限っていない（`apply_stock_change` と同型、Matrix の隣接走査で発見）。
