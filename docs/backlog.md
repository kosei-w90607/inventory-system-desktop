...
- **returnTo 衛生（小 runtime、R3〈search state〉）**: NAV-1（PR #75）の Final Review round 1 pass B / round 2 pass B（P3）起源。(1) `src/lib/return-to.ts` の guard を `/\` と `/%09/` も弾く形へ（`new URL(value, "http://local").origin` 一致判定、先例 `src/features/products/lib/return-to.ts:19-25`）。(2) 「在庫照会へ戻る」等の `returnTo` back-link で pathname を pin（label と行き先の一致）。(3) `<Link to={string with query}>` を `to` = pathname + `search` object へ（`InventoryRecordsPage.tsx:69-71` の規約。業務記録詳細 6 画面 + 在庫変動履歴が対象）。(4) `stockInquirySearchSchema.selected` の `max(20)` と `product_code` 無制限の不整合。(5) T2-num を `stockInquirySearchSchema.parse(defaultParseSearch(...))` まで通す oracle 強化。
