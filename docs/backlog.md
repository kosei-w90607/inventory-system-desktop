...
- **ボタンの最小サイズの不一致（docs 小口）**: `docs/UI_TECH_STACK.md` §5.6 の 44×44px（WCAG 2.1 AAA）と 旧 `docs/design-system/04-backbone.md` の token 表の min-height 40px（`git show dda8560a:docs/design-system/04-backbone.md` で引ける。改訂後は 04 原則 7 が本 entry を指す）が食い違う。どちらを正とするか決めて揃える（design-first）。整理の向き: 日常的に押すボタンは 44×44 CSS px 以上を製品標準、40px は密度の参考値、24×24 は WCAG 2.2 の最小基準であって十分条件ではない、という区分を軸にする。
...
- **`docs/UI_TECH_STACK.md:481` の「大きな文字 (18px+ or 14px+bold)」は px と pt の取り違え**: WCAG の基準は 18pt または 14pt 太字で、通常の CSS 換算では 24px / 約 18.7px。文書だけで判定できる誤りで、既存の「ボタンの最小サイズの不一致」entry と同じ docs 小口で直す。あわせて `docs/design-system/00-foundations.md:22` が自ら AA 未達（対 `--card` 約 4.40:1）と記録している muted 文字を通常サイズの説明文に使っている箇所を実測すると、`StocktakePage.tsx:979` の `CardTitle`（`text-sm font-medium text-muted-foreground`）が `Card` の `bg-card` 上でこの組合せに当たる。起源: 2026-09-22 の外部設計相談（owner 実施）で指摘、Coordinator が現物で確認。
