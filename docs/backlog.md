...
- **`returnToLinkProps` の「値なし」sentinel を型で強制する**: `{ to: "", search: {} }` を `| null` にして呼出側の分岐を型で強制する（PR #78 Final Review round 2 Opus P3-4 起源。業務記録詳細 6 画面は `<Link {...backLinkProps}>` を素通しするため、将来不正な fallback を渡すと戻り link が無反応になる。現 consumer では発火しない。helper の署名変更を伴う小 runtime、R3〈route/search state〉）
...
- **`StockMovementsPage.tsx` の `detailReturnTo` を router の href へ寄せるか**: 現状は `URLSearchParams` の手組みで、数字だけの string 値を持たず無害（PR #78 Non-scope 起源）。入れ子 `returnTo` の形が変わるため別判断、DSR-18 の手組み存置例外の 1 つ。
...
- **棚卸しの評価額が価格の基準数量を持たない**: `src-tauri/src/biz/stocktake_service.rs:469` は原価 × 数量（`valuation_cost_price.checked_mul(item.actual_count)`）をそのまま評価額にする。商品 master に、売価・原価が何数量あたりかを表す列が無いため、長さ商品のように在庫は cm・仕入れと値札は 1 m あたりという商品では、1 m あたりの原価を登録すると評価額が 100 倍になる。本番データが無い今のうちに、上記「単位の拡張」lane の最初の成果物として、各整数が何を 1 としているか・価格がその何個分かという契約（価格の基準数量）を決める。起源: 2026-09-22 の外部設計相談（owner 実施）で指摘、Coordinator が現物で確認。
...
- **評価額の丸めを店の規則に合わせる**（owner 決定 2026-09-24）: 店の規則は、商品別の金額を小数第 3 位で四捨五入して小数第 2 位まで持ち、全商品の金額を合計した最終合計で四捨五入する（[project-memory](project-memory.md) Store Premises Facts の「いまの手作業」、owner 伝聞 2026-09-24）。現行は原価が円の整数（`docs/db-design/master-tables.md` 34・54 行、`src-tauri/src/db/schema_v1.rs:36`・`:223` の INTEGER）で、確定時の総額は `valuation_cost_price × actual_count` の整数積和（`src-tauri/src/biz/stocktake_service.rs:464-475`、`docs/function-design/35-biz-stocktake-service.md` の新方式の total_cost の式と `complete_stocktake` の処理ステップ）。商品別の金額に小数が生じず丸めの段が無いため、数量や単位当たり原価に端数が出る場合（上の「単位の拡張」: m の数量は小数 1 桁）に店の規則と一致しない。実装方針は未定（上の「棚卸しの評価額が価格の基準数量を持たない」と関係する）。
