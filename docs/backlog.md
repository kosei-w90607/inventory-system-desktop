...
- **型生成の失敗が成功扱いになる**: `src-tauri/src/lib.rs` の `export_specta_bindings()`（253 行目、戻り値なし）は `builder.export(...)` / `normalize_generated_bindings` / `append_generated_constants` の失敗を `eprintln!` で終える（347〜365 行目）。`src-tauri/src/bin/generate_bindings.rs` は呼出し後に無条件で完了 message を print して正常終了する。生成が失敗して古い `src/lib/bindings.ts` が残ると、local CI の「生成 + `git diff --exit-code`」が green になり得る。直す向き = 生成処理を `Result` にして CLI は失敗時に非 0 で終了、可能なら一時出力を完成させてから置換、失敗の各段を test。小さく直せて全 lane に効くので、次に着手する最初の小変更（owner 決定 2026-09-22）。起源: 2026-09-22 の外部設計相談（owner 実施）で指摘、Coordinator が現物で確認。
