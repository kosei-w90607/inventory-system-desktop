...
- **STK-2（P1 候補）**: 棚卸し確定後に届く過去販売の日報取込みが二重に減算する（[根拠](research/2026-09-16-diagram-audit.md#stk-2-棚卸し確定後に届く過去販売を二重に減算する)）。カウント基準時点と取込み済み境界を設計する（design-first、R3、STK-1 と同じ lane か直後に並べる）。（**2026-09-16 起票 = ㉗（STK-1 と同 lane）、`plans/2026-09-16-stocktake-count-baseline.md`、wave 12 lane 2、owner の設計判断待ち**）
...
- 店の事実を前提にする判断の前に `docs/project-memory.md` の「現場の前提」と照合する観点を、レビュー / Plan の観点へ足す（2026-09-19 ㉗ Plan Review round 3 が実店舗に本番データが在るという誤った前提で移行設計を議論し owner へ escalation した実発生、同日 owner 回答の複数件に「これも聞かれたことある」とあった実例が起源）。`docs/DEV_WORKFLOW.md` / `docs/templates/**` の改訂を伴うため Final Review 2 本が要る docs 小口として起票する。それまでは Coordinator の発注書に固定項目として運用する。
