...
- [x] 表示小修正 batch 2: 完了（PR #64、2026-09-16）。owner 2026-09-11 所感 + L8-4 owner 決定 2026-09-15 起源。在庫少の基準の区画見出し撤去（説明文のみ）/ Alert title 太字（600）/ 在庫照会の副題 + 絞り込み時「全 N 件」/ 在庫状態 Badge「在庫あり」/ 基準の入力 error 文言 1 本化 / 入出庫履歴の明細数列撤去を実装。[archive packet](archive/plans/2026-09-15-display-fixes-batch-2.md) / [Matrix](archive/plans/test-matrices/2026-09-15-display-fixes-batch-2.md)。
- [x] 一括価格改定の取引先紐付けを既定 off + 文言明示: 完了（PR #67、2026-09-16）。owner 決定 2026-09-16（L3 round 3 所感「価格改定のついでに取引先が変わる」）起源。toggle 既定 off・供給者変更時も off のまま・label 文言「確定した商品の取引先が未設定なら、この取引先を設定する」を実装し、設計書 77 / `ui-task-specs.md` UI-14 を同期、decision-log D-088。[archive](archive/plans/2026-09-16-price-revision-assign-default-off.md)。
- [x] 廃棄・破損の保存結果に「詳細を見る」+ `returnTo`: 完了（PR #71、2026-09-16）。owner 2026-09-11「やったほうがいい」（PR #23 owner L3 所感 2026-08-31 起源）。UI-05-D17 を改訂し、保存結果にも詳細 link（`returnTo` 送信）を追加、入庫・返品交換と対称化。[archive packet](archive/plans/2026-09-16-disposal-result-detail-link.md) / [Matrix](archive/plans/test-matrices/2026-09-16-disposal-result-detail-link.md)。
- [x] ホーム画面を mockup-c 案へ寄せる: 完了（PR #70、2026-09-16）。owner 2026-09-11「これ好き、採用したい」起源、canonical = `reference/mockup-c-home.html`。入口 card に icon + 1 行説明・「売上データ取込み」の primary 強調・補助文言を状態の説明へ変更を実装（PLU 未反映 card は owner L3 所感で撤去）。[archive packet](archive/plans/2026-09-16-home-mockup-c-runtime.md)。
- [x] 在庫変動履歴からの戻りで在庫照会の検索条件と商品選択を保持する（NAV-1）: 完了（PR #75、2026-09-17）。監査 NAV-1（P2、[根拠](research/2026-09-16-diagram-audit.md#nav-1-在庫変動履歴からの戻りで商品選択が失われる)）起源。在庫照会の詳細 → 在庫変動履歴の link に現在の `/stock` URL を `returnTo` として送り、「在庫照会へ戻る」は正規化した `returnTo` へ戻る。欠落・不正時は `Link` の `search` props で商品コード検索付き `/stock` へ fallback（GA5、数字だけの商品コードでも文字列化しない）。[archive packet](archive/plans/2026-09-16-stock-movements-return-selected.md) / [Matrix](archive/plans/test-matrices/2026-09-16-stock-movements-return-selected.md)。

#### やると決めたもの（順番未定）

- **実測とPOS系列の対応を取得・保存する**（design-first、R3）: ㉗のround天井のowner disposition（2026-09-22）から切り出す次のdesign lane。実機確認 `4 点`が着手条件：精算系列の意味（Z004のリセット範囲・同日複数精算・番号リセット・再出力 / 訂正版の識別）、番号印字のEJ記録と取引一連番号 / 時刻との並び・売上 / Z004への副作用、レジ時計の進退のZ004 / EJへの痕跡、棚卸し→アプリ終了→OS再起動→翌日の遅延・逆順取込みで境界確定済み商品の再計数なしに実測後の販売だけが一度減ること。[㉘申し送り](archive/plans/2026-09-16-stocktake-count-baseline.md#後続-runtime-lane-㉘-への申し送り)と[ADRの適用範囲](adr/2026-09-18-stocktake-time-evidence.md#適用範囲の但し書き)を入口とする。番号印字は未検証候補であり採用済みではない。実機確認は後回しとし他の作業を先に進める。EJ parserの構造復元は並行可能だが、時刻分割は本laneの結論を待つ。EJ parser の構造復元（core、IO-08）は完了（PR #94、2026-09-25 merge）。日次の EJ 取込み（画面・BIZ・file の選び方、`docs/function-design/29-io-ej-parser.md` IO-08.10 の申し送り）は、本laneが決める「欠け」の定義を待つ。本laneでは、時点証拠 ADR の「直前の Z004 を利用できる」「前回」の候補を、同一性拒否の記録がなく guard を通過して取り込んだ Z004 に限るかを決める（PR #96 の GA6 後 broad、Fable N2）。日次の EJ 取込みの interface を決めるときは、「直前の Z004 を利用できる」を「guard を通り、名称の組を保存した受領」と定める（同、Opus の観察）。
- **EJ parser に、店がレジで普段使う値引き・訂正・取消の文法を足す**（R3、実物の採取が着手条件）: PR #94 の `parse_ej` は実物 6 本に現れた形式だけを受理し、値引き（％キー・マイナスキー）・訂正・取消を含む取引は復元不能（安全側）に倒す。店はこれらのキーを普段使うため（店主・owner の回答台帳 TD-029、repo 外）、実物ではこれらの取引が復元不能になり、件数が多くなる見込み。次の訪店で、それらを使った日の EJ を採取する（approved-readable、確かめるのは構造と件数だけ）。形状を確かめてから `docs/function-design/29-io-ej-parser.md` の文法表と実装へ足す（既存の復元結果を変えない追加に限る）。
- **STK-1（P1、設計上の問題）**: カウント後の入出庫を棚卸し確定が打ち消す（[根拠](research/2026-09-16-diagram-audit.md#stk-1-カウント後の入出庫を棚卸し確定が打ち消す)、合成DBで実BIZ関数を再現済み）。棚卸し確定の基準時点と確定後 movement の扱いの設計は正本化済み（**㉗、[Plan Packet](archive/plans/2026-09-16-stocktake-count-baseline.md)、PR #85、2026-09-21 merge**）。runtime実装は ㉘（R3、時刻判定の機構は実装対象外）へ、恒常運用は実測とPOS系列の対応の取得・保存を経る次のdesign laneへ引き継ぐ。㉘ の最初の lane で旧棚卸しの開始・カウント保存・確定を BIZ 入口の最初の文で停止した（**PR #95、2026-09-25 merge**、[停止 ADR](adr/2026-09-23-legacy-stocktake-z004-write-stop.md)）。是正と停止の解除は ② 〜 ⑤。
- **STK-2（P1 候補）**: 棚卸し確定後に届く過去販売の Z004 商品別売上取込み（日報 Z001/Z002/Z005 は在庫を動かさない、D-025）が二重に減算する（[根拠](research/2026-09-16-diagram-audit.md#stk-2-棚卸し確定後に届く過去販売を二重に減算する)）。カウント基準時点と取込み済み境界の設計は正本化済み（**㉗、[Plan Packet](archive/plans/2026-09-16-stocktake-count-baseline.md)、PR #85、2026-09-21 merge**）。runtime実装は ㉘（R3、時刻判定の機構は実装対象外）へ、恒常運用は実測とPOS系列の対応の取得・保存を経る次のdesign laneへ引き継ぐ。㉘ の最初の lane で Z004 の業務 commit・取消を BIZ 入口の最初の文で停止した（**PR #95、2026-09-25 merge**、[停止 ADR](adr/2026-09-23-legacy-stocktake-z004-write-stop.md)）。是正と停止の解除は ② 〜 ⑤。
- [x] `returnToLinkProps` の「値なし」sentinel を型で強制する: 完了（PR #106、2026-09-26）。helper が不合格時に `null` を返し、業務記録詳細 6 画面は `??` で既定の戻り先を書く（書き忘れは typecheck で止まる）。PR #78 Final Review round 2 Opus P3-4 起源。
- **商品コードの長さ上限が DB にも BIZ にも無い**: DB（`schema_v1.rs` `product_code TEXT PRIMARY KEY`）にも BIZ（`product_service.rs` は空判定のみ）にも上限が無い（PR #78 起票時実測起源。在庫照会の `selected` / `q` は 100 文字で落ちる。上限を決めるかは owner 判断、design-first）。
- [x] `StockMovementsPage.tsx` の `detailReturnTo` を router の直列化へ寄せる: 完了（PR #106、2026-09-26）。`location.href` ではなく `defaultStringifySearch` へ寄せ、正規の戻り先では出力不変。DSR-18 の手組み存置例外は `buildProductListReturnTo` だけになった。PR #78 Non-scope 起源。
...
- 日報画面の Excel 印刷・バインダー代替受入判定（現行 Excel + 印刷 + バインダーが日別記録を残す唯一の手段。実 1 日分での公式集計・過去日到達・欠落日・修正/再取込み・backup/restore 後の再現を横並び確認し、印刷機能の要否を go-live 前に判定する）。
  - レジ締め時の確認に使っている Excel 貼付け・印刷は、アプリで丸ごと置き換えるつもり（owner 2026-09-16。2026-08-01 には、置き換えられるかは使い手次第とも述べた）。日報の印刷機能はあってもよい。紙はやめていく方針だが、目の悪さもあって紙のほうが見やすい場面がある（owner 2026-09-19）。
...
- [x] ボタンの最小サイズの不一致（docs 小口）: 文書側は完了（2026-09-26）。`docs/UI_TECH_STACK.md` §5.6 を「日常的に押すボタンは 44×44 を目標、現状は既定 36px で未達」に直し、44 / 40 / 24 の区分を書いた。04 の対応表の参照先も更新。
- **ボタンの高さを目標の 44px へ近づけるかを実機で決める**（デザイン一新の束、manual 付き）: 共通の `Button` の既定は 36px（`h-9`）、`lg` は 40px で、`docs/UI_TECH_STACK.md` §5.6 の目標 44×44 に届いていない。どのボタン（日常的に押す主操作・行内の操作・icon ボタン）をどこまで上げるかを、before / after を実機で並べて owner が決める（owner 2026-09-26「今の実装が完璧だとは思わないので、良くなるなら変えてよい」）。行高 40px（04 原則 8）とは別の話。
- **workflow の軽量化 3 段**: 発注の構造化と発注前検査 → Amendment の影響別扱い → 重複文書の統合、を別々の変更で進める（㉗ の dogfood 所見起源、owner disposition 2026-09-22）。1 段目のうち script・schema を伴わない文書だけの部分（普通の一日の操作列・Writer 停止時の規則・問い合わせの行き先）は完了（PR #90、2026-09-22 merge）。1 段目の残り（発注 schema〈WriterOrderV1〉・発注前検査の script）と 2 段目（Amendment の影響別扱い）・3 段目（重複文書の統合）は未着手。
- **Plan Review の「普通の一日で目的を達成できるか」の finding が Review Rules の 3 分類のどれに載るか未定義**: `docs/DEV_WORKFLOW.md` Review Rules の 3 分類（candidate safety / mutation authority / evidence quality）に、この問いから出る finding をどれとして扱うかの割当てが無い（PR #90 Final Review pass B〈Opus〉起源）。
- **問い合わせの行き先の表の行 7 に `AGENTS.md` が別の明示承認とする PR 操作が挙がっていない**: `docs/DEV_WORKFLOW.md` `### 問い合わせの行き先` の行 7 は Windows L3・R4・Ready・merge のみで、`AGENTS.md` が別承認とする label・thread 状態・close・issue comment 等の PR 操作が表のどの行にも無い。注記 (b)（この表は `AGENTS.md` Decision and Approval Boundaries を変更しない）により境界自体は保たれる（PR #90 Final Review pass B〈Opus〉起源）。
- **撤去済みの Phase 名 `independent-review` が 2 か所に残る（ハーネス刷新 PR2 で直す）**: `docs/AGENT_OPERATING_MANUAL.md:22`（Final Reviewer の役割の説明）と `docs/templates/plan-packet.md:196`（Contract Coverage Ledger の行を再確認する時点）。`independent-review` は現行の Phase の 8 値に無く、`docs/DEV_WORKFLOW.md` の分は PR #97 で直した（PR #97 Final Review broad〈Opus〉F3 起源、裁定で PR2 送り）。
- [x] 棚卸しの評価額が価格の基準数量を持たない: 完了（PR #108、2026-09-26）。基準数量を在庫単位で決める（pcs = 1 個、cm = 1 m = 100 cm。商品ごとの列は足さない、owner 決定 2026-09-25）。箱や袋で仕入れてばらして売る商品の扱いは Issue #105 E6 の店の答えで単位の拡張 lane が見直す。
- **在庫更新の共通関数が通常の接続を受け取れる**: `src-tauri/src/biz/inventory_service/common.rs:41` の `apply_stock_change` は `conn: &DbConnection` を受ける。同ファイルの doc comment が明記する「TX の内側から呼ぶ」という約束が型に表れていない（現在の呼出し元は TX を渡しており、部分更新の事故が起きているという意味ではない）。直す向き = 借りた transaction だけを受ける引数にする。㉘ runtime の数量と版の不可分更新と同じ場所を触るので、㉘ に含める。起源: 2026-09-22 の外部設計相談（owner 実施）で指摘、Coordinator が現物で確認。
- **DB の版がアプリより新しい場合の拒否が無い**: `src-tauri/src/db/migration.rs:142` は `migration.version > current_version` の未適用分を流すだけで、DB の schema 版がアプリの知る最大より新しい場合に止まらない。旧版のアプリが新しい DB を開いて書き込む事故を防ぐため、書込みの前に拒否する。上記「MSI 配布手順 docs 化」の手順（v1.0 gate の MSI 配布手順）と一緒に扱う。起源: 2026-09-22 の外部設計相談（owner 実施）で指摘、Coordinator が現物で確認。
- **商品別売上の取込みが、数量 0・金額 0 の行を商品照合の前に捨てる**: `src-tauri/src/biz/csv_import_service/parse.rs:94` は `row.quantity == 0 && row.amount == 0` の行を商品照合前にスキップする。販売と返品が相殺した商品も「動きなし」と区別できず、棚卸しとの前後判定の対象から落ちる。直す向き = 在庫判定の対象になる全行の集合と、売上記録を作る行の集合を分ける（時点証拠の ADR〈`docs/adr/2026-09-18-stocktake-time-evidence.md`〉が既に求めている契約）。㉘ runtime に含める。起源: 2026-09-22 の外部設計相談（owner 実施）で指摘、Coordinator が現物で確認。
- [x] `docs/UI_TECH_STACK.md` の「大きな文字 (18px+ or 14px+bold)」の px と pt の取り違え: 完了（2026-09-26）。18pt / 14pt 太字（CSS 換算 24px / 約 18.7px 太字）に直した。
- **muted の文字色が通常サイズの説明文で AA に届かない箇所**（上の px / pt の entry から分離）: `docs/design-system/00-foundations.md:22` が muted 文字の対 `--card` のコントラストを約 4.40:1（AA 未達）と記録している。`src/features/stocktake/StocktakePage.tsx:979` の `CardTitle`（`text-sm font-medium text-muted-foreground`）が `bg-card` 上でこの組合せに当たる。色の役割表に沿って直すのが筋で、デザインの決まり runtime lane A（色と強調）で扱う。起源: 2026-09-22 の外部設計相談（owner 実施）、Coordinator が現物で確認。
- **backup に、PC の外のコピーと復元の実証が運用として設計されていない**: `docs/function-design/71-mnt-backup.md` は `VACUUM INTO`・自動 backup・保持日数での掃除・復元の退避と巻き戻しを持つが、保存先は同じ PC。店の PC は laptop 1 台で、故障・盗難で台帳と履歴を同時に失う。v1.0 の前に必須: PC を失っても残るコピー（取り外せる外部媒体を別の場所に保管する方式が初期案）、ファイル存在と使える backup の区別（作成途中を成功世代に入れない、外部コピーの照合）、ローカル成功と PC 外の保全成功を分けた表示、復元の予行演習、更新・migration は必要な保全に成功しなければ開始しない。既存の「バックアップ保存先の UNC / ネットワークパス対応」entry（保留）とは別の項目として起票する。起源: 2026-09-22 の外部設計相談（owner 実施）で指摘、Coordinator が現物で確認。
  - 店の事実: 復旧に備える保全単位は、SD の CASIO フォルダ全体と PC 側 CV17 の 2 フォルダの計 3 点（2026-08-15 訪店記録の推奨。実施の記録なし）。現行 CV17 のインストーラと公式利用ガイドのオフライン保管も訪店記録の推奨で、実施は未確認。バックアップを外部に保存するなら置き場所は店（owner回答2026-09-23）で、PC が店にあるときは PC と媒体が同じ場所になる残存リスクがある（推測）。店主は、データが消えたときに 3 日程度前の状態まで戻れれば許容できると答えている。これとは別の問いとして、システムが動かなくなったときに手作業に戻れれば足りるかには、手作業の中身が分からないと判断できないと答えている（いずれも店主回答、2026-03）。
- [x] 評価額の丸めを店の規則に合わせる: 完了（PR #108、2026-09-26）。商品別の金額を 1/100 円で四捨五入して合計し、最終合計を円未満で四捨五入（35 §20.5a）。
- **原価 × 数量の式が価格の基準数量を持たない残り（評価額 lane の Q3、owner 2026-09-25「含めず backlog」）**: 長さ商品（cm）で金額が 100 倍になる既知の不整合。(1) 入庫の原価小計・原価合計（式の正本 `docs/function-design/21-io-inventory-repo.md:103`）、(2) 廃棄のロス原価と入力画面の合計（同 `:354`）、(3) 棚卸し記録詳細のロス原価（画面が計算、⑤ で BIZ から返す形にするのが筋）、(4) 商品フォームの価格欄に「1 m あたり」を出す表示（単位の拡張 lane の m の入力・表示と一緒に）、(5) 手動販売の金額の初期値（`docs/function-design/62-ui-manual-sale.md:26`・`:165` UI-04-D6、`src/features/manual-sale/lib/manual-sale-row-utils.ts:24,40`）。(1)(2) は仕入れ伝票・廃棄の端数の規則が店に未確認。どれも評価額 lane の前からある誤りで悪化していない（master-tables と 35 §20.5a に既知の不整合として明記）。
- **⑤ で棚卸しの `total_cost` を wire・表示まで正確に扱う**（評価額 lane の後続必須条件）: i64 の円額を JS の `number` で受けると 2^53 を超える成功値で精度が落ちる。例: `cm` 原価 `100000000000000001` 円/m・数量 `100` cm は新式で保存でき、JS の `number` では 1 円ずれる。⑤ の結果画面・前回サマリ・記録詳細で十進文字列等にして、この入力を検証する（archive `docs/archive/plans/2026-09-25-stocktake-valuation-basis.md` の Boundary / Wire Contract）。
- **設定時刻の自動バックアップの確認が、バックアップ画面を開いている間しか動かない**: `docs/function-design/71-mnt-backup.md` §71.8 は `check_auto_backup` を setup hook（起動時）と「フロントエンドタイマー（60秒間隔）」から呼ぶとするが、60 秒の timer は `src/features/backup-restore/BackupRestorePage.tsx:167-187` の `useEffect` にしか無く、画面を離れると止まる。起動時の backup は取れるが、設定時刻の backup は画面を開いていない日には取れない。直す向き = timer を画面に依らない場所（app の root 等）へ移すか、§71.8 の記述を実装に合わせるかを決める。PR #93 の Plan Review round 3 P3-c 起源（店の 1 日の動線の文に自動バックアップを書く際に判明、2026-09-24）。
- **デザインの決まり runtime lane A（色と強調）**（manual 付き、before / after を並べて owner が見る L3）: decision-log D-091 と `docs/design-system/00-foundations.md` の色の役割表・強調の段階・迷いやすい場面を、token・横断部品・全画面へ反映する。着手条件 = デザインの決まりの組み直し（D-091）の merge。進め方 = 新 token を別名で追加 → 部品・画面を新 token へ移す → 最後に `--primary` / `--ring` の値を変え、旧 token（`--warning-emphasis` の操作との兼用・`--rank-top-*`）を削る。merge は 1 PR（途中の状態を main に置かない。全画面を 1 PR で出すことは owner 了承 2026-09-24）。対象 = `globals.css`、`button`、`badge`（`defaultVariants` を `default` から外す）、`checkbox`、`progress`（棒の塗り）、`--ring`、待ちの spinner（`Loader2` の `text-primary`）、Home の `ActionButton`（操作の段 2。地を操作の仲間へ揃えるのは試し）、ナビの現在地（`CURRENT_LOCATION_ACCENT`）、現在行（`SupplierPickerDialog`）、ランキング（`ProductRankingTable`）、③強調の badge（最新・上書き件数。今は `variant="default"` で `--primary` の塗りのため、`--primary` の値を変えても owner が試しを採るまで琥珀のままになるよう `--warning-emphasis` へ移す。`--warning` は塗りの上の文字との比が 4.5:1 に届かないため移し先にしない。比の計算結果は D-091）、日報の取込み済みの badge、Home の未取込みの Alert、`alert.tsx`（`destructive` の Alert の地・線・icon。00 の迷いやすい場面「危険・失敗の Alert」。icon を必須にする「Alert」に「非中立の（warning・destructive）」を掛ける文言の直し〈DSR-08・04 原則 2・旧番号の対応表の旧 3 の行。中立の既定 Alert は現行 icon なし、PR #98 の closure C1〉）、詳細を開いた行（`table.tsx` の `data-[state=selected]:bg-muted` と `has-aria-expanded:bg-muted/50`、在庫照会の `ProductListTable`、操作ログの詳細の開閉。詳細を開いた行には `ProductListTable` と同じく明示の class を当て、`TableRow` の既定の `has-aria-expanded` は選択欄〈Radix Select〉を開いた行〈廃棄・返品交換の入力行〉にも当たるため進行中にしない〈外すか stone のまま〉）、取込みの手順の表示（`StepIndicator`。`CsvImportPage` は結果の画面でもいまのステップを進行中のまま残すため、済んだステップと先のステップの見分けとあわせて Plan で役割と段を決める）、月次売上の前月比のセル（`ComparisonCell`、薄い地の chip で枠も icon もない。Plan で役割と段を決める）、失敗ではない赤の知らせ（`ImportingStep` の取込み中の移動制限の `destructive` Alert、入力 4 画面〈Receiving / ManualSale / Disposal / ReturnExchange〉の未保存の案内の赤い文字。Plan で役割と段を決める）、00 に登録されていない token の既存の drift（`IntegrityCheckPage` の `text-warning-foreground`、`PluExportPage` の Alert の `border-info`・`bg-info-soft`・`text-info-strong`〈`info` 家族は `globals.css` にも 00 にも無い〉）、`docs/SCREEN_DESIGN.md` の画面ごとの色の記述（「手動」の黄色バッジの記述は現行の②分類と食い違っている）と 02 の該当節、00 の HEX と DS3。AC = merge 直前に origin/main を取り込み、`primary`・`ring`・`warning-emphasis`・`rank-top`・`row-current`・`toast.info`・`variant="destructive"`・`data-\[state=selected\]`・`has-aria-expanded` の rg を再実行して hit を全て分類済みにする（式は Plan で固定）。L3 項目 = before / after の並べ比べ、ランキング 1 位（順位と太字）、「最新」と②分類の「手動」を言い分けられるか、Home の入口 card の 3 状態（現行 = 琥珀の線と注意の薄い地、token 反映後に owner の現行の決定を保った形 = 操作の新しい線と注意の薄い地、試し = 操作の線と操作の仲間の薄い地）、日報の取込み済みの badge と Alert の役割、Home の未取込みの知らせの色、操作の塗りと完了の緑を文字と icon で言い分けられるか、進行中の囲みが「まだ終わっていない」と受け取られるか、在庫照会で詳細を開いた行の緑寄りの見え方（owner が違和感を言えば stone へ戻す。3 点目に badge を足さない形もあわせて見る）、取込みの手順の表示のいまのステップが進行中の段 2 で目に留まるか、済んだステップと先のステップを見分けられるか、destructive Alert の soft の地と icon の見本、badge の文字の太さ（現行 500）。試しを採る・採らないで直す向きは 00「迷いやすい場面」に従う。既存の項目（card の面色を白へ寄せる、destructive Alert の soft 塗り）と token が重なるため、同じ PR に入れるかを lane A の Plan で決める。
- **デザインの決まり runtime lane B（書体）**（manual 付き）: `docs/design-system/00-foundations.md` の書体の節の選定条件で Noto Sans JP と BIZ UDPゴシックを実機で比べて採用し、同梱が要るなら同梱する（等幅の数字の適用を含む）。着手条件 = デザインの決まりの組み直し（D-091）の merge。依存を足すなら D-030（名指し・`--save-exact`）に従う。BIZ UDPゴシックが Windows 標準に含まれるかを vendor の公式資料と店の PC で確かめ、同梱サイズを実測する。L3 = 店の PC で店主が通常距離で読み分けられるか（owner の目）。runtime lane A と別 lane にする理由 = 依存と配布物が変わり、店の PC での確認が要る。
- **棚卸し画面 D1 の実装**: owner 採用 2026-09-24 の棚卸し画面の案 D1（今のアプリの外枠の中で、部門をページ内の列にする。囲みの中で状態が変わる形を基本にする）を実装する。挙動も変わるため見た目だけ先に変えず、㉘ 以降の棚卸し実装 lane と組にする。着手条件 = デザインの決まりの組み直し（D-091）と runtime lane A の merge（進行中の地・作業中の囲みを使うため）。開始前の画面と中断から戻った画面の見本は未作成。
- **旧 04 の未了の見た目の作業**（design-first で束ねるかは着手時に決める）: 旧 `docs/design-system/04-backbone.md`（`git show dda8560a:docs/design-system/04-backbone.md`）が約束したまま未了の項目。(a) 部門 select の幅を全画面で同一にする（`DepartmentFilter` の `widthClass` が `w-[10rem]` と `w-[11rem]` で混在）、(b) sidebar のラベルの折返し、(c) 未使用の `src/App.css`（参照 0）の撤去、(d) 月数回・年数回の画面の個別 sweep、(e) 押せる行の右端 chevron（hover 背景は `TableRow` の既定で全行に付いており、押せない行にも出るため、押せる印としては chevron か cursor が担う。今の一覧の行に右端 chevron は無い。採るかを含めて決める）。改訂後の 04 は原則 7 と「旧 04 の反映待ちの行き先」の表から本 entry を指す。
...
- summary card（在庫切れ・在庫少）の icon: 件数 ≥ 1 のときだけ title 左に小 icon（在庫切れ = 箱に×、在庫少 = 三角）を数字と同色で。昨日の売上は不要。owner 2026-09-16「今回は見送り」、件数色に慣れてから実機で見比べて判断
- 日報取込み「取込み済み」Alert の title を 1 文に（2 文が `line-clamp-1` で切れ得る。Opus P3、PR #64。L3 では 1 行に収まっていた）
...
- 最小ウィンドウ（`docs/SCREEN_DESIGN.md` の 1024×720）の縦の大きさの下限をもう少し小さくできるとありがたいという要望（ノート PC で画面が小さいため、owner 2026-08-20）。寸法の変更は設計判断で、ここでは記録だけ。
- 商品一覧の「PLU 対象にする／外す」は、押したらすぐ変わるのか、文言から結果を想像しにくい（owner L3 所感 2026-09-03）。
...
- D-023 POS adapter boundary の実装側未充足（2026-09-05 Fable 実測、2026-09-08 Codex 監査で訂正・再棚卸し）: **訂正** — 旧記載「`sales_repo.rs` 37 箇所が最多」は Codex 監査（2026-09-08、read-only、report は Coordinator 保管 `.local/codex-orders/reports/10b-pos-adapter-boundary.md`、gitignore 配下）で全件 `#[cfg(test)]` 内と判明。production の CASIO 固有語漏出は実測 41 行 / 61 出現（IO 層は正当な置き場所として除外）。本体は `DailyReportSourceKind`（Z001/Z002/Z005）が IO → BIZ → CMD → `bindings.ts` → frontend まで型のまま流れている点と、`schema_v4` の CHECK 制約が `casio_sr_s4000` / Z001 等を値として固定している点。**推奨** = B 初段（`io::casio` に literal と source/adapter ID を集約 + report id → `ReportKind` 変換関数、BIZ 以降は `ReportKind` だけを見る、legacy hash / JSON wire 互換は identity helper で維持、7〜9 file、migration 不要、trait / factory / registry は作らない）+ 副産物として「アプリが任意のレジに求めるデータ契約の要件表」を docs 化。A（adapter trait 導入）は第 2 機種の実契約が確認できた時のみ。**owner 状況**（2026-09-08 原文要旨）: 現行レジはリース契約で残り約 2 年、満了時に返却して新機種を探す見込み、「こっちのアプリに合わせて探す」方針あり = 機種変更は見える範囲、検討中。**起票判断**: 衛生 batch 3（⑯）の次の Codex 実装 lane として起票候補（M、R3）
  - リース満了の時期はおおむね決まっているはずだが、年と月は記録に無い（owner 2026-09-19）。満了年月は店主へ確認中。
...
- PLU へ移した商品と未移行の商品の見分け方と誤操作の防止は、店主に考えさせず運用設計の側で案を作る（owner 判断 2026-08-14。PLU 段階移行の運用設計で扱う）。
...
- **ECR+ のサービス終了とリース満了の関係**: CASIO の公式ページ（<https://web.casio.jp/ecr/ble/ecr.html>、2026-09-24 確認）では ECR+ のサービス提供は 2028年12月末に終了予定。店主は精算に ECR+ を要すると答えている（[project-memory](project-memory.md) Store Premises Facts）ため、レジのリース満了が 2028 年 12 月より後だと精算に支障が出る恐れがある（推論）。リースの満了年月は店主へ確認中で、その回答を待って ECR+ 終了後の精算とジャーナル閲覧の代替を判断する。

#### workflow / test / lint / docs

- ~~STATECAP 検査の stacked train 継承除外~~ → 解消（PR #97、2026-09-25 merge: STATECAP 検査そのものを `scripts/check-workflow-git.sh` から撤去したため、範囲判定の是正は不要になった）。
- **helper は Workflow State の値の末尾空白を拒否し、PK4 は削って受理する**: 例 `- Evidence Mode: github `（行末に空白）。`scripts/pr-gate.py` の `workflow_fields` は値をそのまま比較して拒否し、`scripts/doc-consistency-check.sh` の `extract_workflow_field`（`full`）は末尾の空白を削って受理する。PR #97 の前からある差で、helper 側が止める（fail-closed）向きのため gate の緩みではない。揃えるならどちらへ寄せるかを決める（PR #97 Final Review closure〈Opus〉の所見起源）。
...
- 店の事実を前提にする判断の前に `docs/project-memory.md` の「現場の前提」と照合する観点を、レビュー / Plan の観点へ足す（2026-09-19 ㉗ Plan Review round 3 が実店舗に本番データが在るという誤った前提で移行設計を議論し owner へ escalation した実発生、同日の owner 回答の複数件が過去に回答済みの内容だった実例が起源）。`docs/DEV_WORKFLOW.md` / `docs/templates/**` の改訂を伴うため Final Review 2 本が要る docs 小口として起票する。それまでは Coordinator の発注書に固定項目として運用する。
- Writer 発注書 template / 手順の follow-up 4 件（PR #82 closure comment 起源）: (1) `docs/templates/plan-packet.md` Registration 表の `operator 画面新設・改名・削除` 行は右列が `navigation.ts` entry 有効化のみで、削除・改名時の義務が読めない。`function-design doc 新設` 行だけ `新設・改名・削除` への拡張から漏れており、doc 削除時は `build_doc_to_modules_map()` の stale key が機械検出されない。(2) `docs/DEV_WORKFLOW.md` 訂正 sweep の live 定義（Workflow State fields, Scope, AC, Test Plan, Review Focus 等）に、`Plans.md` entry の現在地や gate 完了の主張のような status 記述を明示的に含める。(3) `docs/AGENT_OPERATING_MANUAL.md` §5.6 手順 2 の「指定する doc 節は `rg` で番号の実在と内容の一致を確認する」で、`rg` 手段を追加した際に旧文にあった対象との一致確認が脱落している。(4) archive 済み packet の AC5 / Scope の曖昧表現（既知 WARN M1）は archive のため対象外とし、追加是正は追わない。
- 型生成の失敗検出（PR #88）の follow-up 4 件（PR #88 Final Review（2026-09-22）起源）: (1) 型生成 CLI の smoke check — 合成関数 `export_bindings_to` の中の specta export / 整形 / 定数追記の失敗伝播、CLI の `exit(1)`、`run()` が起動を続けることを固定する自動 test が無い（reviewer の mutant 実測で生存）。production に失敗注入の seam を足さない方針のまま、隔離 fixture の再実行可能な smoke check として残せるかを検討する。(2) 失敗時の cleanup が他者の一時 file を消さないことを固定する test が無い（closure の mutant 実測で生存）。最初の候補を他者が確保 + 出力先を directory にして置換段で失敗させ、他者の file が残ることを見る test 1 本で足りる。(3) 一時 file の確保に失敗したときの error message が「export に失敗しました（出力先 path）」になり、実際の原因（親 directory が書込み不可など）へ辿りにくい。確保の段と親 directory が分かる message にする。(4) Windows native での `create_new` / rename による置換は未実測。次に Windows native で `tauri dev` を起動したとき、起動 log に「TS bindings の置換に失敗しました」等の警告が出ないことを確認する。
- **期間・動線の旧表記の残り 7 か所を店の事実に合わせる**（docs 小口 + code comment 1 行）: PR #93 で棚卸しの期間（準備〈10 月〜〉と年末の正式カウントを分ける、`docs/function-design/73-ui-stocktake.md` §73.1）と 1 日の動線（売上の取込みはレジ精算の後・店を離れる前、`docs/SCREEN_DESIGN.md` `### 利用者の1日の動線`）を直したが、次の旧表記は scope 外として残した: `src-tauri/src/biz/stocktake_service.rs:4`（code comment「10月〜大晦日」）、`docs/function-design/35-biz-stocktake-service.md:374`（「10月〜大晦日の長期作業」）、`docs/architecture/biz-task-specs.md:451`（同）、`docs/db-design/tracking-system-tables.md:137`（同）、`docs/screen_mockups.html:208`（図中の「10月〜大晦日」）、`docs/screen_mockups.html:259`（ボタン説明「閉店後にレジのCSVを読み込む」）、`docs/SCREEN_DESIGN.md:213`（「数週間スパン」）。35 と biz-task-specs は時点証拠 ADR の同期先で、㉗ ADR 修正 lane と同じ file を編集し得る（merge 順で解消）。PR #93 packet の Non-scope と Final Review broad（Opus）の closeout 申し送り起源。
...
- 在庫照会の絞り込み時の件数行は `text-base font-semibold`（catalog ⑩ の PaginationSummary `text-sm muted` と非対称。owner L3 2026-09-15 で気にならないと判断。統一は別 lane、catalog ⑩ に例外を記録）
...
- 61-ui-receiving.md:30/:73・63-ui-return-exchange.md:36/:71 の保存結果導線列挙に「詳細を見る」が無い（PR #20 由来の既存 drift、実装は link あり。次に 61 / 63 を触る lane で同期。㉕ Final Review pass B 起源）
- `ActionButton.tsx` の `size-6 h-6 w-6` 重複 / `SummaryCards.test.tsx` の `pluDirty` override 引数（死に配線）/ `reference/mockup-d-home-sales-admin.html` の「現実装: サマリ 3 枚・説明文なし」は PR #70 前の記述（reference は非正本）— 次に触る lane で整理（㉔ Final Review P3）
