...
- 廃棄・破損の保存結果に「詳細を見る」+ `returnTo` を追加するか（現行 UI-05-D17 は「保存結果に link なし」を契約化済み。追加には UI-05-D17 改訂の design 判断が先行 — PR #23 owner L3 所感 2026-08-31 起源。歴史的非対称であり表示すべきでない業務理由は source docs に見当たらない、が owner 観察）。（**owner 2026-09-11「やったほうがいい」で格上げ**: UI-05-D17 の改訂を design 判断 1 行で先行し、入庫・返品交換と対称化する小 runtime lane）（**2026-09-16 起票 = ㉕、`plans/2026-09-16-disposal-result-detail-link.md` / `plans/test-matrices/2026-09-16-disposal-result-detail-link.md`、wave 11 lane 2。R3〈route/search state の producer 追加〉**）
