...
- **ボタンの最小サイズの不一致（docs 小口）**: `docs/UI_TECH_STACK.md` §5.6 の 44×44px（WCAG 2.1 AAA）と `docs/design-system/04-backbone.md` の token 表 min-height 40px が食い違う。どちらを正とするか決めて揃える（design-first）。整理の向き: 日常的に押すボタンは 44×44 CSS px 以上を製品標準、40px は密度の参考値、24×24 は WCAG 2.2 の最小基準であって十分条件ではない、という区分を軸にする。
