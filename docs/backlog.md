...
- doc-consistency の WARN 5 件掃除（supplier カラムの DB_DESIGN 未定義 3 / ページング上限未定義 1 / M3 マーカー 1〈2026-09-13 実測 4 箇所、本行の旧文言を含む〉）: owner 2026-09-08「残しておいてもいいものでもない」→ ⑯ の次の衛生 lane 候補。Dependabot の default branch 指摘（2026-09-11 push 時表示は 3 件 = moderate 2 / low 1、npm 常設ガード下で名指し確認。**2026-09-13 実測 7 件 = npm dev 5〈js-yaml high / smol-toml high / vitest 系 medium 3、fix version はいずれも publish 7 日超〉+ Cargo 2〈rand 0.7 = tauri-utils build 経路、glib 0.18 = gtk 経由 Linux のみ、当方で fix 不可〉。npm 5 件は名指し更新を同 lane に含め R2 へ、Cargo 2 件は記録目的**）も同 lane。**owner 2026-09-11「後回しにしてるけど直すタイミング設けよう」→ ⑲ merge 後・runtime lane の前に衛生 lane（R1 docs-only、Codex 1 発注）として実施する**。mockup-g の `:has()` selector（参照 mockup 中 g のみ、動作影響なし、⑳ Final Review round 3 Opus P3-11）も同 lane で `.legacy-header` 等の明示 class へ。現行の範囲・R2 判定・測定値は [衛生batch 4 packet](plans/2026-09-14-hygiene-batch-4-doc-warn-deps.md) を参照。
