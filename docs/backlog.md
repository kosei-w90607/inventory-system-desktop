...
- **一括価格改定の取引先紐付けを既定 off + 文言明示**（owner 決定 2026-09-16、L3 round 3 所感「価格改定のついでに取引先が変わる」）: `PriceRevisionPage.tsx:32` の `useState(true)` → `false`、supplier 変更時も off のまま、label 文言「確定した商品の取引先が未設定なら、この取引先を設定する」。設計書 77 REQ-106 / SPEC-PRV-D6 / §UI 記述の「既定 on」「取引先変更時に on へ戻す」を改訂、`PriceRevisionPage.test.tsx:186` の既定 on assert を反転、decision-log 1 件。R2、㉒ と file 互いに素、㉑ closeout 後に起票（**2026-09-16 起票 = ㉓、[Plan Packet](plans/2026-09-16-price-revision-assign-default-off.md)**）
