...
## Rehome addendum（2026-08-23）: public → public full-history replay

本 runbook の `Prepare the snapshot` から `Visibility and development cutover` までの手順は、private repository から sanitized な parentless single-commit snapshot を作成して初めて public 化する Phase B を対象とする。すでに public だった履歴を別の public repository へ再移植する手順ではない。

2026-08-23 の rehome では、`Claude-Session:` trailer を含む旧 public repo `inventory-system-public` の履歴を、trailer の除去と、再生成で変わった commit SHA への docs 内言及の追随を行いながら、新 repo `inventory-system-desktop` へ full-history replay した。author / committer identity と日付は保持し、tree は SHA 言及箇所以外 byte 同一とした。詳細な数値と判断理由は [D-077](decision-log.md#d-077-public-repository-の再移植--inventory-system-public-から-inventory-system-desktop2026-08-23) を参照する。

移植後は検証項目がすべて PASS し、独立した 2 run の結果が一致した。root snapshot `902647b` は byte 同一で SHA も不変だった。本 change の merge 後、owner が新 repo を public 化してから旧 repo を private 化する。
