...
| [mockup-g-filter-toolbar.html](mockup-g-filter-toolbar.html) | フィルタ入力の Label 上置き・セクション見出し：商品一覧の現行 / 提案 a / 提案 b、棚卸し、形態 C の比較と形態 A 不変の注記 | 提案。4 状態の静的 HTML、外部資源なし、合成データ。SegmentedControl の上置き Label と Checkbox 横並びは Human Gate で確認 |
