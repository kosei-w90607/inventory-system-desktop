...
| [mockup-d-forms-b.html](mockup-d-forms-b.html) | 統合案 D（入力画面、Lane 1b→Lane 2 実装値同期→Gated Amendment 4 closure round 1 で現実装 + Lane 2 差分へ限定）の手動販売出庫 / 廃棄・破損 / 棚卸し: 入力枠 `--border-strong` 化。識別列固定・sticky 上部件数帯は商品一覧 pilot のみの `ListShell` 実装のため棚卸しにも描かず後続候補へ移す（DSR-22 mapping 自体は Lane 3〜5 の `ListShell` 移行時に適用）、末尾 4 区分 note | 同上 |
