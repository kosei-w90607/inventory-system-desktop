...
- **(c) container を安定識別する**: route content の唯一の scroll container `<main>` に `data-scroll-restoration-id` を付与する。CSS 階層や class の位置に依存した cache key を使わない。例外: Lane 4 Gated Amendment 1 の対象画面（商品一覧、2026-09-06、pending 実装）では、`<main>` の代わりに専用 scroll 箱に `data-scroll-restoration-id="products-list"` を付与し、箱を安定識別する。
- **(d) `<main>` を先頭 scroll 対象にする**: `scrollToTopSelectors` へ `<main>` の selector を設定し、復元 miss 時と分類④の先頭 scroll が `window` ではなく実際の scroll container に作用するようにする。例外: Lane 4 Gated Amendment 1 の対象画面（商品一覧、2026-09-06、pending 実装）では、`scrollToTopSelectors` に専用 scroll 箱の selector（`[data-scroll-restoration-id="products-list"]`）も加えた 2-selector 配列にする。
...
| 日付 | PR | 内容 |
|---|---|---|
| 2026-09-06 | UI 一覧の背骨 D — Lane 4（Gated Amendment 1、pending 実装） | DSR-17 の Why・(c)・(j) に、商品一覧限定で `<main>` の代わりに専用 scroll 箱が唯一の scroll container になる例外を注記（PR #40 owner L3 run 1 FAIL・案 X、Plan Review round 1 P1-2）。実装（app-router.ts 側の resolver 導入）は Writer が別途行う；本注記は正本 doc の断定文が実装前から矛盾しないための先行同期 |
