...
- **(i) async content による復元 clamp を 1 回だけ再適用する**: router の復元後、app 層の `onRendered` handler は `getElementScrollRestorationEntry` で `<main>` の保存値を取得し、保存 `scrollY > 0` かつ実 `scrollTop < scrollY` の clamp 検出時だけ遅延再適用を armed する。`<main>` subtree の childList を `MutationObserver` で監視し、`scrollHeight >= scrollY + clientHeight` に達したとき `scrollTop = scrollY` を 1 回だけ適用して解除する。適用前の `wheel` / `pointerdown` / `keydown`、または次の `onBeforeLoad` でも解除し、armed は常に高々 1 本とする。分類④ flag が現 location と一致した場合は (g) の先頭 scroll だけを行い、遅延再適用を起動しない。
