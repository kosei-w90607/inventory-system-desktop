...
- **(c) container を安定識別する**: route content の唯一の scroll container `<main>` に `data-scroll-restoration-id` を付与する。CSS 階層や class の位置に依存した cache key を使わない。例外: Lane 4 Gated Amendment 1 の対象画面（商品一覧、2026-09-06、実装済み）では、`<main>` の代わりに専用 scroll 箱に `data-scroll-restoration-id="products-list"` を付与し、箱を安定識別する。
