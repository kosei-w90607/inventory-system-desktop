...
- **(i) async content による復元 clamp と成功後喪失を 1 回だけ再適用する**: router の復元後、app 層の `onRendered` handler は `getElementScrollRestorationEntry` で `<main>` の保存値を取得し、保存 `scrollY > 0` があれば初回復元の成否にかかわらず遅延再適用を armed する。`<main>` subtree の childList を `MutationObserver` で監視し、`scrollHeight >= scrollY + clientHeight` かつ実 `scrollTop < scrollY` のとき `scrollTop = scrollY` を適用する。router の初回適用に加えて app 層の再適用は 1 回を上限とし、再適用上限の到達、適用前の `wheel` / `pointerdown` / `keydown`、または次の `onBeforeLoad` で解除する。初回適用の成功だけでは解除せず、armed は常に高々 1 本とする。分類④ flag が現 location と一致した場合は (g) の先頭 scroll だけを行い、遅延再適用を起動しない。
