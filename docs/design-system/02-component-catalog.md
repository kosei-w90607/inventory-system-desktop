...
// DepartmentFilter 内部（上置き Label、runtime 反映済み（本 PR））: label は raw label 要素、htmlFor は SelectTrigger の id を指す
