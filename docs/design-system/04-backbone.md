...
2. **1 色 1 役割にし、色だけに頼らない。** 色は 00 の色の役割表の 6 役割（操作 / 進行中 / 注意・確認 / 完了 / 危険・失敗 / ふつう・補足）で使い、1 つの色に 1 つの役割だけを持たせる。どの状態も文字か記号を添え、色だけに頼らない（00 の色の役割表の注記）。icon を必須にする範囲は非中立の状態 badge と Alert の個別規則（DSR-08、02 ⑬ ⑥）に従い、入力欄のエラー文のような役割色の文字だけの表示は文言・記号・下線のどれかを添えれば icon を要しない（00 の迷いやすい場面）。役割の無い色を足さず、token は使う前に 00 の色表へ登録する。役割ごとの家族は soft・border・strong・emphasis の段の形を持つ（00 の token 表）。進行中は info（お知らせ）ではない。手を止めて確かめてほしい知らせは注意・確認、手を止めなくてよいお知らせ一般はふつう・補足（Alert の既定）で示す。
   - 受け取り方と効果: 色を見ればその意味が 1 つに決まり、押す・注意・現在地を取り違えない。色が見分けにくい人も文字と記号で分かる。
...
| 旧原則 3 | 状態は icon + 日本語 + 色の 3 点 | 2 | 射程を置換: どの状態も文字か記号を添え、色だけに頼らない（00 の色の役割表の注記）。icon を必須にする範囲は非中立の状態 badge と Alert の個別規則（DSR-08、02 ⑬ ⑥）に委ね、入力欄のエラー文のような文字だけの表示は icon を要しない（00 の迷いやすい場面） |
