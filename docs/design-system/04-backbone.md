...
13. **UI 部品の枠は 3:1、構造線は一段濃くする。** 操作枠（入力・ボタン outline・select・segmented・状態 badge の枠・focus ring）は隣接背景に対し 3:1 以上（WCAG 2.2 SC 1.4.11 非テキスト、focus は 2.4.13 の 2px + 3:1）。構造線（行区切り・表枠・card 枠）は 3:1 の対象外だが、現行 `--border` 実測 ≈1.20:1 は低視力での行追従を妨げるため一段濃くする。token は `--border`（構造線）と新設 `--border-strong`（操作枠）の 2 段。提案値 = 構造線 #cdc8c4（対 `--background` 実測 1.66:1）/ 操作枠 #8a8480（対 `--background` 実測 3.69:1 / 対 `--card` 3.53:1、確定値は Lane 2 の L3 で確定する — token 最終値の確定は本 PR の非目的）。新しい色相は追加しない（Q7 原則①「色数をむやみに増やさないこと」`ui-design-rules-qa-v2.md` 4-1/7-1、原田秀司『UIデザインの教科書［新版］』翔泳社、2020）。（DSR-22、旧 SPEC-UILB-D1〈2026-08-23 旧 Lane 1〉を承継）
14. **一覧の器は上下に件数、header は sticky、識別列は opt-in で固定する。** viewport を超える一覧は table の上下に件数 + 現在位置を置き、統一形「{n} 件中 {from}〜{to} 件目 · {p} / {t} ページ」で pin する（上部は件数 + 現在位置テキストを必須、pager ボタンは任意 — 同じ操作ボタンが上下 2 組あると選択肢が増え判断コストが上がるため、Q12 §1「初めてインターフェースを触るユーザー（初心者）にとっては、操作体系はシンプルなほうが使いやすい（インタラクションコストが少なくて済むため）」`ui-design-rules-qa-v2.md:419`〈同書〉を根拠に安全側にする）。table header は sticky、識別列（商品コード + 商品名等。履歴系は日時 + 種別）は横スクロール時に左固定する opt-in。ソート機能を持つ一覧は基準列を明示する（Q5 原則①、5-4「並べ替え時の基準列明示」）。検索・絞り込みは枠に入れて余白でグループを示し（Q5 原則③、6-9「近接の要因」）、続きがあると分かるよう最終行を意図的に見切れさせてよい（Q5 原則④、6-9「要素の見切れ」）。左 rail への 3 大操作常設（Q5 原則②、5-4）は、本アプリでは sidebar（22 項目、`src/config/navigation.ts` の `status: "active"` 実測値、`RootLayout.tsx:62`）が既に左を占有しているためそのまま採らず、横 toolbar 2 段（原則 6）で代替する（**Q5-② 適用外注記**）。ページングは現在地・件数を保ち「コントロール感」を維持できるため、探索・比較・編集が主目的の一覧では無限スクロールでなく本則とする（Q15、6-8）。同型レコードの一覧の器を統一することは、情報のグルーピングと選択負荷の低減にも資する（Miller's Law / Law of Common Region、Jon Yablonski『UXデザインの法則』第 2 版、相島雅樹・磯谷拓也 訳、オライリー・ジャパン、2025-01。DSR-19/20/21 と同型の引用範囲に限定し、理論書の主張をそのまま規則として転記しない）。（DSR-22 + catalog ⑯、旧 SPEC-UILB-D2 を承継。perPage 既定値は共有定数 1 本を維持し画面ごとに既定値だけ変える裁定案 — 詳細は catalog ⑩）
15. **現在の行は 3 点で示す。** 入力中 / 開いている行（inline 展開）/ 選択行は、左 4px の primary バー（形状）+ 淡い背景 `--row-current`（提案 #fff8e6、対 `--foreground` 本文 16.5:1）+ badge または文言（「入力中」等、DSR-08）の 3 点で示す。hover は従来の accent のまま。色数を絞った上で文字サイズ・形状の強弱をつけるほど視線誘導が効く（Q8 原則④「色数を絞った上で文字サイズの強弱による視線誘導」7-1、同書）ため、行全体の primary 塗りはしない（原則 5 の primary 1 つに反する）。（DSR-22 + catalog ⑯、旧 SPEC-UILB-D3 を承継。実測値は Lane 2 の L3 で確定）
16. **低視力を前提にした L3 項目と行内操作の当たり判定を持つ。** 操作者画面の L3 checklist に (a) Windows forced-colors（ハイコントラスト）で状態・枠・focus が消えない (b) DPI 125% / 150% で崩れない（rem / em 基準、px 直書きを避ける）(c) 実利用者（緑内障等）の Windows native 1 セッションを代表画面で、を含める。行内の icon のみボタンは見た目 16px のまま当たり判定 24×24 以上（padding）。見出しと本文を明確に区別し（Q8 原則①「見出しと本文の明確な区別」4-2、同書）、PC はスマートフォンよりやや遠い距離で使われるため実サイズを大きめに保つ（Q8 原則③「PC はやや遠い距離、実サイズを大きめに」2-3、同書）。**Q12 caution**: 高齢者・IT に不慣れな利用者への直接の配慮規定は同書に記載がなく（Q12 §2/§3、`:424-433`）、operator profile（非 IT・年配の店主）の根拠は QA v2 側からではなく既存の WCAG / Laws of UX 側から引く。（DSR-22、旧 SPEC-UILB-D4 を承継。実装 lane の Human Gate 文面に転記）
...
| 枠（提案、原則 13） | `--border`（構造線、一段濃く #cdc8c4）/ 新設 `--border-strong`（操作枠 #8a8480） | 3:1 は操作枠のみ対象、構造線は対象外（DSR-22） |
| 現在行（提案、原則 15） | 新設 `--row-current`（#fff8e6） | 左 4px primary バー + badge/文言と併用（DSR-22） |
...
| 日付 | 内容 |
|---|---|
| 2026-09-03 | wave 8 lane 1 refresh。原則 13〜16 を新設（DSR-22 / catalog ⑯ を承継、Q5/Q7/Q8/Q12/Q15/Q17 と Laws of UX を Why に反映）。見出しを「16 の原則」へ改題（旧見出しから 4 項目増）し、badge 3 種を指す宙ぶらりん DSR-16 参照 3 箇所（token 表 / 反映先 / 適用の順序）を是正 |
