...
- 各遷移は表の条件が揃うまで進めない。そのほかに止まる場面は本書と [AGENT_OPERATING_MANUAL.md](AGENT_OPERATING_MANUAL.md) が定める。例: field の欠落・enum 外（下の fail-closed）、下の packet 選択規則の不一致、owner の指示を待つ Ready / merge / R4 承認、GitHub Actions が使えない間の merge、Owner Effort Budget の hard stop と goal-drift signal、[AGENTS.md](../AGENTS.md) の Decision and Approval Boundaries が承認を求める破壊的・外部・高コストの操作。本書と AGENT_OPERATING_MANUAL.md が停止を定める場面以外では、報告のために止まらず、許可済みの作業を続ける。
