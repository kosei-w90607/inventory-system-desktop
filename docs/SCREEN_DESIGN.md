...
SPEC-STK-TIME-D1 / D7〜D9の画面変更案。以下の実装済み一覧と現行操作は、この案の実装完了を表さない。

- [棚卸し73](function-design/73-ui-stocktake.md): 商品確認後に「今から数える」、応答後に実測して「数を保存」。古い値の転記をせず、記録変化時は新しく数え直す。
- [取込み55](function-design/55-ui-csv-import.md): 準備不足・要再確認（相殺の確認待ちを含む）・共有JAN行の保留を区別する。要再確認は取込みを止めず、対象商品の数え直しへ案内する。共有JAN行の保留は対象商品を一つずつ保存して再previewへ進め、再起動は同file再選択から再開する。
- [商品設定51](function-design/51-ui-product-form.md): 非連動へ切り替える前に在庫数が直らないことを説明し、保存後/再訪にも未調整の回復先を表示する。電子ジャーナルの毎日の取込みが使えるようになるまで在庫連動を有効にできないことを表示する（商品一括import60も同じ）。55の準備表示も、非連動化の未調整と拒否された未取込み資料を再照会して保持する。
- [記録詳細65](function-design/65-inventory-record-traceability.md): 「今の数を確認して直す」から同じ計数操作へ。確定済み評価額は変わらないことを明示し、差0の商品も対象として探せる。

既存のPageShell/表/警告/feedbackとreturnToを使う。状態は文字と非色シグナルを併用し、数量・単位・保存先・次操作を省略表示で隠さない。実機で確認する操作と合格条件は73の新契約に集約し、合成モデルをUI/nativeの完了証拠にしない。

> **このファイルの目的**: 画面設計の意図・判断・気付きを保存し、セッション間で引き継ぐ。現行の到達・戻り経路は [画面遷移図とroute対応表](diagrams/current-system.md) を参照する。`screen_mockups.html` は初期提案の歴史資料で、現行実装の図ではない。
>
...
[現行の画面構成・主要な往復・route対応表](diagrams/current-system.md#画面の構成) は `navigation.ts` / `src/routes/` / 各画面のLinkを照合している。図示により判明した戻り経路の不整合は [図面監査](research/2026-09-16-diagram-audit.md) に分離し、本書の期待動作と混同しない。
...
- **青（商品管理）**: 商品検索・一覧 → 商品登録 / 商品修正 / 一括インポート、PLU書出し、一括価格改定
- **オレンジ（入出庫）**: 入庫記録 / 返品・交換 / 手動販売出庫 / 廃棄・破損 / 入出庫履歴 / 在庫少一覧 / 棚卸し。在庫変動履歴は商品詳細・記録詳細から到達する
- **黄（システム管理）**: バックアップ・復元 / 操作ログ / 在庫少の基準 / 整合性検証 / 取引先管理

### 利用者の1日の動線
```
開店 → 在庫照会（発注判断） → 入庫記録（商品到着時）
→ 営業中（レジ販売の合間にPC作業（商品登録・編集・廃番など）が発生し得る）
→ レジ精算（店は開けたまま）→ 売上データ取込み（日報Z001/Z002/Z005、店を離れる前）→ 店を離れる
```

売上レポートの確認を1日のどの時点で行うかは決めていない。バックアップはアプリの自動バックアップ（起動時と設定時刻、[`function-design/71-mnt-backup.md`](function-design/71-mnt-backup.md) §71.8）が取り、PC の外へのコピーを1日のどの時点で行うかは決めていない（[backlog](backlog.md) のバックアップの項）。

### 主要な画面間遷移
- ホーム → 各機能画面（入口 card で1クリック）
...
- **レイアウト判断**:
  ...
  - 毎日使う4機能を上段に2×2の入口 card（icon + 題名 + 1 行説明。3 区画とも同じ card）で配置
  - 入出庫4機能を中段に2×2
  - たまに使う機能（棚卸し/バックアップ/設定）を下段に同じ card で 2 列（3 枚、折返し）
- **利用者配慮**:
  - 全ボタンにタイトル＋説明文（「何をするボタンか」がボタン自体で伝わる）
  - 「売上データ取込み」は毎日の最重要操作なので primary 枠 + warning soft 背景で強調（icon 24 + 題名 + 1 行説明の入口 card、mockup-c 採用 2026-09-11）
  - サマリカードの補助文言は状態の説明であって導線ではない（D-089。例: 在庫少「基準を下回る商品」、在庫切れ「在庫 0 の商品」）
  - 在庫切れ件数 / 在庫少件数は日本語ラベルを主情報とし、赤 / 黄色は危険度を補助する強調として使う（件数が 1 以上のときだけ数字に色を付け、0 件は無色。2026-09-16 実装）
...
- **レイアウト判断**:
  ...
  - 現行 build では一時停止中（Z004 の取込みの確定・取消）。[73](function-design/73-ui-stocktake.md) / [55](function-design/55-ui-csv-import.md) / [停止 ADR](adr/2026-09-23-legacy-stocktake-z004-write-stop.md) 参照
- **利用者配慮**:
...
- **レイアウト判断**:
  ...
  - 現行 build では一時停止中（棚卸しの開始・カウント入力・確定）。[73](function-design/73-ui-stocktake.md) / [55](function-design/55-ui-csv-import.md) / [停止 ADR](adr/2026-09-23-legacy-stocktake-z004-write-stop.md) 参照
- **利用者配慮**:
...
- **利用者配慮**:
  ...
  - 備考は保存結果、直近一覧、返品・交換詳細で「備考」と分かる独立ラベル付き領域にし、本文は通常本文色で読める濃さにする。入力なしの場合は「—」と表示する
...
- **レイアウト判断**:
  ...
  - 検索欄は商品名 / 商品コード / JAN コード / メーカー品番を同じ入力で扱う（SPEC-PRV-D2）。バーコードスキャナは HID キーボード入力として検索欄 + Enter で扱い、専用スキャンボタンは初回実装では置かない
  - 部門フィルタ、廃番表示モード（表示中 / すべて / 廃番のみ）、並替え、ページングを同じ画面で扱う。部門候補は検索結果の現在ページから作らず、部門マスタ全件を取得する
  - 商品コード、商品名、部門、売価、原価、在庫数を一覧で見せる。原価は売価の右隣に置く（UI-01a-D13）。商品コードだけで利用者判断を強制しない。廃番状態は専用列を持たず、商品名セル内に廃番 badge と行 muted で示す（UI-01a-D8）
