...
- [取込み55](function-design/55-ui-csv-import.md): 準備不足・要再確認・保留を区別し、対象商品を一つずつ保存して再previewへ。再起動は同file再選択から再開する。
- [商品設定51](function-design/51-ui-product-form.md): 非連動へ切り替える前に在庫数が直らないことを説明し、保存後/再訪にも未調整の回復先を表示する。55の準備表示も、非連動化の未調整と拒否された未取込み資料を再照会して保持する。
