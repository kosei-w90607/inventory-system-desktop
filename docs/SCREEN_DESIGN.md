...
- **利用者配慮**:
  ...
  - `入出庫履歴` は表示のたびにページ先頭へスクロールする（他画面での CSV取込み・日報取込みの取消完了後に本画面へ戻ってきた場合を含む）。layout の `<main>` は route 遷移をまたいで保持されるため、既存 4 画面（L263 / L283 / L417 / L419）と同型の先頭スクロールパターンをこのハブにも適用する。
