...
### 直近の作業状態
- **file 契約一元化（監査是正 順9）**: **実装中 / Final Review 一次是正反映**（R3、2026-07-27）。Rust 上限定数から bindings へ生成 export し、売上・日報・商品 import hook を単一契約へ統一。商品 import の CMD+BIZ 二重 guard、dialog+drop 共通 FilePicker、Z004 実 hook test、静的再導入 sweep、返品画像 Blob プレビューを実装した。Final Review 一次で受理した drop filename 正規化消失、55-ui stale 記述、5 call site の Label in Name 不整合は、共有 basename 抽出 + X10、設計記述追随、accessible name 是正として反映。synthetic 3 画面の初期／選択後 state は dev で確認済み。active [Plan Packet](plans/2026-07-26-file-contract-unification.md) / [Matrix](plans/test-matrices/2026-07-26-file-contract-unification.md)。次は Draft PR の差分再レビューと X10 実測確認。
