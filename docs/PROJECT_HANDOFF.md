...
>
> **最終更新**: 2026-08-26 / PR #4（public repo）取引先管理 実装 merge。UI-15 `/settings/suppliers`（一覧 + usage 件数 + 追加 + インライン改名 + 2 段階統合）、migration v6（suppliers.updated_at）、supplier 3 command、D-052 C21/C22 が main に入り、REQ-107 は required へ昇格。前提の design-first は PR #3（2026-08-25、SPEC-SUP-D1〜D10 正本化）。旧記録: 2026-08-23 / PR #95 価格改定支援 実装 B（UI-14 一括価格改定画面 + `ProductSearchQuery` 取引先 filter）squash merge。UI-14 route / navigation / 行単位確定 / 最近改定 badge / D-052 C20 / 共有 ListSkeleton が main に入り、次は実装 PR C（入庫 cost_diffs、D-052 は C21 以降）と UI 一覧の背骨 D（L3 所見 5 点の規範化 → shared 部品 → 画面群 lane、Lane 0 完了・mockup D 公開済み）
