...
### 直近の作業状態
- **error 境界 + PLU 語彙（監査是正 順8）**: **実装内容完成・検証中**（R3、Phase: implementing）。`CmdError.error_id` と internal sanitize、restore 3 kind の相関、PLU `export_error` / 内部語彙 rename、共通 `describeError` と復元画面の ID 別要素併記を実装した。raw message 直接表示の残りは owner 裁定どおり非 scope。次は exact-HEAD L1、独立 review、Human Gate。正本は active [Plan Packet](plans/2026-07-26-error-boundary-plu-vocabulary.md) / [Matrix](plans/test-matrices/2026-07-26-error-boundary-plu-vocabulary.md)。
