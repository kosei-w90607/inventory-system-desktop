...
>
> **最終更新**: 2026-07-26 / 監査是正 順8「error 境界 + PLU 語彙」はPR #25で完了・archive済み（現在状態は `Plans.md` を優先）
...
### 直近の作業状態
- **GPT-5.6 agent guidance / local override分離（D-057）**: **実装中**（R2、docs-only）。全Codex共通の判断・approval・検証契約とslot-neutral profileをtracked guidanceへ置き、具体model対応はAGENT_OPERATING_MANUAL §3.4だけが所有する。個人用の応答人格と実測出力はignored root override / local-only evidenceへ分離し、公開fixtureだけをrepositoryへ残す。最新Phaseと次actionは[Plans.md](../Plans.md)を参照。
