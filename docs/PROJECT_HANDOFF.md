...
- **CSV取込み記録詳細（65 slice 4b）**: **完了**（R3、Writer = Codex、PR #58 squash merge `3ec84c4`、2026-08-03）。在庫変動履歴「CSV取込み #n」link の 404 を解消 — IO / BIZ / CMD の詳細取得 + generated bindings + `/csv-import/records/$importId` 詳細 route / Page + rollback invalidation（D-052 C9 へ `inventoryRecords.root()` 追加）+ T1-T17。CSV取込み画面は layout + index route へ再構成（gated Amendment 1、Writer fail-closed の true positive 検出起源）。owner L3 P3（ErrorRowsTable accordion の発見性）は owner 裁定の同 PR 是正で「エラー詳細を見る（N件）」の明示的操作文言へ改修し再 L3 PASS（gated Amendment 2、55 §55.5 同期）。編成 = Fable Coordinator / Codex Writer / Sonnet Plan+Final Reviewer（D-062 (c) 適合形の初運用）。Final Review Ledger 22/22・P1/P2=0、mutation 独立再実測 X1-X7 全 red、exact-HEAD 三点一致（run `30767350640`）。証跡は archived [Packet](archive/plans/2026-08-03-csv-import-record-detail.md) / [Matrix](archive/plans/test-matrices/2026-08-03-csv-import-record-detail.md)。follow-up = 一覧 route / 棚卸し詳細 / 74 許可リスト追随（後続スライス）、JAN 列の pre-existing drift（Plans.md backlog）。
- **店舗調査証跡と導入時運用契約の同期**: **完了**（R3 docs-only、PR #55 squash merge `fc19daa`、2026-08-01）。Z004を個数・金額を持つ確認済みPLU別売上として整理し、BIZ-03の在庫増減・rollback実装済み範囲と、layout A/B対応・店舗shape再検証の残件を分離した。日報取込みは`SD -> CV17 -> PC側EcrDatas`を標準手順とし、layout差はoperator選択ではなくadapter互換性として扱う。Excelファイル群の毎日上書き、印刷・バインダーの日別履歴役割、紙代替operator受入の未完了も正本化。Sonnet 5のPlan Review / Final Contract Auditは最終P1/P2/P3=0、Ready HEAD `5251cecb`のlocal / hosted run `30664891332`三点一致を確認した。証跡はarchived [Packet](archive/plans/2026-08-01-field-evidence-operations-sync.md) / [Matrix](archive/plans/test-matrices/2026-08-01-field-evidence-operations-sync.md) / [WER](archive/plans/2026-08-01-field-evidence-operations-sync-workflow-effectiveness-review.md)。後続はZ004 layout A/B、固定slot / bulk onboarding、EcrDatas lifecycle、紙代替受入の各R3。
- **CI 2026-08-01 re-evaluation / public Actions contract**: **完了**（R3 workflow gate、PR #54 squash merge `69ec8fc`）。public repository + standard `ubuntu-latest` ではprivate-era billed-minute thresholdを使わず、completed HEADの状態に応じてReady / dispatch / recovery / no-opをCI-TRIGGER-D1から選ぶ契約へ更新した。live-doc drift guardは`ci-workflow.test.sh`を`local-ci.sh full`から実行するL1 local-onlyで、hosted docs jobは未配線。Sonnet 5 Plan ReviewとFinal Double Audit closureはP1/P2=0、Ready HEAD `01a4f9c`のlocal full / PR body / hosted run `30654559831`の三点一致を確認し、予防的dispatchなしでmergeした。証跡はarchived [Packet](archive/plans/2026-08-01-public-actions-ci-contract.md) / [Matrix](archive/plans/test-matrices/2026-08-01-public-actions-ci-contract.md) / [WER](archive/plans/2026-08-01-public-actions-ci-contract-workflow-effectiveness-review.md)。hosted実防御が必要になった場合の`doc-consistency-check.sh`統合は別R3、scheduled npm monitor dogfoodは2026-08-03 JST、dependency change Bは2026-08-05以降の別change。
- **Claude Hook contract audit**: **完了**（R3 workflow gate、PR #48 squash merge `985eb76`、2026-07-31）。repository-owned project hookを0本へ縮約し、未監査harnessをproject単位で無効化。source-derived fixtureによるsettings / docs / hook propagation、classifier、local full、hosted CI配線を固定した。consultation relayのPlan Gate 3 roundとFinal Double Audit + frozen closureでP1/P2=0、P2-CH10 CLOSED。Ready exact-HEADのlocal / hosted三点一致後にmergeし、Matrix CH10表示名もcloseoutで同期済み。証跡はarchived [Plan Packet](archive/plans/2026-07-31-claude-hook-contract-audit.md) / [Matrix](archive/plans/test-matrices/2026-07-31-claude-hook-contract-audit.md)。次dogfoodは次のClaude Code fresh sessionでproject hook 0本・harness無効・虚偽hook注入なしを確認する。
- **D-058 Fable slot不在編成の相談窓口役**: **完了**（R3 workflow gate docs-only、PR #47 squash merge `3009c77`、2026-07-30）。target PR外の専用remote order branch、order / target両refのcurrent-head照合、target SHA時点Packet、Coordinator予約枠、template `Consultation Relay`欄を正本化した。Double Audit closure P1/P2=0、Ready HEAD `56f76d4`のexact local / hosted三点一致を確認し、Post-Freeze P3 2件もcloseoutで是正済み。証跡はarchived [Plan Packet](archive/plans/2026-07-30-d058-consultation-relay-role.md) / [Matrix](archive/plans/test-matrices/2026-07-30-d058-consultation-relay-role.md)。最初のdogfood targetは次のeligible consultation relay order。
