...
### 直近の作業状態
- **CI 2026-08-01 re-evaluation / public Actions contract**: **re-Plan Gate**（R3 workflow gate）。public repository + standard `ubuntu-latest` の公式 billing contract と実 run / cache / protection 状態を再確認し、private-era billed-minute threshold を current gate から退役、completed HEAD ごとの Ready / dispatch 選択を CI-TRIGGER-D1 へ一意化する source draft を起草した。wiring probe 後、live-doc drift guard は `ci-workflow.test.sh` を `local-ci.sh full` から実行する L1 local-only と訂正し、hosted docs job は未配線であることを明記した。workflow YAML は不変、Sonnet 5 の fresh independent re-Plan Review 待ち。hosted 実防御が必要になった場合の `doc-consistency-check.sh` 統合は別 R3、scheduled npm monitor dogfood は 2026-08-03 JST、dependency change B は 2026-08-05 以降の別 change。[active Packet](plans/2026-08-01-public-actions-ci-contract.md) / [Matrix](plans/test-matrices/2026-08-01-public-actions-ci-contract.md)
