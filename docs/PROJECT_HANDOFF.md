...
>
> **最終更新**: 2026-08-22 / PR #94 価格改定支援 実装 A（backend + UI-01a/01b）squash merge。`revise_product_price` / `create_supplier` / `list_price_history` + keyword maker_code + 原価列 + 価格履歴 + 取引先 inline 追加が main に入り、次は実装 PR B（UI-14、`ProductSearchQuery` の取引先 filter 未定義を先に裁定）/ C（入庫 cost_diffs、B と独立）
