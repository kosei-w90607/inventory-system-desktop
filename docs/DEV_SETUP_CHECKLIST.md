...
```json
{
  "sandbox": {
    ...
    "filesystem": {
      "allowWrite": [
        "~/.claude/projects/-home-kosei-Projects-inventory-system/memory"
      ]
    }
  }
}
...
- `-home-kosei-Projects-inventory-system` は cwd の絶対パスを `/` → `-` 変換した sanitize 文字列。別ホームディレクトリで作業する場合は適宜置換（例: `/home/alice/inventory-system` なら `-home-alice-inventory-system`）
- 検証: `touch ~/.claude/projects/-home-kosei-Projects-inventory-system/memory/.last_audit` で `Read-only file system` が出なくなれば反映済（現セッションで即時反映、restart 不要）
...
- [x] Claude Code の作業ディレクトリは WSL2 上の `/home/{user}/inventory-system` に固定（プロジェクト外保存禁止、CLAUDE.md 「やってはいけないこと」参照）
...
- [x] 10-1 UI-06b 在庫少一覧（REQ-302 個別）: D-047 により本行の独立画面実装タスクとしては解消。`/stock/low` は作らず UI-06a `status=low_stock` フィルタへの deep-link で対応する。サイドバー nav 変更（`navigation.ts` の `search`/`activeMatch` field）は Public PR #9（squash merge `70dc4d1`）で実装済み、詳細は [archive/plans/2026-07-16-sidebar-pending-links.md](archive/plans/2026-07-16-sidebar-pending-links.md) の scope
