...
- Superseded in part by: D-043 removes only D-033's exclusion of `pull_request.synchronize`. Final-only execution, exact-HEAD evidence, `push: main` absence, and concurrency cancellation remain unchanged.
...
- Contract: C1 商品 create、C2 商品 update/廃番 toggle、C3 商品一括 import、C4 入庫、C5 返品（`register_processed` 条件付き）、C6 手動販売、C7 廃棄、C8 売上 CSV commit、C9 売上 CSV rollback、C10 日報 import commit/rollback、C11 棚卸し確定、C12 整合性補正、C13 閾値保存（部分成功を含む）、C14 PLU 書出し confirm、C15 棚卸し開始、C16 棚卸し明細個数更新、C17 レジ登録状況 snapshot 取込み（`import_plu_register_snapshot` 成功、PR #85 / D-072）、C18 PLU prepare 成功（PR #85 L3 round 2 follow-up）、C19 PLU 対象一括更新（`bulk_set_plu_target` 成功、PR #86 / D-073）、C20 商品価格改定（`revise_product_price` 成功、UI-14 / REQ-105）、C21 取引先改名、C22 取引先統合（UI-15 / REQ-107）。C21 / C22 は consumer 全数導出により `productForm.root()` / `priceRevision.root()` / `suppliers.root()` / `productList.root()` / `lowStock(false)` / `stockInquiryRoot()` / `receivings.root()` / `inventoryRecords.root()` の同一の 8 key 集合に確定し、集合の正本は本 Contract 行と `src/lib/invalidation-contract.ts` とする。PLU slot summary は C2 / C14 / C17 / C18 / C19 で stale 化する。各集合の実体は production SSOT、導出原則と除外は UI_TECH_STACK §2.5 を正本とし、function-design に key 全量を複製しない。棚卸し詳細 prefix（`inventoryRecords.stocktakeDetailRoot()`）は C1 / C3 で直接 stale 化し、C11 は `inventoryRecords.root()` の prefix 包含により同じ詳細 consumer を stale 化する。C15 棚卸し開始は 65 slice 4d で `inventoryRecords.root()` を追加して hub 一覧を stale 化し、C16 棚卸し明細個数更新は列レベル導出で非追加を維持する。6 種化に伴い C8 売上 CSV commit / C15 棚卸し開始へ `inventoryRecords.root()` を追加する。
...
- 語義注記（2026-08-30、docs 整合性衛生 batch）: 本 Decision 内の「D-052-E1」は `docs/UI_TECH_STACK.md` §2.5 除外表の E1（operation_logs 系 query の invalidation 除外）を指す一意 ID として確定する。旧 `src/lib/query-keys.ts` に存在した literal 例外容認コメントが同じ「E1」表記を使っていた用法は、PR #29（2026-07-28）でコード側から当該コメントごと撤去済みの歴史的用法であり、現行の唯一の参照先は UI_TECH_STACK §2.5 である。
...
- Superseded in part by: D-082 が GPT-6 Astra 到来に伴い shared contract を世代非依存へ一般化し、§3.4 の Sol / Terra / Luna 3 slot を「OpenAI」1 slot に統合、承認・Human Gate・再検証・応答密度の 4 規範を追加する。profile 分離、slot-neutral 方針、人格を tracked に置かない方針は維持。
...
## D-082: Codex / OpenAI shared contract を世代非依存へ一般化し、承認・Human Gate・再検証・応答密度の規範を追加（D-057 の部分改訂）（2026-09-05）

- Decision: `docs/agent-guidance/gpt-5.6-shared.md` を `shared.md` へ改名し GPT-5.6 固有の表記を「Codex / OpenAI」へ一般化する。model / effort の既定 pin を repo 側 wrapper（`.codex/bin/codex-inventory`）から撤去し、個人用 `~/.codex/config.toml` の解決に委譲する。`AGENT_OPERATING_MANUAL` §3.4 の Sol / Terra / Luna 3 slot を「OpenAI」1 slot に統合し、世代差分は `docs/agent-guidance/model-notes.md` に分離する。shared contract に次の 4 規範を追加する: (1) 「fix して」型の行為要求は scope 内の実行指示とみなし、明示の承認はターンを跨いで引き継ぐ（解決済みの承認を繰り返し求めない）(2) 残りの承認を求める前に、当該 phase で承認済みの独立作業を完了しレビュー可能にする。Skill の一般的指示から追加の Human Gate を推測しない (3) 必須 repository check 通過後の再検証は、新しい変更・失敗・未解決の懸念がある時に限る (4) 簡潔な地の文を優先し、list は比較・順序に有用な時だけ使う。
- Status: accepted（owner 決定 2026-09-05「意図したものではあるよ、公式のドキュメントを元に書き換えてもらった形」。Codex（GPT-6 Astra）が OpenAI 公式 guidance を元に起草、owner 依頼）
- Why: GPT-6 Astra の到来で GPT-5.6 family 前提の shared contract と 3 slot 表が実態と乖離した。model 名を repo tracked file に持つと世代更新のたびに wrapper / profile / 役割表を書き換える必要があり、個人 config への集約で世代更新の影響を `model-notes.md` の差分メモに閉じ込められる。4 規範は Codex が承認待ちで過剰に停止する・解決済み承認を再要求する・意味の薄い再検証を積む、という実運用の摩擦（PR #34 で Codex が list 境界ごとに停止した実発生を含む）への対処で、公式 guidance の推奨と一致する。
...
## D-091: デザインの決まりを見る人の受け取り方から組み直す（2026-09-24）

- Status: accepted（owner 2026-09-24。デザインの決まりの見直しでの判断、ランキング 1 位の試しの了承、検索欄はボタンなしで確定、runtime lane A は 1 PR。owner 2026-09-25。詳細を開いた行を進行中にそろえる試しの了承）。
- Decision: design-system の規則を「見せ方 → 見る人の受け取り方 → 狙う効果 → 根拠」の順で組む。(1) 色は 6 役割（操作 / 進行中 / 注意・確認 / 完了 / 危険・失敗 / ふつう・補足）で使い、1 色 1 役割とする（[00 色の役割](design-system/00-foundations.md#色の役割)）。操作と進行中は利用者がいま自分でしていることとして同じ色の仲間にそろえる。(2) 強調は段 0〜4 で選び、上の段ほど 1 画面に置ける数を減らす。focus ring・操作枠・checked・進み具合の棒・待ちの spinner・役割色の文字だけの表示は段に数えない。(3) 役割表と段の表だけでは答えが 0 か 2 つ以上になる 23 場面の答えを 00 の「迷いやすい場面」に置く。(4) 書体は選定条件（見間違えにくさ・太字でつぶれない・長時間読める・等幅の数字・配布の方法）で runtime lane B の実機比較により採用を決め、それまで現行の system font stack を使う。旧理由（網からの読み込み遅延）は desktop 配布に当たらないため撤回する。(5) 補足情報とサマリカードの主値はラベルと値の組で何の値かを言い切る。(6) 03 の哲学と 04 の 16 の原則を 04 の 11 の原則へ統合し、03 は出典の一覧にする。DSR は番号と見出しを変えずに 01 で話題別に並べる。(7) 撤回・置換した旧文（info 家族を作らない、③強調の琥珀のうちランキング 1 位、primary の琥珀の語、新しい色相を足さない、現在行の primary バー、00 の Primary と Warning の用途、サイドバーの Primary アクセント、ウォーム系の論拠、書体の旧理由、review-checklist の badge の行、DSR-21 の Why、検索ボタン併記）の行き先を 04 の旧番号の対応表に置く。Q7 原則①（色数をむやみに増やさない）とは、色を役割の数を上限とし、進行中の色相 1 つだけを足す（有彩色の系統 3 → 4）ことで両立する。(8) 旧 04 が 00 へ反映すると約束したまま残っていた 3 点を 2026-09-24 の実装の多数派で閉じる: caption 14px muted、ページ余白 `space-6`、icon 12 / 16 / 20 / 24 / 32 の 5 段。旧 04 の token 表の badge の weight 600 は、現行の 500（`font-medium`）で置換する（600 は見出しと `AlertTitle` が使い、badge には使わない）。
- Why: amber 1 色が押す・注意・現在地・ランキング 1 位を兼ね、見る人が役割を取り違える。決まりを「どう見せるか」から決めると部品ごとに答えが割れるため、見る人がどう受け取り、何を狙うかから決める。今の赤・琥珀・緑の割り当て（owner 決定 2026-09-06）は壊さず、変えるものは runtime lane A の L3 で owner が実画面を見て決める試しにする。
- 候補値（runtime lane A の実測と L3 で確定するまで design-system の正本に書かない。2026-09-03 の Codex P2-2 裁定と同じ扱い）: 操作 塗り #1D5C63。進行中 地 #E6F0F0 / 線 #7FB0B4 / 枠 #2F7F86 / 文字 #123E43。注意・確認 amber-50 / amber-200 / amber-900 + icon。完了 green-50 / green-200 / green-900。危険・失敗 red 系。ふつう・補足 stone。
- 起草時のコントラスト（WCAG 2.x の相対輝度式、2026-09-24）: 操作の塗りと背景 #fafaf9 = 7.28:1、進行中の枠と背景 = 4.47:1、進行中の文字と進行中の地 = 10.07:1、進行中の線と背景 = 2.29:1、進行中の地と背景 = 1.11:1、操作の塗りと完了の緑（`--success`）= 1.52:1。参考に現行の `--primary` と背景 = 4.81:1。含意: 進行中の線は 3:1 未満のため、現在行のバーと作業中の囲みの枠に使えない。進行中の地は背景とほぼ同じで文字との併用が必須。操作と完了は色で区別できず、文字と icon が必須。
- ③強調の badge（最新・上書き件数）の移し先: 今は `variant="default"` で `--primary` の塗りのため、runtime lane A が `--primary` の値を変えても owner が試しを採るまで琥珀のままになるよう移す。塗りの上の文字 `--primary-foreground` との比は `--warning-emphasis` = 4.81:1、`--warning` = 3.05:1 で、後者は 4.5:1 に届かないため移し先は `--warning-emphasis` とする。
- 試し（runtime lane A の L3 で owner が決める）: ランキング 1 位を色でなく順位と太字で示す（owner 了承済み。採らなければ runtime lane A が 00・04 原則 4・DSR-08・review-checklist の badge の行を現行へ戻す）。Home の入口 card の地を操作の仲間の薄い地へ揃える、日報の取込み済みの badge を Alert と同じ危険・失敗へ揃える、Home の前日分の未取込みを注意・確認へ揃える、最新と上書き件数を stone の pill と太字へ移す（この 4 つは owner の現行の決定を規則に残し、採れば runtime lane A が 00 を直す）。詳細を開いた行を進行中へそろえる（owner 了承 2026-09-25。3 点目は開いた詳細と「詳細を閉じる」の類の文言が担い、badge を足さない。採らなければ runtime lane A が 00・02 ⑫・DSR-22 をふつう・補足の段 0 へ戻す）。取込みの手順の表示のいまのステップを進行中の段 2 へ移す（Coordinator の既定 2026-09-25。採らなければ runtime lane A が 00 の行を直し、押すボタンでない塗りを残すなら本 log に例外として記録する）。Home の入口 card の地と③強調の琥珀 pill は、試しを採らなければ runtime lane A が本 log に恒久の例外として記録する。
