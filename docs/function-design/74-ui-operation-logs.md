...
- 許可リストは `InventoryRecordsSearch.recordType`（`src/features/inventory-records/types.ts`）と一致させる。`csv_import` は `65-inventory-record-traceability.md` §65.3 の詳細 route が実装済みだが、csv_import 系 `record_type` を書き込む producer が0件のため除外を維持し、producer 側の `record_type` 採用（既存 follow-up）と併せて許可リストへ追加する。`stocktake` は同節の完成形 route 表には載っているが、対応する `$stocktakeId` 詳細 route が未実装のため、当該 route が実装されるまで許可リストから除外する。
...
| `csv_import` / `stocktake` 記録種別の関連リンク | `csv_import` の詳細 route は実装済みだが、csv_import 系 `record_type` を書き込む producer が0件のため除外を維持し、producer 側の `record_type` 採用（既存 follow-up）と併せて追加する。`stocktake` は対応する詳細 route（`/stocktake/records/$stocktakeId`）が未実装のため除外する | `65-inventory-record-traceability.md` §65.10 実装スライス |
