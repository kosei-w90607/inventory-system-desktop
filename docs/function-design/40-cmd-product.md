...
SPEC-STK-TIME-D8 / D9の回復情報を、message文字列の解析なしで伝える追加案。現行の§5.3のkind集合と構造体は実装済み契約であり、本節の追加はruntimeのgenerated enum/bindings切替と同時に行う。
...
- targetsの要素CountRecoveryTargetはproduct_code / product_name / stocktake_item_id（Option）/ stocktake_id（Option）/ actionを持つ。actionはactive_count / independent_recount / no_count_target。商品数量や時刻の証拠をclient入力へ戻さない。参照なしはnullとno_count_targetで表し、架空のIDを使わない。
