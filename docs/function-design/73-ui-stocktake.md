...
一覧は「カウント時在庫」L、「実数」N、「差」L-Nを保存事実として表示し、現在庫は別値と分かる表示にする。uncountedは「—」、auto_filledは「自動入力」、legacyは「更新前の記録」、flagは「要再確認」と文字/非色表示で区別する。要再確認は商品単位で理由（計数と前後不明の販売 / 相殺の行あり / 相殺の確認待ち〈EJ待ち〉 / 相殺の確認待ち〈登録の変化〉 / 旧記録の実測）を対象の型（[40](40-cmd-product.md)のrecount_reasons）から示し、相殺の確認待ちには[55](55-ui-csv-import.md)と同じ文言（EJ待ちの取込み機能の前後と前回のZ004が未受領なだけの区間、登録の変化と、取込み機能の前後の最初の区間）を使う。1商品に理由の違うflagが複数あるときは、相殺の確認待ち2種以外が一つでもあれば「取り込んだ後に数の再確認が必要です」を示し、確認待ちだけのときは55と同じ優先（最初の区間によるものを除く登録の変化、取込み機能の有無、EJが無い・不完全のEJ待ち、最初の区間、前回のZ004の未受領の順）で文言を選ぶ。確定済みの棚卸し・独立再実測に属する商品の要再確認は準備照会のrecount_after_importから受け取り、active明細があればその計数へ、なければ独立再実測へ案内する。過去の値を新しい計数の入力値へ自動で埋めない。
