...
| 記録 | 位置 | 行種 | 形 | 値の扱い |
|---|---|---|---|---|
| 通常・返品 | 明細域 | `Quantity` | 空白で始まり、空白除去後が `整数` + ` 点` + 空白 1 個以上 + `@` + 単価 token（通貨記号なし） | 数量（ASCII 数字の整数）と単価。直後の 1 明細だけに掛かる |
...
- 小数の数量は未観測のため受理しない（その行は `Unknown`）。
...
6. `合  計` の金額（無ければ `現金` の金額。どちらも全角数字の token を読む）= 明細の金額の合計。照合に使うラベルの行はちょうど 1 行で、金額 token が読めること。
