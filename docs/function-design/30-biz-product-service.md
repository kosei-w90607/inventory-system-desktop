...
- pos_stock_syncを変更する場合は共通repoで商品revisionも進める。非連動へ切り替えても現在庫の補正や既存flagの解消は行わない。
- 既存商品の連動再有効化は一意性と新方式の現物確認を要求する。参照できる棚卸し明細がない場合は偽の明細を作らず、ADRの既知の制限として非連動を維持する。checkboxを実測の代わりにしない。
