...
- pos_stock_syncを変更する場合は共通repoで商品revisionも進める。既存商品のtrue→falseはheld履歴の有無を問わず、同TXでpos_sync_disabled_revisionに変更後の版を保存する。受領メタだけではheld対象を再構成できないため、全切替を保守的に記録する。現在庫の補正や既存flagの解消は行わず、記録保存失敗は設定/版も含めTX全体を戻す。updateと既存商品を変更する商品一括importに共通で適用し、新規false/false→falseは記録しない。
