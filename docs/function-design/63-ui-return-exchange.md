...
- 保存ボタンは `返品・交換を保存`。保存成功後は result panel へ移り、フォーム本文は操作不可にする。保存結果の「詳細を見る」は UI-03-D22 に従って現在の返品・交換画面 URL を `returnTo` として送る。保存結果や保存系エラーの Alert はページ先頭側に出るため、保存成功または command 失敗時はページ先頭へスクロールする。result panel には備考を独立行で表示し、入力値がない場合も「備考なし」と明示する。
