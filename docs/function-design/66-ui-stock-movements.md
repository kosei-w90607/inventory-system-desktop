...
- UI-06c-D9（SPEC-NAV-RETURN-TYPES-2026-09-25）: `detailReturnTo` を受け側の route schema で parse すると元の search に戻る（JSON として読める string の `returnTo` を含む）。在庫照会（数字だけの商品コード）→ 在庫変動履歴 → 業務記録詳細 → 「前の画面へ戻る」→「在庫照会へ戻る」の実 router 往復で、各段の href が出発時と文字列一致する。
