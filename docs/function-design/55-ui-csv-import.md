...
| 状態 | 表示・操作 | 書込みと回復 |
|---|---|---|
| file選択前 | 本番準備の不足（共有JAN・旧実測・時計/EJ未検証・旧取込み等の識別メタ不足）を日本語の理由と対象商品/精算日で示す | 準備完了の表示だけでレジ側の運用開始を証明しない。準備完了は未提出の精算資料がないことを意味しないと明示する。資料受領・再確認の入口は閉ざさない |
...
過去日の後追いを一律に禁止する案内は置かない。メタ完備の別精算でも棚卸しとの前後が不明なら既存の保留へ進むため、日付の古さと保留理由を混同しない。runtimeの表示検証では該当日付・準備未完・解決不可の案内を文字/roleで検査し、Windowsの通常L3では既存準備表示の可読性を確認する。移行前行の故障注入は合成自動テストへ置き、operatorへDB操作を要求しない。
...
成功結果の売上0件は正常完了として表示し、要再確認対象は結果後も辿れる。現在庫が直っても準備表示のsettlement_missingを消さず、過去売上の復旧と混同しない。Windows L3は保留→一部保存→再起動→同file再選択→残り確認→取込み、保存した値が二重適用されないことを確認する。実装・native確認は未実施。
