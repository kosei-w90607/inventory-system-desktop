...
| 相殺の確認待ち（取込み結果・準備表示） | EJ待ちは、EJの取込み機能がない間は「この日の電子ジャーナルがないため、売上と返品の相殺を確かめられません。今の数を確認してください」、取込み機能が入った後は「電子ジャーナルを取り込むか、今の数を確認してください」。登録の変化（どの名称にも一致しない明細の行がある区間と、最初の区間を含む前回を証明できない区間）は、いつも「レジの商品の登録が変わったため、売上と返品の相殺を確かめられません。今の数を確認してください」。対象商品と精算日。1商品に理由の違うflagが複数あるときは、相殺の確認待ち2種以外が一つでもあれば上の「取り込んだ後に数の再確認が必要です」、確認待ちだけで登録の変化が一つでもあれば登録の変化の文言、全てがEJ待ちのときだけEJ待ちの文言にする | 理由は対象の型（[40](40-cmd-product.md)のrecount_reasons）で分岐し、文言は32の説明を使う。実行できない取込みを案内しない。進行中の確定を止める要再確認として扱い、数え直しで解消する。EJ待ちは、EJか前回のZ004が後から取り込まれた場合にBIZの再評価で解消するか、相殺の行ありか登録の変化へ変わる。登録の変化はEJでは解消せず、数え直しでだけ解消する |
