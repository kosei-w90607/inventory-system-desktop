...
| 状態 | 表示・操作 | 書込みと回復 |
|---|---|---|
| file選択前 | 本番準備の不足（共有JAN・旧実測・時計/EJ未検証・旧取込み等の識別メタ不足）を日本語の理由と対象商品/精算日で示す | 準備完了の表示だけでレジ側の運用開始を証明しない。資料受領・再確認の入口は閉ざさない |
...
| sync_disabled_unreconciled | 「在庫数は未調整です」、商品と「今の数を確認する」 | 非連動化後も準備表示に残す。active計数→確定/独立再実測へ案内。明細なしは制限を表示。売上のみcommitした結果でも同じ案内を残す |
| settlement_missing（準備表示） | 「取り込めていない精算資料があります」と精算日・拒否理由を表示 | 同一性拒否の未取込みsourceが対象。元資料/記録を確認し、再実測後/再起動後も表示を維持。欠番全般の検出や不足金額の断定をしない |
