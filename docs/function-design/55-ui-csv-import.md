...
| 状態 | 表示・操作 | 書込みと回復 |
|---|---|---|
| file選択前 | 本番準備の不足（共有JAN・旧実測・EJ未検証・旧取込み等の識別メタ不足）を日本語の理由と対象商品/精算日で示す | 準備完了の表示だけでレジ側の運用開始を証明しない。準備完了は未提出の精算資料がないことを意味しないと明示する。資料受領・再確認の入口は閉ざさない |
| preview ready | 精算同一性guard（識別メタ不足の拒否を含む）を通過した集計・同日追加確認 | 同日追加の場合はguardを通った別精算だけ既存確認後にcommit。UIが同一性やBefore/Afterを再計算しない |
| recount_after_import | 「取り込んだ後に数の再確認が必要です」、対象商品と理由（計数と前後不明の販売 / 相殺の行あり / 相殺の確認待ち〈EJ待ち〉 / 相殺の確認待ち〈登録の変化〉 / 旧記録の実測）の一覧 | 最新の実測の所属によらず取込みは完了し、商品ごとの要再確認が残る。確定を急がせず、進行中の棚卸しの明細があればその計数へ、なければ独立再実測へ案内。準備表示のrecount_after_importにも同じ商品が残る |
| 相殺の確認待ち（取込み結果・準備表示） | EJ待ちは、EJの取込み機能がない間は「この日の電子ジャーナルがないため、売上と返品の相殺を確かめられません。今の数を確認してください」、取込み機能が入った後は「電子ジャーナルを取り込むか、今の数を確認してください」、そのうち前回のZ004が未受領なだけの区間は「前回の精算の商品別CSV（電子ジャーナルがまだならそれも）を取り込むか、今の数を確認してください」（EJの取込み機能がない間はこの区別は現れない）。登録の変化（どの名称にも一致しない明細の行がある区間と、前回を証明できない区間）は「レジの商品の登録が変わったため、売上と返品の相殺を確かめられません。今の数を確認してください」。ただし直前のZ004が未受領の間のもの（最初の区間・受領済みの最小より小さい番号の区間・settlement_noが戻った区間）は、EJの取込み機能が入った後は「直前の精算の商品別CSV（電子ジャーナルがまだならそれも）を取り込むか、今の数を確認してください」、取込み機能がない間はほかの登録の変化と同じ文言。対象商品と精算日。1商品に理由の違うflagが複数あるときは、相殺の確認待ち2種以外が一つでもあれば上の「取り込んだ後に数の再確認が必要です」にする。確認待ちだけのときは次の最初に当たる文言にする。(1) 直前のZ004が未受領の間のものを除く登録の変化が一つでもあれば登録の変化の文言。(2) EJの取込み機能がない間は、直前のZ004が未受領の間の登録の変化が一つでもあれば同じ登録の変化の文言、なければ取込み機能がない間のEJ待ちの文言。(3) 取込み機能が入った後は、EJが無い・不完全のEJ待ちが一つでもあれば「電子ジャーナルを取り込むか、今の数を確認してください」、なければ直前のZ004が未受領の間の登録の変化が一つでもあれば直前の精算の商品別CSVの文言、全てが前回のZ004の未受領のときだけ前回の商品別CSVの文言 | 理由は対象の型（[40](40-cmd-product.md)のrecount_reasons）で分岐し、文言は32の説明を使う。実行できない取込みを案内しない。進行中の確定を止める要再確認として扱い、数え直しで解消する。EJ待ちは、EJか前回のZ004が後から取り込まれた場合にBIZの再評価で解消するか、相殺の行ありか登録の変化へ変わる。登録の変化はEJでは解消せず、数え直しで解消する。直前のZ004が未受領の間のものだけは、直前の精算のZ004が取り込まれた場合にBIZの再評価で解消するか、相殺の行ありかEJ待ちかほかの登録の変化へ変わる |
| held（共有JAN行） | 「販売の前後を確認できません。今の数を確認してください」、対象・理由・操作 | JANの候補商品が複数、かつ少なくとも一つが在庫連動対象の共有JAN行（非連動商品との共有を含む）で、全ての在庫連動候補の実測前を証明できない場合だけ。業務commitを無効化し、サーバーでも拒否。全候補を数えて同じfileを選び直す。active明細を優先、なければ独立再実測。明細なしは解除不可と説明 |
| source_identity_conflict | BIZの理由に応じ「同じ精算と思われる別の資料があります」または「精算を識別する情報が足りないため、同じ日の追加取込みはできません」 | 追加確認で解除せず、下記の元資料・取込み記録の確認へ案内。通常heldの計数ボタンへ案内しない |
| sync_disabled_unreconciled | 「在庫数は未調整です」、商品と「今の数を確認する」 | 非連動化後も準備表示に残す。32の当該商品issueの説明を使い、切替後のpendingは確定へ、それ以前の入力は切替後の数え直しへ案内する。active以外は独立再実測、明細なしは棚卸し開始を案内する。売上のみcommitした結果でも同じ案内を残す |
| settlement_missing（準備表示） | 「取り込めていない精算資料があります」とsource単位のissueごとに精算日・BIZの拒否理由説明を表示 | 同一性拒否の未取込みsourceが対象。各issueのsource_ids/settlement_datesは各1要素で、別sourceの日付と理由を混ぜず、説明文を解析しない。元資料/記録を確認し、再実測後/再起動後も表示を維持。欠番全般の検出や不足金額の断定をしない |
...
| 旧記録がある商品の取消の保留 | 「取り消す前に、対象商品の今の数を確認してください」 | 進行中の棚卸しの明細があればその計数と確定、なければ独立再実測の後に、同じIDの取消を再試行する。独立再実測でactiveを迂回しない |

準備状態はget_pos_stock_readinessのread-only結果を使い、取込みの成功/同一性拒否/取消、商品設定変更、計数保存/確定後と画面再訪時にrefetchする。取得失敗は前のreadyを成功表示せず再試行を案内する。fileの個別commit可否はstock_reviewを正とする。count_tokenは[73の共通計数フロー](73-ui-stocktake.md)で扱う。保存中・importing中の連打と離脱を防ぎ、商品切替で未保存contextを破棄する。errorは[40のkind/code/action](40-cmd-product.md)で分岐し、message内のIDや数値を解析しない。

preflightのimport_identity_missingは「精算の識別情報がない取込みがあります。次の日付には追加取込みできません」とsettlement_datesの全日付を一覧表示し、準備未完を日本語と警告iconで示す。日付はtyped fieldをそのまま使い、messageから抜き出さず、商品一覧/計数へのリンクに置換しない。既存の記録ハブで対象日を確認できるよう案内する。初導入は識別メタの抽出・保存を実装した新形式で開始し、旧試験履歴を持ち込まないことを担当者向け導入手順へ記す。試験DBの作り直しが必要でも、この画面から履歴削除・確認による解除は行わず、保全するマスタと売上/在庫移動の整合を含む別作業へ渡す。初導入の本番には旧実測/旧取込みの対象がないが、存在した場合の検出・拒否・表示は省略しない。

過去日の後追いを一律に禁止する案内は置かない。メタ完備の別精算でも棚卸しとの前後が不明なら要再確認が付くため、日付の古さと要再確認の理由を混同しない。runtimeの表示検証では該当日付・準備未完・解決不可の案内を文字/roleで検査し、Windowsの通常L3では既存準備表示の可読性を確認する。移行前行の故障注入は合成自動テストへ置き、operatorへDB操作を要求しない。

source_identity_conflictではBIZの利用者向けmessageで理由と以下の制限を表示し、同じcodeの共通案内を使う。message解析で回復操作を分岐しない。既存の記録ハブ `/inventory/records` で同日の取込みを確認し、取り込もうとした元資料と既存取込みの元資料のマシンNo.・精算回数を確認する。メタを持つ正しい元資料を選び直す場合も再previewが必要で、取込み対象側だけ揃えても既存側の欠落は解消しない。元資料の識別情報を手入力・推測で補う操作は置かない。

別hashの採用決定と既存sourceのメタ補完は本scopeにない。誤った版を取消しても訂正版との衝突は残り、番号resetの別系列を証明できない場合も取込み不可となる。その間は当該精算の売上が未取込みで、その取込みによる在庫減算も行われない。メタ不足の同日追加も拒否条件が残る間は同じ制限となる。再実測で現在庫を合わせても売上欠落は復旧しないと表示する。資料の確認・選び直しで解消できない場合は「この画面では解決できません。元の資料を保管し、担当者に確認してください」と明示する。待機・再起動・計数で解決すると案内せず、拒否を迂回するために正しい既存取込みを取り消す手順も案内しない。
...
成功結果の売上0件は正常完了として表示し、要再確認対象は結果後も辿れる。現在庫が直っても準備表示のsettlement_missingを消さず、過去売上の復旧と混同しない。Windows L3は取込み後の要再確認→再起動→数え直しで解消を確認する。共有JAN行の保留→一部保存→再起動→同file再選択→残り確認→取込みと、保存した値が二重適用されないことも確認する。実装・native確認は未実施。
