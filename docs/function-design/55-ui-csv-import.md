...
| 相殺の確認待ち（取込み結果・準備表示） | EJの取込み機能がない間は「この日の電子ジャーナルがないため、売上と返品の相殺を確かめられません。今の数を確認してください」、取込み機能が入った後は「電子ジャーナルを取り込むか、今の数を確認してください」。対象商品と精算日 | 実行できない取込みを案内しない。進行中の確定を止める要再確認として扱い、数え直しで解消する。完全なEJが後から取り込まれた場合はBIZの再評価で解消するか、相殺の行ありへ変わる |
