...
| recount_after_import | 「取り込んだ後に数の再確認が必要です」、対象商品と理由（計数と前後不明の販売 / 相殺の行あり / 相殺の確認待ち / 旧記録の実測）の一覧 | 最新の実測の所属によらず取込みは完了し、商品ごとの要再確認が残る。確定を急がせず、進行中の棚卸しの明細があればその計数へ、なければ独立再実測へ案内。準備表示のrecount_after_importにも同じ商品が残る |
| 相殺の確認待ち（取込み結果・準備表示） | EJの取込み機能がない間は「この日の電子ジャーナルがないため、売上と返品の相殺を確かめられません。今の数を確認してください」、取込み機能が入った後は「電子ジャーナルを取り込むか、今の数を確認してください」。前回のZ004からスロットと名称の対応が変わった区間（最初の区間を含む）は、取込み機能が入った後も「レジの商品の登録が変わったため、売上と返品の相殺を確かめられません。今の数を確認してください」。対象商品と精算日。1商品に理由の違うflagが複数あるときは、相殺の確認待ち以外が一つでもあれば上の「取り込んだ後に数の再確認が必要です」を示し、この文言は全てのflagが相殺の確認待ちのときだけにする | 理由は対象の型（[40](40-cmd-product.md)のrecount_reasons）で分岐し、文言は32の説明を使う。実行できない取込みを案内しない。進行中の確定を止める要再確認として扱い、数え直しで解消する。完全なEJが後から取り込まれた場合はBIZの再評価で解消するか、相殺の行ありへ変わる。名称の対応が変わった区間はEJでは解消せず、数え直しでだけ解消する |
