...
| 相殺の確認待ち（取込み結果・準備表示） | EJ待ちは、EJの取込み機能がない間は「この日の電子ジャーナルがないため、売上と返品の相殺を確かめられません。今の数を確認してください」、取込み機能が入った後は「電子ジャーナルを取り込むか、今の数を確認してください」、そのうち前回のZ004が未受領なだけの区間は「前回の精算の商品別CSV（電子ジャーナルがまだならそれも）を取り込むか、今の数を確認してください」（EJの取込み機能がない間はこの区別は現れない）。登録の変化（どの名称にも一致しない明細の行がある区間と、前回を証明できない区間）は「レジの商品の登録が変わったため、売上と返品の相殺を確かめられません。今の数を確認してください」。ただし最初の区間によるものは、EJの取込み機能が入った後は「直前の精算の商品別CSV（電子ジャーナルがまだならそれも）を取り込むか、今の数を確認してください」、取込み機能がない間はほかの登録の変化と同じ文言。対象商品と精算日。1商品に理由の違うflagが複数あるときは、相殺の確認待ち2種以外が一つでもあれば上の「取り込んだ後に数の再確認が必要です」にする。確認待ちだけのときは次の最初に当たる文言にする。(1) 最初の区間によるものを除く登録の変化が一つでもあれば登録の変化の文言。(2) EJの取込み機能がない間は、最初の区間の登録の変化が一つでもあれば同じ登録の変化の文言、なければ取込み機能がない間のEJ待ちの文言。(3) 取込み機能が入った後は、EJが無い・不完全のEJ待ちが一つでもあれば「電子ジャーナルを取り込むか、今の数を確認してください」、なければ最初の区間の登録の変化が一つでもあれば直前の精算の商品別CSVの文言、全てが前回のZ004の未受領のときだけ前回の商品別CSVの文言 | 理由は対象の型（[40](40-cmd-product.md)のrecount_reasons）で分岐し、文言は32の説明を使う。実行できない取込みを案内しない。進行中の確定を止める要再確認として扱い、数え直しで解消する。EJ待ちは、EJか前回のZ004が後から取り込まれた場合にBIZの再評価で解消するか、相殺の行ありか登録の変化へ変わる。登録の変化はEJでは解消せず、数え直しで解消する。最初の区間によるものだけは、直前の精算のZ004が取り込まれた場合にBIZの再評価で解消するか、相殺の行ありかEJ待ちかほかの登録の変化へ変わる |
| held（共有JAN行） | 「販売の前後を確認できません。今の数を確認してください」、対象・理由・操作 | 在庫連動の候補が複数ある共有JAN行だけ。業務commitを無効化し、サーバーでも拒否。全候補を数えて同じfileを選び直す。active明細を優先、なければ独立再実測。明細なしは解除不可と説明 |
