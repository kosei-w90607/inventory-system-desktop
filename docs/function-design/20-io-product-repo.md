...
| flag保存/変更/解消（tx, product_code, source_id, import_id, reason） | 未解消の(商品, 資料)を一意に保存し、同じ組の重複追加で行を増やさない。理由の変更（相殺の確認待ち→相殺の行あり）と解消（行の削除）はBIZの指示だけで行い、解消対象はBIZが受領証拠・取消元・EJの再評価で選ぶ。商品単位の未解消flagの読取りは確定・準備照会・一覧に使う。商品非連動への変更で一括削除しない |
