...
|---|---|---|
| 精算系列 | 同機の連続精算・0売上・同日複数・日跨ぎ・番号reset、EJの精算区間とZ004の対応、Z004のsettlement_noがZ004ごとに1ずつ増えるか（他のZ帳票と共有のカウンタでないか） | 同じ精算と対応づけられた区間だけ相殺の判定に使う。reset後の系列は証明できないので、同番の別hashは衝突として拒否。受領済みの最大より小さいsettlement_noのZ004は相殺の確認待ち（登録の変化） |
| EJ完全性 | 精算の開始/終了、取引一連番号、分割fileの必要な続き、先頭/末尾/途中欠落、Z004との同一精算（machine_no・settlement_no）、商品別の符号付き合計の一致（候補でつながる名称と商品の単位〈連結成分〉ごと、Z004側はslot単位。部門名とも一致する名称を含む単位と、clear行の場合の今回のZ004に行のないcodeを含む単位は照合しない）、前回のZ004（同じmachine_noの直前のsettlement_noで区間の開始と連続を証明できるもの）と今回のZ004のスキャニングコードと名称の組の集合差による、対応が変わった名称の算出（価格だけ・廃番・名称変更・新しい名称の追加との区別、前回のZ004が未受領なだけの区間・最初の区間・settlement_noの戻り・区間の開始の証拠がない不完全・前回を証明できない区間の区別）、締めの時点で同じ日のEJをSDから取れるか、未精算の売上があるPLUをPLU書出しのclear行で消せるか | 対応が完全な区間だけ相殺の判定に使う。存在する末尾fileだけで完結と推測しない。不完全（開始の証拠がない場合を含む）と前回のZ004の未受領は相殺の確認待ち（EJ待ち）、前回を証明できない区間（最初の区間・settlement_noの戻りを含む）は相殺の確認待ち（登録の変化） |
