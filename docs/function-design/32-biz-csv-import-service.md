...
6. 売上と返品の相殺の判定: 数量・金額とも0の、Unknownの商品（数えた商品）の行は、同じ精算区間の完全なEJ（ADR D5の完全性の全条件）に当該商品の行（販売・戻・訂正・取消）が一つもなければflagを付けず、一つでもあれば相殺の行ありとする。EJがない・不完全・名称を一意に特定できない場合は、その区間の数量・金額とも0の数えた商品を全て相殺の確認待ち（EJ待ち）にする。前回のZ004から対応が変わった名称（ADR D5。スキャニングコードと名称の組の集合差 `{名称 | (コード, 名称) ∈ 前回 − 今回}`）の明細の行が区間のEJにある場合と、前回を証明できない区間（前回のZ004がない最初の区間を含む）は、相殺の確認待ち（登録の変化）にする。変わった名称の行が区間になければ、価格だけ・廃番・名称変更・新しい名称の追加の書出しの後でも不完全にしない。相殺の確認待ちは2種とも進行中の確定を止め、準備表示に出る。区間の中の前後は問わず、時刻を使わない。

EJの証拠は、EJ parser laneがEJの取込み・解析と行の分類（商品の行・部門売り・明細でない行・帰属できない行）を行い、精算区間ごとにZ004との同一精算か・完全性（ADR D5。不完全が対応の変化によるものかを区別する）・商品別の行の有無をBIZへinterfaceの形で渡す。BIZはこのinterfaceで受けた証拠だけを使い、EJの行を自分で解釈しない。区間のEJが後から取り込まれたら、BIZは同じ資料の相殺の確認待ち（EJ待ち）のflagを同じ業務TXで再評価し、完全で行がなければ解消、あれば相殺の行ありへ変え、変わった名称の行があるか前回を証明できなければ相殺の確認待ち（登録の変化）へ変える。それ以外の理由で完全でなければEJ待ちのまま残す。相殺の確認待ち（登録の変化）は再評価の対象にせず、数え直しでだけ解消する。2種とも数え直しで解消する。
...
| EJ完全性 | 精算の開始/終了、取引一連番号、分割fileの必要な続き、先頭/末尾/途中欠落、Z004との同一精算（machine_no・settlement_no）、商品別の符号付き合計の一致、前回のZ004（同じmachine_noの直前のsettlement_noで区間の開始と連続を証明できるもの）と今回のZ004のスキャニングコードと名称の組の集合差による、対応が変わった名称の行の有無（価格だけ・廃番・名称変更・新しい名称の追加との区別、前回を証明できない区間）、締めの時点で同じ日のEJをSDから取れるか、未精算の売上があるPLUをPLU書出しのclear行で消せるか | 対応が完全な区間だけ相殺の判定に使う。存在する末尾fileだけで完結と推測しない。不完全なら相殺の確認待ち |
...
| 商品同定 | 出力時の名称（Z004で当該商品のスキャニングコードの行が持つキャラクター欄）、改名、共有/衝突、部門名との一致（商品の名称と部門名を合わせて数え、部門名と同じ名称の商品の行は帰属できない行）、16バイトのレジ名称の一意性、JAN候補、行の分類（商品の行・部門売り・帰属できない行）と値引きの行がどの分類に入るか | 一意に帰属できた商品の行と、部門名に一意に一致しどの商品の名称とも一致しない部門売りの行だけを既知とする。不明は帰属できない行として区間を不完全にする。現在のマスタ名だけで過去を推定しない |
