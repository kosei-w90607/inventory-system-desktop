...
recount_after_importは、未解消の要再確認flagを持つ商品ごとに1件返す。targetsは回復先（active_count / independent_recount / no_count_target）の当該1要素、source_idsとsettlement_datesはその商品の未解消flagの資料IDと精算日（各昇順・重複なし）とする。targetsの要素はその商品の未解消flagの理由を型で持つ（[40](40-cmd-product.md)のrecount_reasons）。利用者向け説明はBIZがflagの理由から作る。計数と前後不明の販売・相殺の行あり・旧記録の実測は「取り込んだ後に数の再確認が必要です」。相殺の確認待ち（EJ待ち、offset_check_pending）は、EJの取込み機能がない間は「この日の電子ジャーナルがないため、売上と返品の相殺を確かめられません。今の数を確認してください」、取込み機能が入った後は「電子ジャーナルを取り込むか、今の数を確認してください」、そのうち前回のZ004が未受領なだけの区間は「前回の精算の商品別CSV（電子ジャーナルがまだならそれも）を取り込むか、今の数を確認してください」とする（前回のZ004が未受領かはEJの証拠で分かるので、EJの取込み機能がない間はこの区別は現れず、EJ待ちは全て取込み機能がない間の文言になる）。相殺の確認待ち（登録の変化、offset_mapping_changed。どの名称にも一致しない明細の行がある区間と、最初の区間を含む前回を証明できない区間）は、EJを取り込み直しても解消しないので、いつも「レジの商品の登録が変わったため、売上と返品の相殺を確かめられません。今の数を確認してください」とする。1商品に理由の違うflagが複数あるときは、相殺の確認待ち2種以外が一つでもあれば「取り込んだ後に数の再確認が必要です」、確認待ちだけで登録の変化の側が一つでもあれば登録の変化の説明、全てがEJ待ちの側のときだけEJ待ちの説明とする。EJ待ちの中では、EJが無い・不完全の側が一つでもあれば「電子ジャーナルを取り込むか、今の数を確認してください」、全てが前回のZ004の未受領のときだけ前回の商品別CSVの文言とする。取込みを止めるissueではなく、進行中の棚卸しに属する商品では確定を止める。
...
6. 売上と返品の相殺の判定: 数量・金額とも0の、Unknownの商品（数えた商品）の行は、同じ精算区間の完全なEJ（ADR D5の完全性の全条件）に当該商品の行（販売・戻・訂正・取消）が一つもなければflagを付けず、一つでもあれば相殺の行ありとする。明細の行が複数の商品の名称に一致する（16バイトの切り詰めによる同名を含む）、商品の名称と部門名の両方に一致する、または前回のZ004から対応が変わった名称（ADR D5。スキャニングコードと名称の組の集合差 `{名称 | (コード, 名称) ∈ 前回 − 今回}`）に一致する場合は、区間を不完全にせず、その名称を持つ候補の商品（今回のZ004の組の商品。対応が変わった名称では前回の組と今回の組の商品の和）のうち、その区間で数量・金額とも0の数えた商品を全て相殺の行ありにする。候補でない数えた商品には付けない。したがって価格だけ・廃番・名称変更・新しい名称の追加の書出しの後でも区間を確認待ちにしない。区間全体を確認待ちにするのは次に限る。EJがない・不完全（精算の同一性、開始と終了・取引一連番号・分割fileの連続、商品別の符号付き合計の不一致）な場合と、前回のZ004がまだ取り込まれていないだけの場合は、その区間の数量・金額とも0の数えた商品を全て相殺の確認待ち（EJ待ち）にする。どの商品の名称にも部門名にも前回のZ004の名称にも一致しない明細の行が区間のEJにある場合と、前回を証明できない区間は、相殺の確認待ち（登録の変化）にする。前回のZ004が存在しない最初の区間は、取込みの時点で相殺の確認待ち（登録の変化）にし、EJの取込みを案内しない。部門名はZ004どうしの比較に含めない（区間の途中で部門名が変わった場合の誤った相殺の行ありは見逃しにならないので受け入れる。ADR D5）。相殺の確認待ちは2種とも進行中の確定を止め、準備表示に出る。区間の中の前後は問わず、時刻を使わない。

EJの証拠は、EJ parser laneがEJの取込み・解析と行の分類（商品の行・部門売り・明細でない行・帰属できない行）を行い、精算区間ごとにZ004との同一精算か・完全性（ADR D5。帰属できない行と前回を証明できないことによる確認待ちをほかの不完全と区別し、前回のZ004が未受領なだけの区間を示す）・商品別の行の有無（名称が重なる行は候補の商品全てを行ありとする）をBIZへinterfaceの形で渡す。BIZはこのinterfaceで受けた証拠だけを使い、EJの行を自分で解釈しない。区間のEJか前回のZ004が後から取り込まれたら、BIZは同じ資料の相殺の確認待ち（EJ待ち）のflagを同じ業務TXで再評価し、完全で当該商品を候補に含む行がなければ解消、あれば相殺の行ありへ変え、どれにも一致しない行があるか前回を証明できなければ相殺の確認待ち（登録の変化）へ変える。それ以外の理由で完全でなければEJ待ちのまま残す。相殺の確認待ち（登録の変化）は再評価の対象にせず、数え直しでだけ解消する。2種とも数え直しで解消する。
...
| EJ完全性 | 精算の開始/終了、取引一連番号、分割fileの必要な続き、先頭/末尾/途中欠落、Z004との同一精算（machine_no・settlement_no）、商品別の符号付き合計の一致（名称が重なる候補の商品はその名称の合計で照合）、前回のZ004（同じmachine_noの直前のsettlement_noで区間の開始と連続を証明できるもの）と今回のZ004のスキャニングコードと名称の組の集合差による、対応が変わった名称の算出（価格だけ・廃番・名称変更・新しい名称の追加との区別、前回のZ004が未受領なだけの区間と前回を証明できない区間の区別）、締めの時点で同じ日のEJをSDから取れるか、未精算の売上があるPLUをPLU書出しのclear行で消せるか | 対応が完全な区間だけ相殺の判定に使う。存在する末尾fileだけで完結と推測しない。不完全と前回のZ004の未受領は相殺の確認待ち（EJ待ち）、前回を証明できない区間は相殺の確認待ち（登録の変化） |
| EJ復元 | 通常/返品（戻の行に商品名が載るか）、数量×単価、名称反復、訂正/取消の行の形と店が使うか、値引き（％－キー）の行の形、未知行、取引内点数・商品別符号付き合計、行の分類（商品の行・部門売り・合計や支払などの明細でない行・帰属できない行） | Z004純合計一致は必要条件のみ。未知形式・相殺する欠落があれば区間を不完全とし相殺の確認待ち（EJ待ち）、どれにも一致しない帰属できない行があれば相殺の確認待ち（登録の変化） |
| 商品同定 | 出力時の名称（Z004で当該商品のスキャニングコードの行が持つキャラクター欄）、改名、共有/衝突、部門名との一致（商品の名称と部門名を合わせて数え、部門名と同じ名称の商品の行は候補の商品の行）、16バイトのレジ名称の重なりの程度（重なると候補の商品が余分に相殺の行ありになる）、JAN候補、行の分類（商品の行・部門売り・帰属できない行）と値引きの行がどの分類に入るか | 商品の行（名称が重なる行と対応が変わった名称の行は候補の商品全ての行）と、部門名だけに一致する部門売りの行を既知とする。どれにも一致しない行は帰属できない行として相殺の確認待ち（登録の変化）にする。現在のマスタ名だけで過去を推定しない |
