...
| EJ完全性 | 精算の開始/終了、取引一連番号、分割fileの必要な続き、先頭/末尾/途中欠落、Z004との同一精算（machine_no・settlement_no）、商品別の符号付き合計の一致（候補でつながる名称と商品の単位〈連結成分〉ごと、Z004側はslot単位。部門名とも一致する名称を含む単位と、clear行の場合の今回のZ004に行のないcodeを含む単位は照合しない）、前回のZ004（同じmachine_noの直前のsettlement_noで区間の開始と連続を証明できるもの）と今回のZ004のスキャニングコードと名称の組の集合差による、対応が変わった名称の算出（価格だけ・廃番・名称変更・新しい名称の追加との区別、前回のZ004が未受領なだけの区間・最初の区間・欠番の後追い・受領済みの最小より小さいsettlement_no・settlement_noの戻り・区間の開始の証拠がない不完全・前回を証明できない区間の区別）、締めの時点で同じ日のEJをSDから取れるか、未精算の売上があるPLUをPLU書出しのclear行で消せるか | 対応が完全な区間だけ相殺の判定に使う。存在する末尾fileだけで完結と推測しない。不完全（開始の証拠がない場合を含む）と前回のZ004の未受領は相殺の確認待ち（EJ待ち）、前回を証明できない区間（最初の区間・受領済みの最小より小さいsettlement_no・settlement_noの戻りを含む）は相殺の確認待ち（登録の変化）。直前のZ004が未受領の間の登録の変化（最初の区間・受領済みの最小より小さいsettlement_no・settlement_noの戻り）は直前のZ004の受領で再評価する |
| EJ復元 | 通常/返品（戻の行に商品名が載るか）、数量×単価、名称反復、訂正/取消の行の形と店が使うか、値引き（％－キー）の行の形、未知行、取引内点数・商品別符号付き合計、行の分類（商品の行・部門売り・合計や支払などの明細でない行・帰属できない行） | Z004純合計一致は必要条件のみ。未知形式・相殺する欠落があれば区間を不完全とし相殺の確認待ち（EJ待ち）、どれにも一致しない帰属できない行があれば（前回のZ004を受領しているときだけ判定）相殺の確認待ち（登録の変化） |
