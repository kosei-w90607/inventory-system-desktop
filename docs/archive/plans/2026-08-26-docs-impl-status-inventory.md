...
- AC-8: Writer の content commit の `git diff --stat` が Scope 列挙の docs 10 点（Scope 1〜7 の distinct 8 doc — Scope 4 は 65/74 の 2 file — + Scope 8 で新規に加わる 69-ui / 53-ui の 2 doc）のみ（src / src-tauri / 設定 file / `docs/Plans.md` の変更 0。gated amendment 2 で「9 点」の算術誤りを 10 点へ訂正、gated amendment 3 で `docs/Plans.md` を Writer diff から除外 — Plans.md の live 同期は plan-first commit 済み + Post-Merge Closeout で行う。継続発注書には除外を明記済みで、packet 側の追随漏れを是正）。
