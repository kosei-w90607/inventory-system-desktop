...
- **一定文字数超過時は `truncate` + `title` 属性で省略表示する**（`OperationLogsPage.tsx:522` の既存パターンを再利用）。全文確認手段（DSR-12）は note の種類で分かれる: 記録本体の note フィールド（`ManualSaleRecordDetailPage.tsx:149` 等、truncate なし）は各記録詳細ページのヘッダー部が直接表示するが、`MovementTable.tsx` の行 note（`movement.note`）は `source` が無い行に元記録リンクが無く（UI-06c-D6）、`StocktakeRecordDetailPage.tsx:234` 等の記録詳細ページ自身も同じ `MovementTable` を再利用し truncate されたままのため、詳細ページ任せの代替経路が無い。したがって movement note の全文確認手段は `MovementTable` 自身が担い（`title` に加え折り返し・展開等の手段を持つ）、`title` は一覧上での補足（DSR-11「Tooltip は補足に限る」）と位置づける。`MovementTable.tsx:94` は現在 truncate のみで `title` を欠くため、これも本規則の是正対象に含める。現行 `ReturnExchangePage.tsx` の `whitespace-normal` 折り返しは行の高さが note の長さに応じて伸びる問題があり、truncate 化で行高を揃える。
...
- **S1 catalog ① 説明セクション**: `02-component-catalog.md` ① ページヘッダ（`:24-61`）に「説明セクション」使用パターン（`PageHeader` 内 `space-y-1` グループ、`actions` と併存可能）を追加。`PageHeader` の `actions`/`subtitle` 併存不可という component gap を Don't 節または備考として明記。
...
|---|---|---|---|---|---|
| L8-7 | `02-component-catalog.md` ①（新設パターン） | UIB3-D1 | `PageHeader` に新規 prop を足す代替は runtime 変更を伴い docs-only の本 packet の範囲を超える。`PageHeader.tsx:29-36` の `actions` 分岐を直す root-cause fix（L8-8/UIB3-D5 と一本化）で `space-y-1` グループに描画し、既存 sibling `<p>` を移行する。component 拡張の実装自体は Non-scope の runtime lane へ（**Codex round 2 是正**） | `02-component-catalog.md`、`docs/function-design/60,67,68` | AC1/AC2 rg |
...
| L8-8 | `02-component-catalog.md` ①（S1 と共有） | UIB3-D5 | `PageHeader.tsx` の `actions`/`subtitle` 排他という component 契約が根本原因。使用パターン（wrapper div）は component 拡張が入るまでの代替に留め、`PageHeader.tsx:29-36` の root-cause fix を推奨案に一本化する（**Codex round 2 是正**、両論併記ではない）。実装（component 拡張そのもの）は Non-scope の runtime lane へ | `02-component-catalog.md` | AC1 rg |
