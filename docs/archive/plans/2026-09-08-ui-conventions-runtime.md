...
⑦（design-first 候補提示、PR #39 squash `6ff2247`、[archived packet](2026-09-05-ui-conventions-batch-design.md) / [Matrix](test-matrices/2026-09-05-ui-conventions-batch-design.md)）は design-system canonical docs（`00-foundations.md` / `01-decision-rules.md` / `02-component-catalog.md`）のみを改訂した docs-only packet で、Non-scope に「`src/**` の実装変更全て」を明記し runtime 反映を後続 lane（本 lane）へ申し送った。本 lane はその runtime 反映（状態 Badge の `tone` prop 化・CTA `secondary` 中間段・SearchBar live 型可視 Label・Alert `warning` variant・DSR-08 増減数値の色）と、あわせて発見された `01-decision-rules.md` の DSR-23 重複ブロック・`04-backbone.md`/catalog の陳腐化 anchor を是正する docs 同期を扱う。
...
- `DailySalesPage.tsx:174-176` / `MonthlySalesPage.tsx:165-167` の「レジ日報は未取込みです」を plain `<p>` のまま放置する（owner 決定 2026-09-06、[archived packet](2026-09-05-ui-conventions-batch-design.md):530 参照）、または `role`/`AlertTitle` を誤る
...
- **S7 追加（Gated Amendment 1）**: DSR-01（`01-decision-rules.md:21`）の「`--secondary` stone-200 塗り + `--border` 枠」を「`--secondary` stone-200 塗り + `--border-strong` 枠」へ、catalog `:606` の live 型「`div.flex.items-center.gap-2` 配下に可視 `Label` + `Input`（`max-w-md` 維持）を並べる」を「`div.grid.gap-1` 配下に可視 `Label` を上置きし `Input`（`max-w-md` 維持）を続ける。呼び出し側 toolbar は `items-end` で入力欄の下辺を揃える」へ更新し、両 doc の `## 更新履歴` に owner L3 2026-09-08 決定として 1 行ずつ追記する。`:477` 等の既存履歴行は append-only のため書き換えない
...
- AC-L3-2（owner Windows native L3）: CTA の `secondary`（「新しい取引先を追加」「追加する」）が primary/outline と見分けがつき、くどすぎない枠に見える。page 地・card 地から枠で浮いて見え、溶け込まない（**Gated Amendment 1**）
- AC-L3-3（owner Windows native L3）: 商品一覧・在庫照会・一括価格改定・入出庫履歴（`SearchBar.tsx` 呼び出し 4 画面）の検索欄に「商品を検索」という可視ラベルが見える。4 画面とも toolbar の折返し・並びが Label 追加で崩れない（**F8 追加**）。Label は入力欄の上に置かれ、入出庫履歴の他フィルタ・部門フィルタと揃って見える（**Gated Amendment 1**）
...
| — | 起票時実測「S6」節、[archived packet](2026-09-05-ui-conventions-batch-design.md):530 | RUNTIME-D4（**F1 是正、2026-09-08**） | `DailySalesPage.tsx:174-176`/`MonthlySalesPage.tsx:165-167` の plain `<p>`「レジ日報は未取込みです」は owner 決定（2026-09-06）どおり `<Alert variant="warning" role="status">` へ変換する。catalog `:404` はこの owner 決定を正しく記録しており citation 誤りではなかった（当初の Coordinator 判定を訂正）。catalog `:404` の行番号のみ `PriceRevisionPage.tsx:112-116`→`:79-83` へ訂正する（S7） | `DailySalesPage.tsx`、`MonthlySalesPage.tsx`、`02-component-catalog.md` | 各 file 既存 test の拡張 |
...
2026-09-08: 是正 round 2 = Codex Writer（medium、発注書 `16-fix-pr45-l3.md`）`493740d`、実装手戻り 2（整形 / 旧 selector 更新）。Final Review 一次 = Sonnet fresh（隔離 worktree @ `493740d`）**approve、P1 0 / P2 0 / P3 0**: Gated Amendment 1 の literal 全項目一致、mutant 6 本（枠 token 戻し / wrapper 横並び戻し / 3 toolbar `items-center` 戻し / `shrink-0` 再付与）全 kill、assertion 追加のみで削除・skip なし、docs 更新履歴は append-only、Non-scope 無変更。Coordinator 裁定 = approve + P3 1 no-action（`SearchBar.test.tsx` の「初期表示時に検索 input へ focus する」test に SC15 assertion が同居。round 1 からの既存 pattern、test hygiene Backlog へ）。owner L3 round 2（Windows native、`493740d`）: **AC-L3-2 PASS**（secondary 強枠で「こんな感じ」。「追加はオレンジでは」「一括価格改定に追加ボタンがあること自体」の疑問は DSR-01 の主動線規律を維持したまま、取引先 Select + 追加を検索付き picker dialog へ統合する design-first lane で扱う、owner 合意）/ **AC-L3-3 PASS**（Label 上置きで揃った。他フィルタの Label 上置き統一は別 design lane）。owner Ready 承認 2026-09-08。**state-only 遷移 implementing->local-verified->independent-review->human-confirm->ready-hosted-final を本 commit で圧縮記録**: local-verified = Writer の `local-ci.sh full` PASS（`493740d`、evidence は PR body）、independent-review = Sonnet 一次 approve + Coordinator 裁定、human-confirm = owner L3 round 2 AC-L3-1〜4 PASS、ready-hosted-final = owner Ready 承認。Amendments = `1ea2af6`、Reviewed Content HEAD = `493740d`。次: 本 HEAD で L1 full → PR body 更新 → Ready → hosted final（pull_request run）→ squash merge → closeout（Backlog 起票 13 件を同乗）。
