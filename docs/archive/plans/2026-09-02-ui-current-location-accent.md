...
7. **active 全周枠の経路 parity（gated amendment、L3 round 2 (iv) FAIL 是正）**: `SidebarLink.tsx` の `baseClass` から border 色 class（`border-transparent`）を外し、base は `border border-l-[3px]`（幅のみ）に留める。`border-transparent` は inactive 側に置く — `inactiveClass`（`ActiveMatchSidebarLink` と `<Link inactiveProps>` の両方が使う）に追加し、pending span は `cn(baseClass, "border-transparent cursor-not-allowed ...")` で明示。active 側は既存の `SELECTION_TONE_ACTIVE`（`border-stone-400`）+ `SELECTION_TONE_ACTIVE_ICON` + `CURRENT_LOCATION_ACCENT` のまま。これで plain `<Link>` 連結経路と `ActiveMatchSidebarLink` 単一 `cn()` 経路の class 集合が同一になり（両経路とも「base + active」または「base + inactive」の和集合で、同一 CSS property の衝突対がゼロ）、生成 CSS 順に依存せず active 全周枠 `border-stone-400` が両経路で描画される。tests: (a) active plain Link に `border-stone-400` あり・`border-transparent` なし、(b) inactive plain Link に `border-transparent` あり、(c) **parity oracle**: plain Link の active 要素と `ActiveMatchSidebarLink` の active 要素で `border` で始まる class の集合（`className.split(" ").filter(c => c.startsWith("border")).sort()`）が完全一致。既存 test 無改変。
8. **一時 diagnostic probe（gated amendment、owner L3 round 2 要請、追加観察〈sidebar recenter / 遷移 loop〉の機序確定用。Ready 前に撤去）**: `src/components/layout/sidebar-diagnostic-probe.ts` を新設し、`Sidebar.tsx` から mount する（`useEffect` で install / cleanup）。記録対象と各 record の項目: ①`pointerdown` / `pointerup` / `click` / `focusin`（document capture）: `performance.now()`、event type、target 最近傍 `a` / `[role=link]` の label + href、`document.activeElement` の label + href、sidebar viewport（Radix `[data-radix-scroll-area-viewport]`、`nav[aria-label="メインナビゲーション"]` の祖先）の `scrollTop`、pointer `clientX/clientY` + `document.elementFromPoint` の最近傍 link label ②sidebar viewport の `scroll`（`scrollTop`、前値との差）③router `onBeforeLoad` / `onRendered`（`pathname` + `search`、`appRouter.subscribe`）。in-memory ring buffer 上限 500 件。表示: `Ctrl+Shift+D` で画面右下の `<pre>` overlay（`position: fixed`、選択・コピー可、既定非表示、`z-index` 最大）を toggle、`Ctrl+Shift+C` で `navigator.clipboard.writeText`（失敗時は overlay に "clipboard unavailable" 表示）。制約: UI 文言・layout・既存 component の class / 挙動を変えない、既存 test を改変しない、依存追加なし、`console` 出力は行わない（DevTools 不要、window geometry を保つ）。**撤去義務**: L3 round 3 完了後、Ready 遷移前の content commit で probe file と mount 行を削除し、AC9 で確認する。probe の log は owner が PR コメントへ貼付し、機序（H2 = TanStack scrollRestoration の sidebar viewport 復元）の確定は Coordinator が行う。H2 の是正は router option 変更（R3）のため別 lane。
...
- AC8（gated amendment、parity）: Scope 7 の test (a)(b)(c) が green。mutant「`border-transparent` を `inactiveClass` から `baseClass` へ戻す」で (a) と (c) が red、mutant「`ActiveMatchSidebarLink` の active 側から `SELECTION_TONE_ACTIVE` を外す」で (c) が red。dist CSS の出現順に依存しないことを、active plain Link の className に `border-transparent` が含まれないこと（実 DOM）で確認
- AC9（gated amendment、probe 撤去）: Ready 遷移前の content commit 後に `rg -n "sidebar-diagnostic-probe" src` = 0 hit、`Sidebar.tsx` が amendment 前と同一（`git diff 01350de -- src/components/layout/Sidebar.tsx` が空）
- AC6 (iv) 補足: 「全画面で現在地が一目で分かる」には両経路の active 全周枠が同一に見えることを含む。追加観察（sidebar recenter / loop）は本 PR diff 由来でないことを probe log で確定した上で別 lane へ起票し、AC6 の PASS 判定は本 packet Scope の挙動に限る（owner 裁定）
...
- Final Review round 3（是正 `096f857` + probe `c088fbe`、別個体 fresh Sonnet）: P1/P2/P3 = 0。観察 = AC9 baseline は 1 hit（import 行）/ probe 経由で `app-router` singleton が Sidebar test に eager import される（撤去で解消）/ `Ctrl+Shift+C` は WebView2 が横取りし得る（overlay 選択 + `Ctrl+C` で代替）/ `focus-visible:border-ring` は focus 中のみ左辺色を上書き（既存、Scope 外）。
