...
| C2 | FM3 | unit（BIZ、実 DB） | T2 `biz::product_service::tests::test_commit_import_req104_rejects_overlong_product_code_without_writes`: `valid_rows` = 正常な 1 行 + `A`×101 の 1 行で `commit_import` → `Err(BizError::ValidationFailed(msg))`、msg が上の文言。前後で `products` の件数・`operation_logs` の件数・`inventory_movements` の件数が同じ（正常な行も入らない） | 再検証が無い（再検証を TX の前に置く順序は、TX の中で止まっても Drop で rollback されるため件数では見分けられない。順序は Review Focus の source 確認で見る） |
