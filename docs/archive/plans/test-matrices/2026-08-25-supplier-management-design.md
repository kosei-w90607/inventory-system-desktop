...
|---|---|---|---|
| M-D1 | SPEC-SUP-D1 / D10（Deferred 解除範囲と境界） | `rg -F '取引先の改名・統合・専用管理画面、問屋チャネル、約 80 社の事前一括投入' docs/ --glob '!docs/archive/**' --glob '!docs/plans/**'` = 0 hit、かつ `rg -F '取引先の削除（統合で代替）、問屋チャネル、約 80 社の事前一括投入' docs/function-design/77-ui-bulk-price-revision.md` ≥ 1 hit | 旧 Deferred 行が残存する / 削除・問屋・80 社が誤って解除される（新行から欠落する） |
...
| M-D5 | SPEC-SUP-D2 / D6（78 doc の画面契約） | `docs/function-design/78-ui-supplier-management.md` が存在し、`rg -o 'SPEC-SUP-D[0-9]+' <同 file> \| sort -u` が D1〜D10 の 10 ID、統合確認 dialog の影響件数文言（「付け替わります」）と不可逆明記、および追加導線文言（`rg -F -c '新しい取引先を追加' <同 file>` ≥ 1、owner 裁定 2026-08-25）を含む。dialog 契約の実質は文言 oracle で直接拘束: `rg -F -c '付け替わります' <同 file>` ≥ 1 かつ `rg -F -c '元に戻せません' <同 file>` ≥ 1（Final Review P2-2: 文言が将来削除されても検出できない gap の是正） | doc 欠落 / SPEC ID の一部欠落 / 確認 dialog 契約（件数文言・不可逆明記）の欠落 / 追加導線の欠落 |
...
| M-D9 | SKIP_DOCS 登録 + 削除不在 | `cd src-tauri && cargo test --test design_compliance_test` PASS、`rg -i 'delete_supplier' docs/ --glob '!docs/archive/**' --glob '!docs/plans/**'` = 0 hit | SKIP_DOCS 漏れで CI fail（PR #93 WER 教訓）/ 削除機能が紛れ込む |
...
| M-D11 | SPEC-SUP-D4（merge 参照全数） | `rg -c 'receiving_records' docs/function-design/78-ui-supplier-management.md docs/db-design/master-tables.md` 各 ≥ 1（merge 契約文脈で products と併記）。30-biz は merge 節へ機械的に scope を絞る: `awk '/^### 4\.7\.5/,/^### 4\.7\.6/' docs/function-design/30-biz-product-service.md \| rg -c 'receiving_records'` ≥ 1（file-wide count は将来の無関係言及で感度を失う、Final Review P2-1 = M-D3 round 2 是正と同型）。かつ `rg -c 'SPEC-SUP-D4' docs/db-design/transaction-tables.md` ≥ 1（値所有 doc 側 cross-reference、round 1 P3-2） | receiving_records が契約から欠落し、実装が products のみ付替え → DELETE 時 FK 違反の設計欠陥が正本化される / transaction-tables だけを読む実装者が merge 挙動を見落とす |
