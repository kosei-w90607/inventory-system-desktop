...
- UICONV-D3: `01-decision-rules.md` DSR-08 に増減数値の色規則（+ success-strong / − destructive-strong / 0 muted-foreground）を追記する。`00-foundations.md` の `--success` 用途は「取込み完了」のみ残し、`--success-emphasis` は「増減用途は DSR-08 が置換、icon 用途 1 site（ProductImportPreview）残置」とする（repoint ではなく AA 是正を伴う実質的な色変更、`monthly-sales`/`daily-sales` の `SummaryCardsBar.tsx` の `text-success-emphasis`〈対 `--background` 3.16:1、AA 未達〉→`text-success-strong`〈8.69:1〉が runtime gap、Plan Review round 2 是正）。
- UICONV-D4: badge.tsx の②分類枠線欠落（`--border` 追加が正、`--border-strong` ではない）・①/③種類取り違え・solid pill 2 箇所を runtime gap として記録する。`ProductTable.tsx:84`「反映済み」は UICONV-D12（GA2、owner 原文2、2026-09-06）により success tone の runtime gap として扱う（旧: owner 承認済み現状として gap から除外、撤回済み）。
...
- UICONV-D12（Gated Amendment 2、Writer `068c82c`）: Human Gate (1) 回答（tone family は感情で分ける）を catalog ⑬ 冒頭規約文 + DSR-08 Why + owner culling 列（原文 2、全 4 行）へ反映し、「反映済み」→success 行・「取込み済み」→warning 行の移動と旧 runtime-gap-not 文言の削除を行う。catalog ⑥ Alert 節に未取込み通知 3 箇所の適用先と success Alert 本文色維持（owner 保留）を追記する。
...
- badge.tsx / button.tsx の runtime gap 記述で枠 token が `--border-strong` のまま残る（owner v2 決定は `--border`）。`反映済み` が runtime gap 化されず旧「owner 承認済み現状のため runtime gap ではない」文言のまま残る（UICONV-D12/GA2 で撤回済み）。
...
| UICONV-D2 | success token 値誤り・conditional 化残存 | doc-oracle | `rg -Fc "#bbf7d0" docs/design-system/00-foundations.md` ≥ 1、`rg -Fc "#14532d" docs/design-system/00-foundations.md` ≥ 1、`rg -Fc "| Success Soft " docs/design-system/00-foundations.md` = 1 | 値が異なる、Success Soft 行が 2 行になった、または `--success-border` が未登録のままの場合に検出（owner v2 決定は無条件登録） |
...
| UICONV-D3 | foundations 用途セル未更新 | doc-oracle | `rg -Fc "増減用途は DSR-08 が置換、icon 用途 1 site（ProductImportPreview）残置" docs/design-system/00-foundations.md` ≥ 1（`--success-emphasis`）、`rg -Fc "取込み完了" docs/design-system/00-foundations.md` ≥ 1（`--success`）、`rg -Fc "増減プラス数値" docs/design-system/00-foundations.md` = 0、`rg -Fc "①状態 badge success tone の soft 背景" docs/design-system/00-foundations.md` ≥ 1（`--success-soft`、Plan Review round 3 是正 Opus 指摘 7 番）、`rg -Fc "前月比プラス soft 背景" docs/design-system/00-foundations.md` = 0 | いずれかの用途セルが旧文言のまま、または `ProductImportPreview.tsx:238`〈第 3 消費者〉を無視して「用途なし」と誤記した場合に検出 |
...
| UICONV-D10b | 中立①状態の icon 例外が欠落 | doc-oracle | `rg -Fc "中立 tone の①状態 badge は icon 任意" docs/design-system/01-decision-rules.md` ≥ 1（Plan Review round 3 是正 — packet:172 の記述先は DSR-22〈`01-decision-rules.md`〉であり catalog ではない。catalog 側は`icon 任意`とのみ書く、`StockStatusBadge.tsx:42`「通常」が icon なしで準拠する記述）。reviewer 目視で `review-checklist.md:86` と `:87` の両行が「非中立のみ icon 必須」で揃っていることも確認する（round 2 で :87 の同期漏れを是正、P3-1） | icon 必須ルールが中立 badge にも一律適用されたままの場合に検出 |
| 全体 | catalog:588 冒頭矛盾文の残存 | doc-oracle | `awk '/^## ⑨/{f=1} /^## ⑩/{f=0} f' docs/design-system/02-component-catalog.md` の出力に対し `rg -Fc "検索 \`Input\` は両モードとも"` = 0（`:563`〜`:621` の見出しブロックに限定。Plan Review round 3 是正 — bare "両モードとも" は `:547,581,584` の正当な用法にも一致し unsatisfiable だったため、`:588` 冒頭の一意な文言へ anchor） | SearchBar アクセシビリティ節冒頭の「両モードとも aria-label」矛盾文が残っている場合に検出 |
...
| UICONV-D6 | live 型 Label 反転の欠落・commit 型巻き込み | doc-oracle | 全 oracle は `awk '/^## ⑨/{f=1} /^## ⑩/{f=0} f' docs/design-system/02-component-catalog.md` の出力に対して実行（`:563`〜`## ⑩` 直前 `:621` の見出しブロックに限定）: `rg -Fc "live 型は可視"` ≥ 1、`rg -Fc "live 型は可視 Label を持たない設計"` = 0、`rg -Fc "commit 型は可視 \`Label htmlFor\` + \`aria-label=\"商品検索\"\` の両方を持つ"` ≥ 1（commit 型の Label/aria-label 併存が残っていることの positive oracle、Codex review 5129505910 P1 是正 — 旧 oracle は commit 型契約の破壊を検出できなかった） | 反転漏れ、旧文言が残った、または commit 型の Label/aria-label 併存が誤って削除された場合に検出 |
| UICONV-D6 | aria-label 廃止が runtime lane へ先送りされる | doc-oracle | `awk '/^## ⑨/{f=1} /^## ⑩/{f=0} f' docs/design-system/02-component-catalog.md` の出力に対し `rg -Fc "aria-label は持たない"` ≥ 1（`:563`〜`:621` の見出しブロックに限定） | catalog 本文で確定せず先送り文言のままの場合に検出 |
...
| UICONV-D10 | 04-backbone / review-checklist の同期漏れ | doc-oracle | `rg -Fc "枠線（\`--border\`、DSR-22" docs/design-system/04-backbone.md` ≥ 1、`rg -Fc "枠線（隣接背景に対し 3:1、DSR-22）" docs/design-system/04-backbone.md` = 0、`rg -Fc "操作枠 3:1 は interactive 部品限定" docs/quality/review-checklist.md` ≥ 1、`rg -Fc "②分類は枠線 3:1、icon は識別に必要な場合のみ可" docs/design-system/04-backbone.md` = 0（`:44` token 表、Plan Review round 3 是正） | いずれかの file が narrow 化前の旧文言のまま残っている場合に検出 |
| UICONV-D10 | DSR-22 Why/関連・catalog コメントの 3:1 残存（Plan Review round 3 追加） | doc-oracle | `rg -Fc "境界（枠線または背景）で 3:1 を担保し" docs/design-system/01-decision-rules.md` = 0（`:445`）、`rg -Fc "Badge/outline chip の 3:1" docs/design-system/01-decision-rules.md` = 0（`:451`）、`rg -Fc "分類 badge は枠線 3:1（DSR-22）が規範" docs/design-system/02-component-catalog.md` = 0（`:158`） | round 2 で見落とされた 3:1 の 3 箇所が残っている場合に検出（Opus round 3 指摘 6 番） |
| 全体 | `src/**` 混入 | repo-oracle | `git diff --name-only 07302b5..HEAD -- src` の出力が `src/styles/globals.css` / `src/features/home/HomePage.tsx` / `src/features/backup-restore/BackupRestorePage.tsx` / `src/features/plu-export/PluExportPage.tsx` の 4 file のみ（AC14 の許容範囲、パイプなし）、`git diff --numstat 07302b5..HEAD -- src/styles/globals.css` が追加 4 行・削除 0 行、他 3 file は `git diff --numstat 07302b5..HEAD -- src` で各 file の追加行数 = 削除行数（class 順のみの差分、Plan Review round 3 是正 — round 2 の「非パイプ」修正が AC14 の Gated Amendment 1 例外〈globals.css token 4 行 + 既存 3 file の class 順整形〉と矛盾する `src diff = 0 行` のままだった） | AC14 の許容範囲を超える runtime file が混入、または globals.css に `--success-border`/`--success-strong` 以外の追加・削除が混入した場合に検出 |
| UICONV-D12（GA2） | tone family 感情規約文の欠落 | doc-oracle | `rg -Fc "tone family は感情で分ける: 緑 = 終わったことを伝えるプラスの報告、琥珀 = 「ちょっと待って」の注意、赤 = 警告。状態の名前を伝えるだけの badge は無色" docs/design-system/02-component-catalog.md` ≥ 1、`rg -Fc "badge の tone family は感情で分ける" docs/design-system/01-decision-rules.md` ≥ 1 | catalog ⑬ 冒頭または DSR-08 Why に規約文が反映されていない場合に検出 |
| UICONV-D12（GA2） | 反映済み/取込み済みの行移動が未反映 | doc-oracle | `rg -Fc "owner 承認済み現状のため runtime gap ではない" docs/design-system/02-component-catalog.md` = 0、``rg -Fc '「反映済み」（`variant="default"`（橙）→ success tone の runtime gap）' docs/design-system/02-component-catalog.md`` ≥ 1、`rg -F "\| destructive" docs/design-system/02-component-catalog.md \| rg -Fc "取込み済み"` = 0（row-scoped）、`rg -F "\| warning" docs/design-system/02-component-catalog.md \| rg -Fc "取込み済み"` ≥ 1（row-scoped） | 「反映済み」が旧文言のまま、または「取込み済み」が destructive 行に残る/warning 行に移っていない場合に検出 |
| UICONV-D12（GA2） | owner culling 列の未記入 marker 残存 | doc-oracle | `rg -Fc "owner culling 列は design PR 上で記入（AC-HumanGate、未記入）" docs/design-system/02-component-catalog.md` = 0 | Human Gate (1) 回答転記後も未記入 marker が残っている場合に検出 |
| UICONV-D12（GA2） | Alert 追加適用先・success 本文色維持の記述漏れ | doc-oracle | `rg -Fc "追加適用先（owner 決定、2026-09-06）" docs/design-system/02-component-catalog.md` ≥ 1、`rg -Fc "success Alert の本文色は現状（\`text-success-strong\`）を維持する" docs/design-system/02-component-catalog.md` ≥ 1 | 未取込み通知 3 箇所の適用先、または success Alert 本文色の owner 保留決定が catalog ⑥ に反映されていない場合に検出 |
...
|---|---|---|---|---|
| ①状態 = outline + icon + soft tone（枠は tone 色、3:1 不要） | `rg -n "<Badge" src/features src/components --glob '!*.test.*'` 全件（約 35 箇所） | `StockStatusBadge.tsx:25,34,42`（stockout/low/ok）、`StocktakePage.tsx:396-401`（未入力 N、warning）、`PreviewStep.tsx:75-80`（同日データあり、warning）、`DailyReportImportPage.tsx:164,183`（同日データあり、warning） | `ProductTable.tsx:79`「未反映」（`variant="secondary"` 取り違え、runtime gap）、`ProductTable.tsx:84`「反映済み」（`variant="default"`（橙）→ success tone の runtime gap、UICONV-D12/GA2 で owner 承認済み現状の除外を撤回）、`ResultStep.tsx:47`「部分成功」「成功」（tone なし、runtime gap）、`CsvImportRecordDetailPage.tsx:140,192`「取消済み」「明細取消済み」（tone なし、runtime gap）、`DailyReportImportPage.tsx:179`「取込み済み」（`variant="destructive"` 塗り、runtime gap 候補）、`StocktakePage.tsx:404`/`IntegrityCheckPage.tsx:387`「補正済み」（`bg-success` 直接塗り、4種目相当、要移行）、`IntegrityCheckPage.tsx:381`（差異ラベル、owner culling） | 起票時実測、runtime lane で `rg` 再実行し是正状況を確認 |
