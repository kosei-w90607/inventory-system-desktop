...
| SPD-D7 | §61.9 Test Focus の UI-02-D3 行に picker dialog 経由の観点が追加されていない（Gated Amendment 4 J6） | doc-oracle | `rg -Fc "picker dialog 経由" docs/function-design/61-ui-receiving.md` ≥ 1（baseline 0、§61.9 は `:151`） | テスト観点への反映漏れを検出（AC33） |
...
| SPD-D1/SPD-D2 | canonical が本 packet 内部の節参照・archive 後に壊れる語（Gated Amendment 1 A1、J5 で multi-file awk の誤りを訂正し個別 2 回実行に） | doc-oracle | `awk '/^## 更新履歴/{exit}{print}' docs/design-system/02-component-catalog.md \| rg -c "Contract Probe\|起票時実測参照\|起票時実測「\|catalog には\|Human Gate \(2\) で覆されない限り"` = 0（baseline 0）かつ `awk '/^## DSR-24/,/^## DSR-25\|^## 更新履歴/' docs/design-system/01-decision-rules.md \| rg -c 同上パターン` = 0（baseline 0、DSR-24 節に限定し無関係な DSR-17/18 の「Contract Probe」を誤検出しない） | canonical に packet 内部参照（archive 後に壊れる語）が残った場合に検出（AC20） |
...
| SPD-D2/SPD-D3 | 列見出し「選択」が視覚表示されたまま sr-only 化されていない（Gated Amendment 3 H4、owner「列タイトルが取引先名と比べて浮いてる」所感、状態数を 7 に訂正） | doc-oracle | `rg -c 'class="sr-only">選択' docs/design-system/reference/mockup-f-supplier-picker.html` ≥ 7（baseline 7 実測 — 状態 1/2/4/5/6a/6b/7 すべてに既に実装済み、次回改訂での回帰防止 oracle） | 列見出しの「選択」が視覚要素のまま残っている場合に検出（AC28） |
| SPD-D2/SPD-D3 | 追加ボタンが secondary/全角「＋」のまま、または primary + inline SVG icon 化されていない（Gated Amendment 4 J1/J2、owner round 2「＋じゃ寂しい」所感） | doc-oracle | catalog: `awk '/^\*\*picker dialog/{f=1} f{print} f&&/^---$/{exit}' docs/design-system/02-component-catalog.md \| rg -Fc "新しい取引先を追加（\`Plus\` icon 付き）"` ≥ 1（baseline 0）+ `rg -c "secondary"` 同範囲 = 0（baseline 1、AC31）。mockup: `rg -c 'class="btn primary"' docs/design-system/reference/mockup-f-supplier-picker.html` ≥ 7 かつ `rg -c 'class="picker-footer".*<svg' docs/design-system/reference/mockup-f-supplier-picker.html` ≥ 7（baseline: primary 10〈footer 7 含む〉/ footer svg 0） | 追加ボタンが secondary のまま、全角「＋」表記のまま、または SVG icon が無い場合に検出（AC29a/AC29b/AC31） |
| SPD-D2/SPD-D3 | 「現在の選択」固定帯が scroll 箱の外に実装されていない、または小見出しラベルが欠落（Gated Amendment 3 H6 + 4 J3/J4、owner round 1/2「枠の上に固定」「見分けられる」所感） | doc-oracle | `rg -c 'class="current-band"' docs/design-system/reference/mockup-f-supplier-picker.html` ≥ 5（baseline 4、状態 1/2/4/7 実装済み・6b が対象）かつ reviewer 実読で `current-band-label`〈小見出し「現在の選択」、`--muted`〉が band と同数存在することを確認（`rg -c '現在の選択'` は state-7 lead 文の地の文とも一致するため単独では不十分、AC32 参照） | 固定帯が実装されず一覧側の選択行表示だけで済まされた場合、または小見出しラベルが欠落した場合に検出（AC30/AC32） |
...
- AC22 は Gated Amendment 2（A10）で `class="badge b-state">選択中` の literal へ硬化し、旧オラクル（`rg -Fc "選択中"` ≥ 3、`aria-label` だけで満たせた）の弱点は解消した。ただし mockup の render 判定は静的 HTML の文字列読みで行っており、実際にブラウザで枠線・背景色・列幅が意図どおり見えるかは owner の Human Gate（実機確認）のみが担保する — automated では見た目の正しさまでは検証できない。
