...
- Plan Commit: d9f7b53
- Amendments: 2（`1135628` Contract Probe #1〜#4 結果確定 / `1567b42` post-publish staging unlink 失敗 = Err 契約確定。件数は列挙 SHA と一致させる）
...
- `src-tauri/src/db/mod.rs` `migrate_legacy_db`（MNT-03-D2/D3）: 3 ファイル個別コピーを廃止し、旧 DB を NO_CREATE + 通常モードで開いて `VACUUM INTO` で一時名（`.migrating`）へ生成 → 成功後に最終名へ **no-clobber publish**（publish 直前の destination 不在再確認 + 既存 destination を置換しない platform primitive、Contract Probe #4 で確定 — 22 §12.1 ステップ6）。失敗時は一時ファイルを削除して Err。旧 DB 存在確認の error は skip（`Ok(false)`）ではなく Err（MNT-03-D4 経路）。「新パス main は完成品しか存在しない」不変条件。
...
- Assumptions and constraints: SQLite 挙動（VACUUM INTO の WAL 取込み / SHM 再構築 / WAL 再生）は design phase で 3 系統検証済み、実 WAL fixture テストが empirical validation を兼ねる。`tauri-plugin-single-instance` の挙動と no-clobber publish primitive の platform 差は未検証前提 → Contract Probe #3/#4。
...
| Fact check / design decision split | 未検証の外部前提（plugin 挙動 / publish primitive / dialog threading）を Contract Probe 4 件に分離、設計判断（fail-closed 等）は確定済み正本に依拠 | Contract Probe |
...
4. **no-clobber publish primitive**（22 §12.1 ステップ6）: Rust std `rename` は既存 destination を置換し得るため、Windows で「既存 destination を置換しない」ことが保証される primitive の選定と実証。
...
- negative tests: 失敗注入・中断注入・fail-closed 分岐（M2, M3a, M3b, M4〜M6, B1, B3〜B9, B11, B12）が本 PR の主体。
...
- no-create / no-clobber の徹底 — create-capable な経路・migration publish で既存 destination を置換する primitive の残存ゼロ。
