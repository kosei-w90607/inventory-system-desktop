...
- State Narrative（2026-07-24）: public-writer clone、期待 origin、clean `main`、
  基準 HEAD `21ce834`、active packet 不在を確認し、fetch / checkout / pull で
...
- State Narrative（2026-07-24、merge / archive）: PR #24 は exact-HEAD hosted
  ...
  （このchangeでの介入3回目 / 予算3回）により squash merge `45da61b` となった。
...
基準 HEAD: `21ce834`。監査 finding の旧行番号ではなく、現実装を実読した。
