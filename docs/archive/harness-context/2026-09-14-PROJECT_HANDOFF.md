...
>
> **最終更新**: 2026-08-26 / PR #6（public repo）docs 実装状況棚卸し merge。docs 10 点の stale 表記 16 箇所を実装実態へ同期し、REQ-206/207 を `required` へ昇格（REQ-208 は未実装につき `deferred` 維持）。当初仕様の committed 残実装ゼロは PR #5 で達成済み、次はリリース道筋 ③〜⑤ から owner 選定。旧記録: 2026-08-26 / PR #5 実装 PR C 入庫時原価差分検出 cost_diffs merge。BIZ step 10（COMMIT 後の完全一致比較、保存成否に非影響）+ `ReceivingCreateResult.cost_diffs`（field 追加のみ）+ UI-02 保存完了時の原価差分ダイアログ（`reviseProductPrice` 流用・新売価据え置き）が main に入り、REQ-209 の T3 WARN が解消 — **当初仕様の committed 残実装はゼロ**。次はリリース道筋 ②〜⑤（docs 実装状況棚卸し / 表示磨き batch / UI 一覧の背骨 D / go-live 検証 flow + MSI 配布手順）から owner 選定。旧記録: 2026-08-26 / PR #4 取引先管理 実装（UI-15 `/settings/suppliers`、migration v6、supplier 3 command、D-052 C21/C22、REQ-107 required 昇格。design-first は PR #3）
...
### 直近の作業状態
- **docs 実装状況表記の一括棚卸し**: **完了**（R2 docs-only、PR #6 merge `bb4969e`、2026-08-26）。docs 10 点の stale 表記 16 箇所を実装実態へ同期（SCREEN_DESIGN §1 状態列 / FUNCTION_DESIGN 実装済み追記 / 52・53・65・68・69・74 の将来形記述是正）+ REQ-206/207 `required` 昇格（test 実在を実測、T3 WARN なし）+ REQ-208 `deferred` 維持 + 90-traceability 再生成。表記規約 = 新規表記は PR 番号非転記・dated 履歴 block は据え置き。Codex fail-closed（真陽性 6 箇所）と Final Review（Q40 overclaim 捕捉)を経て gated amendment 3 本で収束、hosted run 初回 1 fail は main 既存の failpoint 並列 race（backlog 起票済み、rerun success で三点一致）。証跡は archived [Packet](../plans/2026-08-26-docs-impl-status-inventory.md)。
- **実装 PR C: 入庫時原価差分検出 cost_diffs**: **完了**（R3、PR #5 merge `854948f`〈merge commit〉、2026-08-26）。31-biz §12.3 step 10（COMMIT 後の完全一致比較・±1 円も差分・保存 TX 非影響・idempotent replay は空配列）、`CostDiff` 4 field + `ReceivingCreateResult.cost_diffs`（field 追加のみで既存 consumer 互換、bindings 追加のみ diff）、UI-02 保存完了時の原価差分ダイアログ（`CostDiffDialog.tsx`、「マスタ原価をこの実原価に更新する」= `useReviseProductPrice` hook 流用で D-052 C20 適用・新売価は `getProduct` 現売価据え置き・行単位成功/失敗 + 再試行・見送り無記録）、予約 Rust test 4 本 + RTL 5 本、90-traceability 再生成で REQ-209 の T3 WARN 解消。invalidation は新規 C entry なし（SPEC-PRVC-D1、D-052 無改変）。Plan rally 2 round、Final Review = AC 8/8 再実測 + mutation M1〜M7 独立注入全 red・P1/P2 0、Windows native L3 5 項目 PASS、hosted run `32884586081` で exact-HEAD `0154aea` 三点一致。証跡は archived [Packet](../plans/2026-08-26-receiving-cost-diffs-impl-c.md) / [Matrix](../plans/test-matrices/2026-08-26-receiving-cost-diffs-impl-c.md)。UX 所見 3 件（非阻害 P3）は Plans.md backlog の表示磨き batch 候補へ起票。
- **取引先管理（UI-15）design-first + 実装**: **完了**（R3。design-first = PR #3 merge `ff08a6b`〈2026-08-25、SPEC-SUP-D1〜D10 / REQ-107 / D-078 の正本化〉、実装 = PR #4 merge `c2cdda8`〈2026-08-26〉）。UI-15 `/settings/suppliers`（name 昇順一覧 + 関連商品数・入庫記録数 / 追加 dialog / インライン改名〈trim・同名衝突は統合案内・同値 no-op〉/ 統合 dialog 2 段階〈影響件数 + 不可逆文言〉）、migration v6（`suppliers.updated_at` nullable + backfill、`MigrationKind::Custom`）、`rename_supplier` / `merge_suppliers`〈products + receiving_records 付替え → DELETE の 1 TX + operation_log〉/ `list_suppliers_with_usage`、D-052 C21/C22（consumer 全数導出の 8 key 対称集合）、navigation `ui-15` + REQ-107 到達テスト、REQ-107 `required` 昇格。Plan rally 2 round、gated amendment 4 本（全 true positive）、Final Review P1/P2 0（mutant 7 件実注入全 red）、Windows native L3 4 項目 PASS、hosted run `32870443858` で exact-HEAD `b4bb17c` 三点一致。証跡は archived [design Packet](../plans/2026-08-25-supplier-management-design.md) / [impl Packet](../plans/2026-08-25-supplier-management-impl.md) / 各 Matrix。backlog 起票 3 件（78 §78.4 表記 drift / 改名 double-click / 改名時通知非対称）は Plans.md 参照。
- **価格改定支援 実装 B（UI-14 一括価格改定画面 + `ProductSearchQuery` 取引先 filter）**: **完了**（R3、PR #95 squash merge `4b2b109`、2026-08-23）。SPEC-PRV-D3 / D4 / D6 / D7 を実装（取引先 filter + 未設定含む既定 on / 新原価案 floor と掛率の pure module / 行単位確定 + D-052 C20 / 取引先 inline 追加と漸進紐付け / 本日 changed_at の「最近改定」badge / navigation `ui-14` / 共有 `ListSkeleton`）。gated amendment 1（離脱ガード EXCLUDED〈UI-USW-D3 (c)〉）。Plan rally 3 round、Final Review P1/P2/P3 = 0（mutant 10 件全 kill）、Windows native L3 4 項目 PASS、hosted run `32594023444` で exact-HEAD `0fca6eb` 三点一致。介入 3/3。証跡は archived [Packet](../plans/2026-08-23-price-revision-impl-b.md) / [Matrix](../plans/test-matrices/2026-08-23-price-revision-impl-b.md)。L3 の UX findings 5 点は「UI 一覧の背骨 D」へ（Plans.md 次の行動）。
- **価格改定支援 実装 A（backend + UI-01a/01b）**: **完了**（R3、PR #94 squash merge `58adcc2`、2026-08-22）。SPEC-PRV-D2 / D5 / D6 / D9 / D10 を実装（keyword maker_code〈通常検索 + 一括 PLU 母集団〉/ `revise_product_price`〈no-op・売価変更時のみ plu_dirty・price_history 4 値・operation_log・NULL のときだけ supplier〉/ `create_supplier` / `list_price_history` / CMD 3 件 + bindings / UI-01a 原価列 + placeholder / UI-01b 価格履歴 + 取引先 inline 追加 / 90-traceability 再生成）。Plan Gate rally 3 round、Final Review P2 1（tie-break 未テスト）是正後 P1/P2/P3=0、L1 full + hosted run `32581375434` 三点一致、Windows native L3 3/3 PASS。証跡は archived [Packet](../plans/2026-08-22-price-revision-impl-a.md) / [Matrix](../plans/test-matrices/2026-08-22-price-revision-impl-a.md)。次 = 実装 PR B（UI-14。`ProductSearchQuery` に取引先 filter が未定義のため packet で amendment 同梱か design-first 先行かを owner 裁定）/ 実装 PR C（入庫 cost_diffs、B と独立）。
- **価格改定（値上げ連絡）支援 design-first**: **完了**（R3 docs-only、PR #93 squash merge `4b0fede`、2026-08-22）。SPEC-PRV-D1〜D12 を source docs へ正本化（suppliers = メーカー意味 / keyword maker_code / UI-14 一括価格改定画面新設 / revise_product_price + create_supplier + list_price_history / 入庫 cost_diffs step 10 / UI-01b 価格履歴 + inline 取引先 / UI-01a 原価列 / REQ-105・106・209 + 90-traceability 再生成）。Plan Gate rally 3 round、Final Review 是正後 P1/P2/P3=0、L1 full + hosted run `32561913686` 三点一致。証跡は archived [Packet](../plans/2026-08-22-price-revision-design.md) / [Matrix](../plans/test-matrices/2026-08-22-price-revision-design.md)。次 = 実装 PR A（backend + UI-01a/01b）packet 起草。
- **stacked train + 発注書規律の workflow docs 正本化**: **完了**（R3、PR #89 squash merge `712af0b`、2026-08-21）。DEV_WORKFLOW へ stacked train 小節（単段 merge 確立 / STATECAP 二段 cap 継承計上と content commit 同乗）+ D-074 新設 + 採番規律 + 発注書規律（referent 一致 / traceability 完了条件）+ fixture encoding を正本化。Plan Gate 3 round / Final Review Double Audit P1/P2/P3=0、exact-HEAD 三点一致（`dc1d856`、run `32391371212`）。同日 PR #88 で nanoid 3.3.18 更新（issue #82 消化）。証跡は archived [Packet](../plans/2026-08-20-stacked-train-workflow-docs.md) / [Matrix](../plans/test-matrices/2026-08-20-stacked-train-workflow-docs.md)。
- **PLU bulk onboarding 実装（実装 B）**: **完了**（R3、PR #86 squash merge `b2389b1`、2026-08-20）。商品 CSV 任意列 `PLU対象` / `bulk_set_plu_target`（filter 一致全件の 1 TX 一括 ON・OFF + 件数確認 dialog）/ 商品一覧の PLU 移行 badge 独立列 + `plu` URL filter / UI-01c preview 列 / D-052 C19 を実装。C 番号は PR #85 amendment 5 の C18 = prepare と衝突したため gated amendment 5 で C19 へ改番。squash 後の base 付け替えは rebase / 2 段 merge を実測棄却して origin/main 単段 merge + content commit 同乗遷移で成立（stacked train の workflow docs 正本化は follow-up）。Final Review round 1 P1/P2=0 + delta 再検証 P1/P2/P3=0、owner visual confirmation V1〜V6 全 PASS、hosted run `32374702949` で exact-HEAD 三点一致（`5079a79`）。証跡は archived [Packet](../plans/2026-08-19-plu-bulk-onboarding-implementation.md) / [Matrix](../plans/test-matrices/2026-08-19-plu-bulk-onboarding-implementation.md)。
- **PLU slot core 実装（実装 A）**: **完了**（R3、PR #85 squash merge `014c064`、2026-08-20）。`plu_slots` migration v5 / Z004 snapshot 照合 / sticky 最小空き予約 / clear 行解放 / UI-08 snapshot step + Diff gate / UI-01b レジメモリNo. を実装。Windows native L3 2 round で実機確認し、S9 起源の gated amendment 5（release_pending の Diff 計上 + D-052 C2/C14/C18）を同 PR で正式化、clear 行の CV17 受理・レジ未設定化の実機確認で `PLU_CLEAR_ROW_ENABLED=true` 確定。Final Review delta P1/P2=0、L1 full + hosted run `32368937596` の exact-HEAD 三点一致（`b131fd1`）。証跡は archived [Packet](../plans/2026-08-18-plu-slot-core-implementation.md) / [Matrix](../plans/test-matrices/2026-08-18-plu-slot-core-implementation.md)。次 = 実装 B（bulk onboarding）packet 起草。
- **CSV取込み記録詳細（65 slice 4b）**: **完了**（R3、Writer = Codex、PR #58 squash merge `3ec84c4`、2026-08-03）。在庫変動履歴「CSV取込み #n」link の 404 を解消 — IO / BIZ / CMD の詳細取得 + generated bindings + `/csv-import/records/$importId` 詳細 route / Page + rollback invalidation（D-052 C9 へ `inventoryRecords.root()` 追加）+ T1-T17。CSV取込み画面は layout + index route へ再構成（gated Amendment 1、Writer fail-closed の true positive 検出起源）。owner L3 P3（ErrorRowsTable accordion の発見性）は owner 裁定の同 PR 是正で「エラー詳細を見る（N件）」の明示的操作文言へ改修し再 L3 PASS（gated Amendment 2、55 §55.5 同期）。編成 = Fable Coordinator / Codex Writer / Sonnet Plan+Final Reviewer（D-062 (c) 適合形の初運用）。Final Review Ledger 22/22・P1/P2=0、mutation 独立再実測 X1-X7 全 red、exact-HEAD 三点一致（run `30767350640`）。証跡は archived [Packet](../plans/2026-08-03-csv-import-record-detail.md) / [Matrix](../plans/test-matrices/2026-08-03-csv-import-record-detail.md)。follow-up = 一覧 route / 棚卸し詳細 / 74 許可リスト追随（後続スライス）、JAN 列の pre-existing drift（Plans.md backlog）。
- **店舗調査証跡と導入時運用契約の同期**: **完了**（R3 docs-only、PR #55 squash merge `fc19daa`、2026-08-01）。Z004を個数・金額を持つ確認済みPLU別売上として整理し、BIZ-03の在庫増減・rollback実装済み範囲と、layout A/B対応・店舗shape再検証の残件を分離した。日報取込みは`SD -> CV17 -> PC側EcrDatas`を標準手順とし、layout差はoperator選択ではなくadapter互換性として扱う。Excelファイル群の毎日上書き、印刷・バインダーの日別履歴役割、紙代替operator受入の未完了も正本化。Sonnet 5のPlan Review / Final Contract Auditは最終P1/P2/P3=0、Ready HEAD `5251cecb`のlocal / hosted run `30664891332`三点一致を確認した。証跡はarchived [Packet](../plans/2026-08-01-field-evidence-operations-sync.md) / [Matrix](../plans/test-matrices/2026-08-01-field-evidence-operations-sync.md) / [WER](../plans/2026-08-01-field-evidence-operations-sync-workflow-effectiveness-review.md)。後続はZ004 layout A/B、固定slot / bulk onboarding、EcrDatas lifecycle、紙代替受入の各R3。
- **CI 2026-08-01 re-evaluation / public Actions contract**: **完了**（R3 workflow gate、PR #54 squash merge `69ec8fc`）。public repository + standard `ubuntu-latest` ではprivate-era billed-minute thresholdを使わず、completed HEADの状態に応じてReady / dispatch / recovery / no-opをCI-TRIGGER-D1から選ぶ契約へ更新した。live-doc drift guardは`ci-workflow.test.sh`を`local-ci.sh full`から実行するL1 local-onlyで、hosted docs jobは未配線。Sonnet 5 Plan ReviewとFinal Double Audit closureはP1/P2=0、Ready HEAD `01a4f9c`のlocal full / PR body / hosted run `30654559831`の三点一致を確認し、予防的dispatchなしでmergeした。証跡はarchived [Packet](../plans/2026-08-01-public-actions-ci-contract.md) / [Matrix](../plans/test-matrices/2026-08-01-public-actions-ci-contract.md) / [WER](../plans/2026-08-01-public-actions-ci-contract-workflow-effectiveness-review.md)。hosted実防御が必要になった場合の`doc-consistency-check.sh`統合は別R3、scheduled npm monitor dogfoodは2026-08-03 JST、dependency change Bは2026-08-05以降の別change。
- **Claude Hook contract audit**: **完了**（R3 workflow gate、PR #48 squash merge `985eb76`、2026-07-31）。repository-owned project hookを0本へ縮約し、未監査harnessをproject単位で無効化。source-derived fixtureによるsettings / docs / hook propagation、classifier、local full、hosted CI配線を固定した。consultation relayのPlan Gate 3 roundとFinal Double Audit + frozen closureでP1/P2=0、P2-CH10 CLOSED。Ready exact-HEADのlocal / hosted三点一致後にmergeし、Matrix CH10表示名もcloseoutで同期済み。証跡はarchived [Plan Packet](../plans/2026-07-31-claude-hook-contract-audit.md) / [Matrix](../plans/test-matrices/2026-07-31-claude-hook-contract-audit.md)。次dogfoodは次のClaude Code fresh sessionでproject hook 0本・harness無効・虚偽hook注入なしを確認する。
- **D-058 Fable slot不在編成の相談窓口役**: **完了**（R3 workflow gate docs-only、PR #47 squash merge `3009c77`、2026-07-30）。target PR外の専用remote order branch、order / target両refのcurrent-head照合、target SHA時点Packet、Coordinator予約枠、template `Consultation Relay`欄を正本化した。Double Audit closure P1/P2=0、Ready HEAD `56f76d4`のexact local / hosted三点一致を確認し、Post-Freeze P3 2件もcloseoutで是正済み。証跡はarchived [Plan Packet](../plans/2026-07-30-d058-consultation-relay-role.md) / [Matrix](../plans/test-matrices/2026-07-30-d058-consultation-relay-role.md)。最初のdogfood targetは次のeligible consultation relay order。
- **Node 24 toolchain alignment**: **完了**（R2、PR #46 squash merge `102f111`、2026-07-30）。`.node-version`のNode 24.18.0をlocal / frontend CI / npm security monitorのsingle ownerとし、`engines.node` / `devEngines.runtime`、`@types/node`、DEV_SETUP、両workflowのstatic drift testを同期した。Claude Sonnet 5 / xHigh Final Review P1/P2/P3=0、Ready HEADのexact local full / Hosted CI三点一致を確認。証跡はarchived [Plan Packet](../plans/2026-07-30-node24-toolchain-alignment.md)。CI側dogfoodは完了し、npm security monitor側は次回scheduled/manual runで確認する。
- **finite search / threshold descriptor contract（監査順16 / wave 4 lane 1）**: **完了**（R3、PR #38 squash merge `a29f2e8`、2026-07-30）。product / movement / records / daily / monthly / stock inquiryの有限値ownerとthreshold 2 fieldをfeature-local descriptorへ集約し、独立literal oracleと実mutationで片側driftを閉じた。Claude Opus 5 / Sonnet 5 closure P1/P2/P3=0、merge-train rebase同値、final HEADのexact-HEAD local / hosted三点一致を確認。表示・操作・URL・fallback・wire・保存意味論は不変。証跡はarchived [Plan Packet](../plans/2026-07-29-finite-search-threshold-contract.md) / [Matrix](../plans/test-matrices/2026-07-29-finite-search-threshold-contract.md) / [wave 4 WER](../plans/2026-07-30-wave-4-workflow-effectiveness-review.md)。dependency / lockfileは非変更で、audit warn-onlyの更新判断は次の独立changeへ分離した。
- **PK3 negative-glob false WARN是正（wave 4 lane 3）**: **完了**（R2、PR #39 squash merge `669dfee`、2026-07-30）。Plan Packetのtest token探索から重複negative globを除去し、runtime argv guard・3 root canary・Git初期化済みignored-only fixtureで偽WARNの非再発を固定した。Claude Opus 5 / Sonnet 5 closure P1/P2=0、merge-train rebase同値、final HEADのexact-HEAD local / hosted三点一致を確認。証跡はarchived [Plan Packet](../plans/2026-07-29-pk3-negative-glob-false-warn.md) / [Matrix](../plans/test-matrices/2026-07-29-pk3-negative-glob-false-warn.md) / [wave 4 WER](../plans/2026-07-30-wave-4-workflow-effectiveness-review.md)。
- **request builder shared primitives（監査是正 順21 P1-4 / wave 4 lane 2）**: **完了**（R2、PR #40 squash merge `b3ac5e5`、2026-07-30）。入庫・返品交換・手動販売・廃棄のidempotency key、local date、strict safe integer parserを`src/lib/request-helpers.ts`へ集約し、feature固有payload・validation・UI・wire契約を維持した。旧PR #37のOwner Effort Budget / STATECAP問題はclean replacementで閉じ、governance closure P1/P2=0、final HEAD `3c0af9d`のexact-HEAD local / hosted三点一致を確認。R2 per-lane Subagent Budget超過はincidentとしてPacketへ記録済み。証跡はarchived [Plan Packet](../plans/2026-07-29-request-builder-primitives.md) / [Matrix](../plans/test-matrices/2026-07-29-request-builder-primitives.md) / [wave 4 WER](../plans/2026-07-30-wave-4-workflow-effectiveness-review.md)。
- **商品更新patch wire契約（監査是正 順13 / wave 3 lane 2）**: **完了**（R3、PR #36 squash merge `90cc963`、2026-07-29）。Rust serdeとgenerated TypeScript入力型のomitted / null / value意味論を一致させ、frontendの手書き`Partial` / castを撤去。51 / SCREEN_DESIGNのUI-01b generated command statusとdurable wording guardも同期した。operator-visible UI、route、BIZ更新、DBは不変。Final Review closure P1/P2=0、STATECAP訂正、exact-HEAD local / hosted三点一致を確認。証跡はarchived [Plan Packet](../plans/2026-07-29-product-update-patch-wire-contract.md) / [Matrix](../plans/test-matrices/2026-07-29-product-update-patch-wire-contract.md) / [wave 3 WER](../plans/2026-07-29-wave-3-workflow-effectiveness-review.md)。
- **Home orchestration回帰test（監査是正 順10 / wave 3 lane 1）**: **完了**（R2、PR #35 squash merge `b9d7e49`、2026-07-29）。production TypeScriptを変えず、ホーム画面の4 query、派生値、部分障害、visible日付またぎ、前日未取込みwarning、独立2 toastを実hook / QueryClient経由の3 test fileで固定した。operator-visible UI、route、wire、DB、既存testは不変。exact-HEAD local / hosted三点一致とFormal Final Review P1/P2=0を確認。証跡はarchived [Plan Packet](../plans/2026-07-29-home-orchestration-tests.md) / [Matrix](../plans/test-matrices/2026-07-29-home-orchestration-tests.md)。
- **SortableHeader共通component抽出（監査是正 順21a / wave 3 lane 3）**: **完了**（R2、PR #34 squash merge `3f5086b`、2026-07-29）。日次商品別・月次部門別・月次商品ランキングの3 tableに重複していたheader実装をgeneric canonical componentへ集約し、列集合、click payload、ARIA、indicator、alignment、operator-visible表示を維持した。exact-HEAD local / hosted三点一致とFormal Final Review P1/P2=0を確認。証跡はarchived [Plan Packet](../plans/2026-07-29-sortable-header-extraction.md) / [Matrix](../plans/test-matrices/2026-07-29-sortable-header-extraction.md)。
- **未参照UI wrapper / RHF専用dependency退役（監査是正 順19 / wave 2 lane 1）**: **完了**（R2、PR #32 squash merge `29b35ed`、2026-07-28）。consumer 0のwrapper 3fileとdirect dependency 2件を退役し、フォーム採用方針をfeature-local controlled state + Zodへ一本化。現役`zod` / `radix-ui`、route、画面表示・操作、wire / DBは不変。証跡はarchived [Plan Packet](../plans/2026-07-28-unused-ui-wrapper-dependencies.md) / [Matrix](../plans/test-matrices/2026-07-28-unused-ui-wrapper-dependencies.md) / [wave 2 WER](../plans/2026-07-28-wave-2-workflow-effectiveness-review.md)。
- **import内部contract最小化と日報parse診断接続（監査是正 順20 / wave 2 lane 2）**: **完了**（R3、PR #33 squash merge `6c53c44`、2026-07-28）。CSV / 日報importの内部型からconsumer未使用fieldを除去し、日報parse error 5fieldをstructured diagnostic WARNへ接続した。CMD preview token lifecycle、利用者向けerror、operation log、wire / DB / operator UIは不変。証跡はarchived [Plan Packet](../plans/2026-07-28-import-internal-contract-minimization.md) / [Matrix](../plans/test-matrices/2026-07-28-import-internal-contract-minimization.md) / [wave 2 WER](../plans/2026-07-28-wave-2-workflow-effectiveness-review.md)。
- **GPT-5.6 agent guidance / local override分離（D-057）**: **完了**（R2、docs-only、PR #31 squash merge `c7b0fce`、2026-07-28）。全Codex共通の判断・approval・検証契約とslot-neutral profileをtracked guidanceへ置き、具体model対応はAGENT_OPERATING_MANUAL §3.4だけが所有する。個人用の応答人格と実測出力はignored root override / local-only evidenceへ分離し、公開fixtureだけをrepositoryへ残した。fresh-session regression・独立Final Review・publish-boundary audit・exact-HEAD local / hosted三点一致を完了。証跡はarchived [Plan Packet](../plans/2026-07-28-gpt56-agent-guidance-local-override.md) / [Matrix](../plans/test-matrices/2026-07-28-gpt56-agent-guidance-local-override.md)。次は[Plans.md](../../../Plans.md)項0のwave 2編成または単線継続のowner選定。
- **file 契約一元化（監査是正 順9）**: **完了**（R3、PR #26 squash merge `5a9d7a0`、2026-07-27）。Rust 上限定数から bindings へ生成 export し、売上・日報・商品 import hook を単一契約へ統一。商品 import の CMD+BIZ 二重 guard、dialog+drop 共通 FilePicker（3 画面 5 call site）、`useCsvImportFlow` 実配線 test、静的再導入 sweep、返品画像 Blob プレビューを実装。Final Review 一次が drop 経路の Windows WebView2 絶対パス防御消失（P1）を捕捉し、共有 `src/lib/extractFilename` へ統合是正 + Matrix X10。X1〜X10 は clean tree 実注入で全件独立再実測、Plan Review 3 round / Final Review 2 round で P1/P2=0、視認 PASS、L3 = synthetic Z004 で取込み完走（実採取 layout A の graceful stop は既存 backlog「Z004 parser の layout A 対応」で追跡）、exact-HEAD L1 / hosted final 一致、owner 介入 3/3 で完了。証跡は archived [Plan Packet](../plans/2026-07-26-file-contract-unification.md) / [Matrix](../plans/test-matrices/2026-07-26-file-contract-unification.md) / [WER](../plans/2026-07-27-file-contract-unification-workflow-effectiveness-review.md)。次候補は監査是正 順10 のscope精査。
- **error 境界 + PLU 語彙（監査是正 順8）**: **完了**（R3、PR #25 squash merge `10e33cb`、2026-07-26）。`CmdError.error_id`（wire × 診断ログの相関、D-053）と internal sanitize（raw 技術詳細の wire 混入根絶）、restore 3 kind の相関 ID、PLU `export_error` 分離と `PluFileOutput` / `plu_output` rename、共通 `describeError` + 復元画面の ID 別要素併記（68 §68.7 の recovery 文言設計は維持）を実装。X1-X8 mutation の Final Review 独立再実測、視認 PASS、exact-HEAD local full / hosted final 三点一致、owner 介入 3/3 で完了。raw message 直接表示 約15 箇所の共通化と `restore_failed_recovered` raw text 整合は follow-up として `Plans.md` に記録。証跡は archived [Plan Packet](../plans/2026-07-26-error-boundary-plu-vocabulary.md) / [Matrix](../plans/test-matrices/2026-07-26-error-boundary-plu-vocabulary.md) / [WER](../plans/2026-07-26-error-boundary-plu-vocabulary-workflow-effectiveness-review.md)。次候補は監査是正 順9のscope精査。
- **filesystem failureの記録・伝搬（監査是正 順7）**: **完了**（R3、PR #24 squash merge `45da61b`、2026-07-24）。backupの判定変更errorを既存Result境界へ伝搬し、diagnostic logとrestoreの継続可能な個別失敗をcontext付きWARNとして記録する。production / failure-injection testの同一helper経路、X1-X7 mutation、Final Review独立再実測、exact-HEAD local full / hosted final三点一致を完了。Final Review P3×2のsource design test名転記ずれもcloseoutで反映済み。証跡はarchived [Plan Packet](../plans/2026-07-24-filesystem-failure-observability.md) / [Matrix](../plans/test-matrices/2026-07-24-filesystem-failure-observability.md) / [WER](../plans/2026-07-24-filesystem-failure-observability-workflow-effectiveness-review.md)。次候補は監査是正 順8のscope精査。
- **render-phase ref accessのcommit/event同期化（監査是正 順6）**: **完了**（R3、PR #23 squash merge `062f6b8`、2026-07-23）。UI-11cをcommit後state/effect snapshot、UI-05を保存/失敗/reset event同期gateへ置換し、`eslint-plugin-react-hooks@7.1.1` exact + `react-hooks/refs`限定guardを導入。Final ReviewでX3 behavior killの非再現を検出し、実scheduler制約を確認してlint単独防御へ訂正、X3a/X4b専用testを追加した。owner是正確認、exact-HEAD local full / hosted三点一致後にmerge。証跡はarchived [Plan Packet](../plans/2026-07-23-render-phase-ref-sync.md) / [Matrix](../plans/test-matrices/2026-07-23-render-phase-ref-sync.md) / [WER](../plans/2026-07-23-render-phase-ref-sync-workflow-effectiveness-review.md)。次候補は監査是正 順7のscope精査。
- **業務validation所有層一本化 + production CMD test（監査是正 順5）**: **完了**（PR #22 squash merge `26c7cff`、2026-07-23）。商品・棚卸し・整合性validationをBIZ単独所有へ戻し、4 CMD moduleの対象testを実production command + 独立転記exact oracleへ置換。production mutation全件kill、Final Review P1 / P2 / P3 = 0、Ready exact-HEAD local / hosted evidence三点一致で完了。証跡はarchived [Plan Packet](../plans/2026-07-23-validation-ownership-production-cmd-tests.md) / [Matrix](../plans/test-matrices/2026-07-23-validation-ownership-production-cmd-tests.md) / [WER](../plans/2026-07-23-validation-ownership-production-cmd-tests-workflow-effectiveness-review.md)。次候補は監査是正 順6のscope精査。
- **workflow docs WER consolidation**: **完了**（PR #18 squash merge `234a740`、2026-07-21）。WER Adjustment 積み残しを D-050 裁定で DEV_WORKFLOW / template 群へ正本化。packet は `docs/archive/plans/2026-07-21-workflow-docs-wer-consolidation.md` へ archive 済み
- **整合性補正の意味論正本確定 + コード追随（監査是正 順 3、design + 実装完結）**: **完了**（design = PR #19 squash merge `9b7582e` / 実装 follow-up = PR #20 squash merge `f0b4c80`、いずれも 2026-07-22）。D-051 正本化に加え、`fix_integrity` の同一 TX 必須ログ（失敗時 rollback）、UI-13-D9 文言同期、UI-11c-D14 operator-readable 表示、§21.7 の 3 oracle テストを実装。packet は `docs/archive/plans/2026-07-21-integrity-fix-semantics-design.md` / `2026-07-22-integrity-fix-semantics-impl.md` へ archive 済み。
- **mutation→consumer query 契約の SSOT 化（監査是正 順 4）**: **完了**（PR #21 squash merge `187d7a7`、2026-07-23）。全 16 mutation / 18 success handler の invalidation を `invalidation-contract.ts`（D-052 SSOT）へ統一し、監査 P5 系 findings の欠落全件を解消。導出原則と除外表 E1〜E6 は UI_TECH_STACK §2.5 + decision-log D-052 が正本。テストは契約表からの独立転記 oracle と完全一致比較（production 非 import を静的 gate で機械強制）。packet は `docs/archive/plans/2026-07-22-mutation-consumer-query-contract.md` へ archive 済み。次 = 順 5 以降の scope 精査 or roadmap 1-4 移行判断（詳細は Plans.md「次の行動」0）。
...
- **書類整備完了**: Phase 2 UI-00 着手前の natural break で 2 PR 体制で書類同期完了（PR-A #53: SCREEN_DESIGN + PROJECT_HANDOFF / PR-B #54: DEV_SETUP_CHECKLIST.md 書き直し）。プラン archive: [docs/archive/plans/2026-05-08-pre-phase-2-docs-sync.md](../plans/2026-05-08-pre-phase-2-docs-sync.md)
...
**優先度F: バックエンド全層実装 → 全完了**
- [x] F-1〜F-6. v0.1.0-db-layer 〜 v0.6.0（IO/BIZ/CMD/MNT 全層）。詳細は [docs/archive/v0_tag_history.md](../v0_tag_history.md)
...
詳細は [Plans.md](../../../Plans.md) / [docs/archive/plans/2026-05-08-pre-phase-2-docs-sync.md](../plans/2026-05-08-pre-phase-2-docs-sync.md) / [docs/archive/plans/2026-05-09-phase-2-ui-00.md](../plans/2026-05-09-phase-2-ui-00.md) を参照。
...
| CI / 静的検査 | D-033/D-043/D-063 の L0 pre-push / L1 `scripts/local-ci.sh` / L2 hosted final。shared classifier と shell fixtures、`scripts/{doc-consistency-check,check-env-safety}.sh`、lefthook。Node 24.18.0の`.node-version`をfrontend CI / npm security monitorのsingle ownerとし、両workflowの片側driftをstatic testで拒否する。CI workflow は public repository で owner Enable 済み、Draft/Ready/`synchronize` eventをdogfood済み。standard GitHub-hosted runnerにはprivate-era billed-minute thresholdを適用せず、CI-TRIGGER-D1でcompleted HEADごとのReady / dispatch経路を一意化する。D-063 の live-doc drift guard は `ci-workflow.test.sh` を `local-ci.sh full` から実行する L1 local-only で、hosted docs job は未配線。hosted 実防御が必要になった時点で `doc-consistency-check.sh` へ統合する別 R3 を起票する。Actions-unavailable closed route、cache上限監視、required-check未設定は維持。scheduled npm dogfoodは2026-08-03 JST、dependency change Bは2026-08-05以降 |
...
- **workflow の wave 運用（D-055）**: 互いに非干渉な lane は `Plans.md` の Wave Registry で 2〜3 lane を Draft PR まで並列化し、per-lane gateを維持したままmerge trainは直列に進める。wave 1の2 lane pilotに続き、wave 3で初の3 lane（小R2×2 + R3×1）を完走した。file footprint分離・generated artifact lane専有・terminal名固定は成立。次waveのreview発注は`Sonnet 5 / xHigh`とBase / merge-base / Reviewed Content HEAD / State HEADをReview Baseline Bundleとして明示し、state遷移は既存compression rule + 直後workflow-gitを徹底する。詳細は[wave 3 WER](../plans/2026-07-29-wave-3-workflow-effectiveness-review.md)。
...
- POS連携は双方向: 取込み（現店舗の日報主入力は `Z001`/`Z002`/`Z005`、PLU移行後の商品別trackは`Z004`）+ 書出し（システム→CV17 1.1.1向けPLU登録 `.txt`）
- Z004 parser は 2026-07-06 issue #135で確認した`EcrDatas` layout Aと従来shapeの二形状対応済み（SPEC-Z4A-D1〜D7）。Z004自体は個数・金額を持つ確認済みPLU別売上で、BIZ-03在庫増減pipelineも実装済み。固定slot・段階導入・店舗採取shapeでのend-to-end再検証（CSV-09/10）をgo-live前の後続R3で扱う
...
**実現方法（2026-08-01 field evidence sync後の実行順）**: `Z001`/`Z002`/`Z005` の公式日報集計、日次・月次レポート、Z004従来shapeのsale_records/在庫増減/rollbackは実装済み。日報取込みの通常手順は「SD→CV17取込み→PC側`EcrDatas`をアプリで選択」に固定し、初回は手順書、以後は既存の前回フォルダ記憶で案内する。layout A/Bはparser互換性であってoperatorの選択肢ではなく、SDバックアップ・CV17明示書出しは復旧/調査経路とする。現行Excelファイル群は日々ほぼそのまま貼付・上書きされて日別履歴を持たず、印刷・バインダーだけが各日の状態を残す。go-live準備では、(1) 段階導入に耐える固定PLU slot / bulk onboarding、(2) `EcrDatas`の保持・再取込み境界と店舗マニュアル、(3) Z004 layout A/Bと店舗採取shapeでの返品・複数数量・在庫増減end-to-end再検証、(4) 日次/月次画面がExcel印刷・バインダーをoperatorにとって代替できるかの受入、の順序と依存をDesign Phaseで確定する。
...
詳細は [Plans.md](../../../Plans.md) と [docs/archive/v0_tag_history.md](../v0_tag_history.md) に集約済み（個別 PR プランは `docs/archive/plans/` 配下）。要点のみ記述。
...
- プラン: [docs/archive/plans/2026-05-08-pre-phase-2-docs-sync.md](../plans/2026-05-08-pre-phase-2-docs-sync.md)
- Phase 2 8-1 UI-00 ホーム画面実装プラン: [docs/archive/plans/2026-05-09-phase-2-ui-00.md](../plans/2026-05-09-phase-2-ui-00.md)（critic subagent レビューで P1 × 3 / P2 × 8 潰し込み済、ユーザー判断 3 件確定）
