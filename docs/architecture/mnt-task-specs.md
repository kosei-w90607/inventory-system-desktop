...
SPEC-STK-TIME-D1 / D8。新しい業務判断をMNTへ移さない。MNTは計数contextに何も供給せず、OSの通知の登録や時計の検査を持たない（ADR D1）。
...
本番有効化前の外部probeは[32](../function-design/32-biz-csv-import-service.md)に置き、operatorへ手動のDB故障注入を依頼しない。
