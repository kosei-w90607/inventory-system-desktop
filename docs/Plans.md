...
- **㉗ 棚卸しの基準時点を「各商品の最終カウント時点」にする（STK-1 / STK-2、design-first、R2、design〈backtrack〉）**: [Plan Packet](plans/2026-09-16-stocktake-count-baseline.md)。branch `agent/stocktake-count-baseline-design`、Plan Commit pending。監査 STK-1（P1）/ STK-2 / DOC-2 起源。owner の設計判断 3 問（方式 / 同日販売の扱い / STK-2 境界の実装時期）を design → plan-draft の遷移条件として packet に置く。owner 回答 2026-09-16: 方式 = snapshot（candidate）/ STK-2 境界 = 含める（candidate）/ 同日販売 = 店の回答「時間帯関係なし、売れたらその場で訂正」で「カウント前扱い（`<=` スキップ）」に確定。Opus 設計レビュー round 0（P1 1 / P2 8）と Plan Review round 1（Sonnet P2 1 / Opus P2 7、全 accept）を反映済み。評価額は確定時点の数量で合計（D-D7）。round 2〜3 closure 済み（天井到達）。**2026-09-17 owner が Q2 `<=` 一律適用を不承認 → design へ backtrack**。同日 POS 販売の「反映済み」判定（手動 / 電子ジャーナル）を設計中。runtime は後続 ㉘（R3）。wave 12 lane 2、㉖ と file 互いに素。
...
- **wave 12（非干渉並走 2 lane、owner 2026-09-16「次何やるかふたつとって早速始めよう」、lane 選定は Coordinator）: 進行中（lane 1 implementing / lane 2 design〈backtrack〉）** — file footprint 互いに素（lane 1 = `src/features/stock-movements/**` + `src/features/stock-inquiry/components/StockDetailContent*` + 58 / 66、lane 2 = docs-only: 35 / 32 / 42 / 20 / 73 / `db-design/tracking-system-tables.md` / `architecture/biz-task-specs.md` / decision-log / `diagrams/cross-feature-verification.md`）、同じ source document を編集せず、生成 file の再生成なし。D-055 の並列 wave。merge train 順は Draft PR 到達順で owner が指定（既定案 = human-confirm 到達順）。
  - lane 1: ㉖ 在庫変動履歴からの戻りで在庫照会の検索条件と商品選択を保持する（R3、`agent/stock-movements-return-selected`、`plans/2026-09-16-stock-movements-return-selected.md`、Draft PR 未作成、Phase plan-gate、介入 0/3）
  - lane 2: ㉗ 棚卸しの基準時点を「各商品の最終カウント時点」にする（R2 design-first、`agent/stocktake-count-baseline-design`、[Packet](plans/2026-09-16-stocktake-count-baseline.md)、Draft PR 未作成、Phase design〈Q2 不承認で backtrack〉、介入 3/4〈設計判断 3 問 + Q2 の店回答 = 1、Goal 言い直し + 評価額方針 = 2、Q2 不承認 = 3。上限は理由付きで 4〉）
...
- STK-1 / STK-2（[監査](research/2026-09-16-diagram-audit.md) 由来、棚卸し確定が入出庫を打ち消す設計問題と二重減算）は是正方式が owner 判断待ち。2026-09-16 に ㉗（wave 12 lane 2、[Plan Packet](plans/2026-09-16-stocktake-count-baseline.md)）として起票。owner 回答 2026-09-16 で方式（snapshot）と STK-2 境界（含める）は candidate 採用、同日販売は店の回答「時間帯関係なし、売れたらその場で訂正」でカウント前扱い（`<=`）に確定。設問と回答は packet の「owner への設問と回答」節
