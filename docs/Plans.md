...
1. workflowの軽量化1段目のうち文書だけの部分: packet templateへ普通の一日の操作列の節を足す、Writerが止まったときの規則文、問い合わせの行き先の表。**implementing（2026-09-22 起票、R3 / Final Review Minimum 2、Plan Review = Opus 2 round で P1/P2 なし）**: [Plan Packet](plans/2026-09-22-workflow-ordinary-operation-and-stop-routing.md) / [Matrix](plans/test-matrices/2026-09-22-workflow-ordinary-operation-and-stop-routing.md)。branch `agent/workflow-lightening-1a-docs`。次 = Sonnet Writer の実装 → Draft PR → Final Review broad 2 本（Codex + Opus）。
