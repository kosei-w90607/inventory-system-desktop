...
- Initial deployment baseline, owner-confirmed 2026-09-19: outside the register workflow (`SR-S4000` + `CV17` register tool), store operations are paper-based. Register-tool output is currently copied manually; it is not an existing inventory-system integration. This app is a first deployment, and no store production app DB or production app import history (including Z004 product-level / PLU sales) exists as of that date. Existing app DBs are development/demo/test only; real register samples are not proof of an app production DB. Owner's original words:「今のうちの店にはレジのシステム以上のものも存在しないし、レジは商売の中の取引以外できないし、レジスターツールだって今コピペしてる。このアプリはほぼゼロからのスタート」。At production launch, append the actual launch date and changed assumptions here; do not continue treating production history as absent after launch. This baseline is not permission to delete development/test data.
