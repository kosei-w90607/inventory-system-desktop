...
- 廃番・取引先消滅の印は在庫照会では無くても済むが、棚卸しリストでは要る（原文「いる気がする」。廃棄の判断基準になるため）。
...
- **(d-3)** 取引先消滅は data が無く、`master-tables.md:178`「シンプルに name 一つだけ」の最小設計のため状態列を持たず、追加は同設計意図に反する（同 doc が明示的に拒否するのは問屋テーブル・種別カラムであり、状態列そのものを名指しでは拒否していない — 最小設計の原則からの推論）ため、「該当商品を廃番に更新する運用」で代替する。起票なし。
