...
| M3 R3 | 進行中の確定が未解消の flag を素通りする。最新の実測が確定済みの棚卸し・独立再実測に属する商品の flag を、新しい棚卸しの未計数の明細の force_fill や廃番の auto_filled で素通りする（Gated Amendment 5、Codex broad〈GA4 後〉#1） | model / Contract Audit | `check_unknown_apply_recheck`、packet AC5 (m) の check、ADR D4 と 32 / 35 / tracking の読取り | 数え直し前の確定が通る、明細の kind（未計数・auto_filled・measured）や最新の実測の所属で確定の拒否が外れる。確定の拒否を旧条件（最新の実測が進行中の棚卸しに属する商品だけ）へ戻す mutation (xxii) で red |
| M3 R3 | 所属で保留へ分ける旧規則が残る | docs search | packet AC3 の検索 | ADR :132 相当か 32 の step 5 相当の文が残る |
| M3 R3 | 共有 JAN 行の保留が消える、非連動化の記録を残す理由が消える | Contract Audit | ADR D4 と 32 / 55 の読取り | 共有 JAN 行の file 全体保留、または `pos_sync_disabled_revision` の理由の記述が欠ける。保留の条件を在庫連動の候補の数で狭め、JAN の候補商品が複数で少なくとも 1 つが在庫連動対象の行（非連動商品との共有を含む）を保留から外す（Gated Amendment 4、Codex broad #3） |
...
| M9 R9 | 最初の区間の登録の変化が受領順に凍結し直前の Z004 を後から取り込んでも解消しない、前回の Z004 が未受領のまま帰属できない行を判定する、settlement_no の戻りで戻る前の Z004 を前回とみなす、EJ の先頭欠けを前回を証明できないとする、clear 行で消えた code の商品を判定から落とす、再評価で行の有無を帰属できない行より先に見る（Gated Amendment 3 の補足 3、相談役 F1〜F6） | 合成モデル AC5 (i)、Contract Audit | R9 の「前回」・行の分類 (4)・区間全体の確認待ち・完全な EJ と ADR D4 / D5 の照合 | 次の mutation で red: 最初の区間の登録の変化を直前の Z004 を使う業務 TX で再評価しない (xiv)、前回の Z004 が未受領の間に帰属できない行を判定する (xv)、settlement_no の戻りを未受領として扱う (xvi)、帰属できない行の判定を行の有無の判定より後にする (xvii)。F4（開始の証拠が無い区間が EJ待ちになる）と F5（前回にはある code の商品を 0 の行として EJ で判定し、その code を含む単位の合計を照合しない）は assert だけで確かめる |
| M9 R9 | 受領済みの最小より小さい番号・番号の戻りの区間の登録の変化が受領の順で固定され、直前の Z004 を後から取り込んでも解消しない（10 → 計数 → 9 → 8 の逆順）、前回の code が分からない区間（直前の settlement_no の Z004 を受領していない区間。最初の区間・受領済みの最小より小さい番号・前回の未受領・番号の戻り）で clear された数えた在庫連動商品を確認待ちの対象から落とし、前回の受領後も相殺を見逃す、再評価で証拠の矛盾による登録の変化まで解消する（Gated Amendment 4、Codex broad #1 / #2。語と (k') は Gated Amendment 5、Opus closure N1 / N2） | 合成モデル AC5 (j)(k)(k')(l)、Contract Audit | R9 の登録の変化・区間全体の確認待ちの対象と ADR D4 / D5 の照合 | 次の mutation で red: 登録の変化の再評価を最初の区間だけに戻す (xx)、前回の code が分からない区間で今回の Z004 に行のない数えた在庫連動商品を確認待ちの対象から外す (xxi)、その対象の選び方を前回の未受領の区間だけに残す (xxiii)、印の付いた登録の変化を対象外になっても解消しない (xxiv。安全側の誤りで、red にならなければ Writer が報告)。(k) は前回の未受領、(k') は最初の区間と受領済みの最小より小さい番号の区間を検査し、番号の戻りの区間は (j) の再評価だけで確かめる（D3 の同一性 guard により ㉘ では到達しない）。帰属できない行・開始の証拠の不一致による登録の変化が再評価の後も残ること（凍結）は (l) の assert で確かめる |
| M9 R9 | 数量 0 で金額が 0 でない行を EJ 待ちにする、または見落とす | model | 同 check の (e) | その行が相殺の行ありにならない |
| M9 R9 | 後から取り込んだ EJ で確認待ちを再評価しない、行ありを見落とす | Contract Audit | ADR D4 / D5 と 32 の読取り | EJ待ちが数え直しでしか解消しない、行ありの EJ で解消してしまう、または登録の変化が EJ の再取込みで解消する、直前の Z004 が未受領の間の登録の変化（最初の区間・受領済みの最小より小さい番号・番号の戻り）が直前の Z004 を使う業務 TX で再評価されない |
| M9 R9 | 受領だけで flag を変える、受領の後の中止・業務 TX の失敗で再評価の機会を失う、再起動・同 hash の再選択で再試行しない、確定していない印を失う、凍結した登録の変化を再び開く（Gated Amendment 6、Codex broad〈GA5 後〉#1） | 合成モデル AC5 (n)、Contract Audit | `check_receipt_business_retry`、既存の `check_unknown_apply_recheck`、ADR D4 / D5・32・tracking・pos・55 の照合 | 受領・中止・拒否・失敗の後は業務状態が変わらず、同 hash は最初の受領 ID を保ち、再試行の成功で D4 の結果を確定する。次の mutation で red: 受領で再評価する (xxv)、業務 TX の再評価を落とす (xxvi)、失敗した業務 TX の変更を残す (xxvii)、結果が変わらない再評価で revision を進める (xxviii)。実測・取消で解消した flag は再評価で復活せず、再評価で凍結した登録の変化は EJ の再取込みで開かない |
...
| 要再確認 flag（(商品, 資料)、理由 5 値、内部の直前資料待ちの印） | なし | 直前資料待ちの登録の変化は印を持つ。資料を受領しただけでは変えない | 作成・再評価の結果・印と必要な revision を取込みの業務 TX で確定 | 当該資料を開始前に受領した新しい実測の保存、当該 import の取消。EJ待ち（前回の Z004 が未受領なだけの区間を含む）と印の付いた登録の変化は、R9 の業務 TX による再評価（帰属できない行と前回の証明を先に判定）。ほかの登録の変化と再評価で凍結したものは数え直しで解消。force_fill では解消しない | 準備照会・棚卸し一覧。照会は変えない | 未解消の flag と印を保存状態から取り直す | flag・印・作成 import の参照を保つ。旧 preview は復元しない | 業務 TX 全体を戻し、独立の受領記録と従前の flag・印を残す | 同じ file の再選択で最初の受領 ID へつなぎ業務 TX を再試行、または数え直し。成功済みの active hash は重複拒否 | M3 / M9、`check_receipt_business_retry` |
...
- 確定の拒否を最新の実測が進行中の棚卸しに属する商品だけに戻すと、どの assert が落ちるか: packet AC5 (m) の check（確定済みの棚卸し・独立再実測を基準に持つ商品の未計数・auto_filled の明細で確定が通る）。
- 取消で flag を消さないと、どの assert が落ちるか: 同 check の取消後の flag 不在。
- 再評価を受領で行う・業務 TX の再評価を落とす・失敗した業務 TX の変更を残す・結果が変わらない再評価で revision を進めると、どの assert が落ちるか: `check_receipt_business_retry` の `receipt mutated business state` / `same-hash retry missed reevaluation` / `failed TX leaked business state` / `reevaluation was not idempotent`（packet AC5 (n) の (xxv)〜(xxviii)）。
