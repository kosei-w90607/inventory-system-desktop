...
| M9 R9 | 区間の途中で名称が入れ替わった行を別の商品へ帰属させる、または価格だけ・廃番・名称変更の PLU 書出しで区間全体を不完全にして動きのない数えた商品を全て確認待ちにする（Gated Amendment 1 / 2） | 合成モデル AC5 (f)、Contract Audit | R9 の変わった名称の集合と区間の EJ の行の照合、「前回」の決め方、ADR D5 | 変わった名称の行があっても区間を完全とみなす（mutation (vi)）、または変わった名称があるだけで行の有無を問わず不完全とする（mutation (vii)） |
