...
| M9 R9 | 区間の途中で名称が入れ替わった行を別の商品へ帰属させる、または価格だけの PLU 書出しで区間を不完全にして動きのない数えた商品を全て確認待ちにする（Gated Amendment 1） | 合成モデル AC5 (f)、Contract Audit | R9 の Z004 どうしのスロットと名称の対応の比較と ADR D5 の照合 | 対応が変わっても区間を完全とみなす（mutation (vi)）、または価格だけの変更・新しい名称の追加でも不完全とする |
