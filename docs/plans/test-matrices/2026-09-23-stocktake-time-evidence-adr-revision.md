...
| M9 R9 | 受領済みの最小より小さい番号・番号の戻りの区間の登録の変化が受領の順で固定され、直前の Z004 を後から取り込んでも解消しない（10 → 計数 → 9 → 8 の逆順）、前回の Z004 が未受領の間に clear された数えた在庫連動商品を確認待ちの対象から落とし、前回の受領後も相殺を見逃す、再評価で証拠の矛盾による登録の変化まで解消する（Gated Amendment 4、Codex broad #1 / #2） | 合成モデル AC5 (j)(k)(l)、Contract Audit | R9 の登録の変化・区間全体の確認待ちの対象と ADR D4 / D5 の照合 | 次の mutation で red: 登録の変化の再評価を最初の区間だけに戻す (xx)、前回の Z004 が未受領の区間で今回の Z004 に行のない数えた在庫連動商品を確認待ちの対象から外す (xxi)。帰属できない行・開始の証拠の不一致による登録の変化が再評価の後も残ること（凍結）は (l) の assert で確かめる |
