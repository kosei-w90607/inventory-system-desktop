...
- FM18: 復元を始めた後に確認が呼ばれる、または復元の前に発火して待機中だった確認の結果が停止の後に toast・invalidate になる
...
| C4 | FM7 | unit（file の bytes） | T8 (i) `db::migration::tests::test_init_database_req903_d11_newer_schema_leaves_file_bytes_unchanged`: T6 と同じ DB を閉じて SHA-256 → `init_database` → `Err(SchemaNewerThanApp)` → drop → SHA-256 が同じ、tempdir の file の集合が同じ（Probe S3-b の形） | 拒否より前に書く処理がある（PRAGMA の順を変えた等） |
...
| C9 | FM14 / FM16 | integration（hook を mount する wrapper + `BackupRestorePage`、既存 page test の fake timer と mock） | T18 `BackupRestorePage.test.tsx` の既存 「QR-05 REQ-901 stops auto backup checks after double failure」を、`useAutoBackupCheck()` を呼ぶ wrapper と page を一緒に描く形に直す: (a) 二重失敗（`restore_failed_unrecoverable` と `restore_durability_unknown` の各々）の表示の後 60 秒進めても呼出し回数が増えない。(b) 同 file の新しい test「復元の前に発火して待機中の確認の結果は捨てる」: `checkAutoBackup` を未解決の deferred にして 60 秒で発火させ、`restoreBackup` も deferred にして復元を始める → 復元の間に 120 秒進めても `checkAutoBackup` の呼出しが増えない → 待機中の確認を `true` で解決しても `toast.success` と一覧の invalidate が 0 → 最後に両方の deferred を settle する。(c) 同 file の新しい test「fatal でない結果の後は再開する」: 復元が成功、`restore_failed_recovered` の Err、IPC の例外（reject）の各場合で、結果の後 60 秒で `checkAutoBackup` が再び呼ばれる。wrapper なしの page 単体では timer が無く assertion が自明に通るため、wrapper を必須とする | page が `suspendAutoBackupCheck()` を呼ばない / 復元の後にしか止めない / 停止の後に届いた結果を捨てない / fatal でない結果で再開しない / fatal 2 種で再開する |
...
| 復元の間の停止（S4） | 未停止 | 復元の前に suspend、停止の後に届いた結果は捨てる | 成功 → resume | — | — | 二重失敗の後は画面を移っても停止のまま | 再起動（reload）で解除 | fatal 2 種 → 停止のまま / fatal でない Err・IPC の例外 → resume | 復元をやり直せる | T14 / T18 |
...
| in-memory の module flag（UI-11b-D11 の形） | `src/lib/restore-success-notification.ts` | `suspendAutoBackupCheck` / `resumeAutoBackupCheck`（test の間の独立も本番 API の resume で取る） | test 専用の reset の export | T14 / T18 |
...
- If a guard is removed, which test fails? 上限の判定 → T1、commit の再検証 → T2、版の比較 → T6 / T8、suspend → T14 / T18、連続失敗の判定 → T13、実行中の guard → T19、停止の後に届いた結果の破棄 → T18 (b)
