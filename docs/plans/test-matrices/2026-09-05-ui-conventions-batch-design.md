...
| 全体 | catalog:588 冒頭矛盾文の残存 | doc-oracle | `rg -Fc "検索 \`Input\` は両モードとも" docs/design-system/02-component-catalog.md` = 0（Plan Review round 3 是正 — bare "両モードとも" は `:547,581,584` の正当な用法にも一致し unsatisfiable だったため、`:588` 冒頭の一意な文言へ anchor） | SearchBar アクセシビリティ節冒頭の「両モードとも aria-label」矛盾文が残っている場合に検出 |
...
| UICONV-D6 | live 型 Label 反転の欠落・commit 型巻き込み | doc-oracle | `rg -Fc "live 型は可視" docs/design-system/02-component-catalog.md` ≥ 1、`rg -Fc "live 型は可視 Label を持たない設計" docs/design-system/02-component-catalog.md` = 0 | 反転漏れ、または旧文言が残った場合に検出 |
| UICONV-D6 | aria-label 廃止が runtime lane へ先送りされる | doc-oracle | `rg -Fc "aria-label は持たない" docs/design-system/02-component-catalog.md` ≥ 1 | catalog 本文で確定せず先送り文言のままの場合に検出 |
...
| 全体 | `src/**` 混入 | repo-oracle | `git diff --name-only 07302b5..HEAD -- src` の出力 0 行（`echo $?` で exit code も記録。パイプなし、AC14 と同じ非パイプ pathspec 絞り込み、Plan Review round 3 是正 — round 2 の修正は「非パイプ」と称しつつ `| wc -l` を残しており未達だった） | runtime file が 1 件でも混入した場合に検出 |
