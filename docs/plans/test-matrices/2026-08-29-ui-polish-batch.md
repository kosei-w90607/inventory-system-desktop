...
| 取消成功通知の視認性 | duration 延長が消える | RTL | T7 = 取消完了 toast 2 箇所（csv / 日報）の `duration: 8000` を両 hook test で実 assert（gated Amendment 2 で確定。文言・Toaster 全体設定は不変） | duration 指定の除去・既定値化 |
| 〈除外〉先頭スクロール（gated Amendment 2） | — | — | T8 は除外（実装 revert に伴い test も撤去。scroll 復元方針は design-first 裁定候補） | — |
...
| 取消 flow の通知 | 確認 dialog（構造化 summary、T2） | 取消中 | toast（duration 8000ms、T7） | 既存 root() 契約不変 | 一覧再取得（既存） | — | — | 既存 error 経路不変 | — | Matrix |
...
| 先頭スクロール | SCREEN_DESIGN 既存 4 画面 | 〈除外 — gated Amendment 2〉 | mount 一律は詳細戻り UX を壊すため revert、design-first 裁定候補へ | — |
