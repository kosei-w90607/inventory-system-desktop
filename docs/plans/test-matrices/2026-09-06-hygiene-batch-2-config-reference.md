...
| SC4 既存 block 無変更（S2） | 既存 block に誤って手を加える | static（block diff） | `diff <(git show 07302b5:eslint.config.js \| bat --plain --line-range 79:97) <(bat --plain --line-range 79:97 eslint.config.js)` = 空（exit 0）。base `07302b5` の既存 block（`files` 行〜閉じ `},`）とテキスト全体を逐語比較する（Codex 実測 2026-09-07: `rg -Fc` の出現数チェックは新規 block へ `"src/features/**/*.{ts,tsx}"` を混入させても 1 のまま変化せず検出漏れ） | 既存 block のテキストが base から 1 文字でも変わる（`files`/`ignores`/selector/button selector のいずれか） |
| SC5 新規 block の静的構造（S2） | 新規 block が無い、既存 block と `files` が重複する、または `ignores` が無い | static（`rg -Fx` 配列完全一致） | 既存・新規 2 block の `files`/`ignores` 行を期待 literal 配列と完全一致で確認（4 assertion、各 = 1）: `rg -Fx '    files: ["src/features/**/*.{ts,tsx}", "src/components/patterns/**/*.{ts,tsx}"],' eslint.config.js` / `rg -Fx '    ignores: ["src/features/**/*.test.{ts,tsx}", "src/components/patterns/**/*.test.{ts,tsx}"],' eslint.config.js` / `rg -Fx '    files: ["src/components/ui/**/*.{ts,tsx}", "src/components/layout/**/*.{ts,tsx}"],' eslint.config.js` / `rg -Fx '    ignores: ["src/components/ui/**/*.test.{ts,tsx}", "src/components/layout/**/*.test.{ts,tsx}"],' eslint.config.js`（`-Fx` は行全体の literal 完全一致のため要素の追加・削除は即 0 件になる） | いずれかの block の `files`/`ignores` 行が期待の 2 要素ちょうどの配列と完全一致しない——新規 block の `files` に `"src/features/**/*.{ts,tsx}"` を追加する mutation（Codex 実測 2026-09-07、旧 `rg -Fc` 出現数チェックでは 4 件とも 1 のまま通過していた）を含め、要素の追加・削除・別 block への混入をすべて検出する |
