...
| SC2 単一化の実効性（S1） | inline options 残存 | Writer probe（mutation、**`$TMPDIR` 配下の独立した使い捨てコピー A・B で**、両方に同じ `tsr.config.json`〈`target: "solid"`〉を配置） | コピー A で `npm run generate:routes`（CLI のみ）、コピー B で `npx vite build`（plugin のみ）を実行し、両コピーの `src/routeTree.gen.ts` を `diff` で比較。是正が正しければ両者は揃って変化し byte-identical のまま。**コピー B の `vite build` プロセス自体は `@tanstack/solid-router` が node_modules に無いため routeTree 書き出し後に non-zero exit するが、これは期待された後段failureであり oracle に含めない**（`diff` のみが oracle）。probe 後に両コピーを破棄し、worktree 側は `git status --short -- src/routes` = 空で無傷を確認 | `diff` 不一致（`vite.config.ts` に inline `target: "react"` が残っていて plugin 側だけ変化せず両者が乖離する）。**1 つのコピー内で CLI→plugin を順に実行するのは誤り**——leg 1 の書き込みが leg 2 を `routeTree.gen.ts` 生成前に `SyntaxError` で失敗させ比較不能になる（Plan Review round 2 Opus P1 で発見・実測確認） |
