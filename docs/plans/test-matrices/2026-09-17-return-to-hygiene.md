...
|---|---|---|---|---|
| C1 | FM1 / FM2 | unit（`it.each` 追加） | T1 `src/lib/return-to.test.ts` 「normalizeReturnTo (REQ-207 / DSR-15 / DSR-18 …)」に `"/\\evil.example"` → fallback、`"/\t/evil.example"` → fallback、`"/ok#frag"` → `/ok`、`"/inventory/records?q=%22123%22&page=2"` → 同値、を追加。既存 8 case は期待値不変 | origin 判定が無い（旧 `startsWith`）/ 正当な query を落とす / hash を残す |
...
| C2 / C3 | FM3 | unit（実 router、`renderWithClient`） | T3 `OtherRecordDetailPages.test.tsx` の既存 T11 系に、検索条件つき `returnTo` の case を 1 つ追加し、描画 href を `defaultParseSearch` した結果が元の search と deep-equal であることを確認 | 分解で条件が欠ける / 再直列化で型が変わる |
...
| C1〜C5 | FM2 / FM8 | integration（実 routeTree + memory history、`ReturnToFlow.test.tsx` の harness） | T8 `ReturnToFlow.test.tsx` 「REQ-207 / REQ-303: 入出庫履歴（数字だけの検索語）→ 詳細 → 前の画面へ戻る で検索語が残る」+ 既存の 3 段往復 case が pass。T2-num（`StockMovementsPage.test.tsx:419`）は `stockInquirySearchSchema.parse(defaultParseSearch(url.search))` まで通す形へ強化 | 入れ子 `returnTo` を guard が落とす / 戻り先で schema に落ちる |
