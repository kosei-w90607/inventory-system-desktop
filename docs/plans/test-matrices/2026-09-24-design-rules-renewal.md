...
| D11 | F13 | CLI | T13: 題ごとの loop。`for t in 'デザインの決まり runtime lane A（色と強調）' 'デザインの決まり runtime lane B（書体）' '棚卸し画面 D1 の実装'; do rg -n -F "$t" docs/backlog.md; done` で各題の行を取る。lane A は行から対象の部分を `rg -o '対象 = .*?。AC = '` で切り出し、その部分に `--ring`・`defaultVariants`・`ActionButton`・`progress`・`spinner`・`alert.tsx`・`table.tsx`・`StepIndicator`・`ProductListTable` を `rg -F` で確かめ、AC の部分を `rg -o 'AC = .*?。L3 項目'` で切り出し、その部分に `variant="destructive"`・`data-\[state=selected\]`・`has-aria-expanded` を確かめ（対象の部分にも同じ語があるため、行全体では AC の rg の並びから語が抜けても通る。Final Review broad〈GA1 後〉Opus N5）、行全体に `着手条件`・`1 PR`・`origin/main`・`L3` を確かめる（GA1）。lane B は行全体に `着手条件`・`D-030`・`L3`、D1 は `着手条件`・`lane A` | どれかの題が 0 行か 2 行以上、lane A の行に「対象 =」から「。AC =」までの部分か「AC =」から「。L3 項目」までの部分が 1 つに切り出せない、またはその部分・行に必須語のどれかが無い |
