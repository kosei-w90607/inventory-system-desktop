...
- UIB3-D3: `02-component-catalog.md` ③「直近実績サマリテーブル」に備考列規則（必須・空欄表示 owner culling・truncate+title）と A1(a)(b)(c) 統合（文言統一・列見出し・囲み是正）を追記する。
- UIB3-D4: `ManualSalePage.tsx:739` の記録状態を既存 5 箇所と揃える runtime gap を記録する（design 側の記述変更なし）。
- UIB3-D5: `SupplierManagementPage.tsx:35-38` の間隔 runtime gap と `PageHeader.tsx` の `actions`/`subtitle` 排他という component 制約を記録する（catalog ① の使用パターンで当座対応）。
...
| UIB3-D1 | PLU書出し説明文が enabling flow に触れていない | doc-oracle | `rg -Fc "レジ登録状況を読み込む" docs/function-design/67-ui-plu-export.md` ≥ 1、`rg -Fc "未反映から外す" docs/function-design/67-ui-plu-export.md` ≥ 1 | 一連の流れ（読込み→書出し→確定）が説明文に含まれない場合に検出 |
...
| UIB3-D3 | 備考規則・A1統合の欠落 | doc-oracle | `rg -Fc "備考" docs/design-system/02-component-catalog.md` ≥ 1、`rg -Fc "truncate" docs/design-system/02-component-catalog.md` ≥ 1 | 備考列規則が catalog ③ に追加されていない場合に検出 |
...
| UIB3-D5 | PageHeader component gap の記録漏れ | doc-oracle | `rg -Fc "actions" docs/design-system/02-component-catalog.md` の該当段落に `subtitle` との排他関係が reviewer 実読で確認できること | component gap が記録されず使用パターンだけが書かれた場合に検出（automated では検出困難、Residual Test Gaps 参照） |
...
|---|---|---|---|---|
| 備考の空欄表示（`hasNote`/`formatNote`） | `ReceivingPage.tsx` / `ReturnExchangePage.tsx` / `ManualSalePage.tsx` / `DisposalPage.tsx` の note 関連コード全件 | `ReturnExchangePage.tsx:103-110,963-969`（既に薄字「備考なし」実装済み、canonical 候補） | `ReceivingPage.tsx:689`（素の空文字、runtime gap）、`ManualSalePage.tsx:715-745`（備考列自体が無い、runtime gap）、`DisposalPage.tsx`（note フィールド無し、対象外） | 起票時実測（本 packet「起票時実測」節）、runtime lane で `rg` 再実行し是正状況を確認 |
...
- If a key branch is inverted, which test fails? — 囲み是正方向を「他 3 画面に箱を足す」へ逆転させたら、AC5 の rg オラクル（「備考」「truncate」の存在）は通ったままになる → reviewer 実読で DSR-16 整合性を確認する（Residual Test Gaps 参照）。
...
- If an output field is omitted, which test fails? — 説明文 3 案の表から 1 画面分の行が丸ごと欠落したら AC2（PLU 書出しのみ検証）は通るが、商品一括インポート・バックアップの欠落は検出できない → reviewer 実読で 3 画面分の文案が揃っていることを確認する（Residual Test Gaps 参照）。
...
- 説明文 3 案が 3 画面すべてに揃っているか（1 画面分の欠落）は AC2 が PLU 書出しのみを検証するため、他 2 画面の欠落は reviewer 実読に依存する。
