...
| UIB3-D6 | 単位表示ルールの欠落 | doc-oracle | `awk '/^## 更新履歴/{exit}{print}' docs/design-system/02-component-catalog.md \| rg -Fc "formatStockDisplay"` ≥ 1（更新履歴表の同一文言に釣られないよう本文範囲に限定） | 共通 formatter 使用ルールが catalog ③ に追加されていない場合に検出 |
