...
| SC14a S27 presence: `在庫整合性検証` / `整合性チェック実行` / `補正する` | `for w in 在庫整合性検証 整合性チェック実行 補正する; do rg -c -F -- "$w" docs/design-system/reference/mockup-d-home-sales-admin.html; done` | 1 / 1 / 3（見出し行の説明文 + dialog 警告文「補正すると」1 行、checkbox aria-label 1 行、note 1 行） | green |
