...
- Gated Amendment 7: SC15 = `--control-surface: #fafaf9`（S46）。SC8 / SC10 = 帯に `border-b` / `border-border` なし（S47）、root に `[&_thead_tr]:bg-list-head` あり（S48）。SC12 lists = `.pager.top` に `border-bottom` なし。SC17（S48、fs literal + DOM）= globals の corner mask 2 rule（`radial-gradient` + `var(--background)`、`@layer` 外）と root の `list-shell-sticky`（sticky 時のみ）
...
| SC8 sticky 帯 surface（Gated Amendment 5 S39） | 帯に `bg-list-head` / `bg-muted` / `forced-colors:border-b` 残存 / thead の `bg-list-head` 欠落 | unit | SC8: ListShell with stickyHeader applies bg-background and border-b border-border to the summary band while keeping bg-list-head on thead th and thead tr, and the band carries no bg-list-head, bg-muted or forced-colors:border-b | 帯の classList に `bg-background` / `border-b` / `border-border` のいずれかが無い、帯に `bg-list-head` / `bg-muted` / `forced-colors:border-b` のいずれかが残る、または thead th / thead tr のいずれかから `bg-list-head` が消える |
...
| SC10 帯の隣接 + inset + 横追随 + ellipsis + 下線（Gated Amendment 5 S39 で forced-colors 限定を解除） | 帯と table の間に page 地 / 左基準線ずれ / root の `space-y-3` 消失 / wrapper 幅が table に追随しない / hard clip / 帯下線消失 | unit（`ListShell.test.tsx`） | SC10: ListShell with stickyHeader and topSummary renders the summary band with px-2, overflow-hidden, child truncate and border-b border-border inside a wrapper whose className is exactly w-min min-w-full and is not the root, the root keeps space-y-3, and the band's next sibling is the table container | 帯の classList に `px-2` / `overflow-hidden` / `[&>div]:min-w-0` / `[&>div]:truncate` / `border-b` / `border-border` のいずれかが無い、裸の `truncate` が残る、`forced-colors:border-b` が残る、wrapper の `className` が `w-min min-w-full` と不一致、wrapper が `container.firstElementChild` と同一、root の classList に `space-y-3` が無い、または `nextElementSibling` が `[data-slot=table-container]` でない |
...
| SC12 mockup / catalog / 文言表 同期（Gated Amendment 5 S39 で lists の帯色を追加） | mockup 帯なし / catalog 字面 stale / 文言表 未登録 / 3 file 未帯化 / history 上部文言 / 同一明度 stale / lists の帯が旧 `--d-head` のまま | doc oracle（rg、presence + absence） | SC12: mockup-d-lists nests `.pager.top` inside `.tbl` as the Lane 2 pilot with `background:var(--bg)` and a 1px `border-bottom`, no longer `--d-head`（`rg -c -F 'class="pager top"' mockup-d-lists.html` = 1、`rg -c '\.pager\.top\{[^}]*--d-head' mockup-d-lists.html` = 0、`rg -c '\.pager\.top\{[^}]*border-bottom' mockup-d-lists.html` = 1）; history, home-sales-admin and import-export no longer render `.pager.top`（Amendment 4 closure round 1 で除去、`for f in mockup-d-forms-b mockup-d-history mockup-d-home-sales-admin mockup-d-import-export; do rg -c -F 'class="pager top"' $f.html; done` = 0/0/0/0）; history has no 件目を表示; catalog ⑯ item 3 carries the new wording（`灰色面は列見出しのみ`）and item 1 says 3 段; the wording table registers the caption and the new dialog title | S15 Spec の 7 本、S18 Spec の 3 file × 3 本 + history 1 本、S19 Spec の 2 file × 2 本 のいずれかが期待値と異なる、3 file の `class="pager top"` が 1 以上、または lists の `.pager.top` に `--d-head` が残る／`border-bottom` が無い |
...
| SC14a S28 absence: `.pager.top`（forms-b / history / home-sales-admin / import-export、`mockup-d-lists.html` の pilot は対象外） | `for f in mockup-d-forms-b mockup-d-history mockup-d-home-sales-admin mockup-d-import-export; do rg -c -F 'class="pager top"' docs/design-system/reference/$f.html \|\| echo 0; done` | 0 / 0 / 0 / 0 | green |
...
| SC14a S31 absence: `結果（前回の取込み）` / `memory No.`（import-export） | `for w in '結果（前回の取込み）' 'memory No.'; do rg -n -F -- "$w" docs/design-system/reference/mockup-d-import-export.html \| rg -v 'class="note"' \| wc -l; done` | 0 / 0 | green |
...
| X33 | ListShell の summary 帯に `border-b border-border` を追加（Gated Amendment 7 S47 で書き換え、旧 = `border-b` 削除） | SC8 / SC10 |
...
| X36 | ListShell root から `[&_thead_tr]:bg-list-head` を削除（Gated Amendment 7 S48 で書き換え、旧 = 復活） | SC4d / SC8 / SC17 |
...
| X40 | ListShell root（stickyHeader 時）から `list-shell-sticky` を削除 | SC17 |
| X41 | `globals.css` の corner mask 2 rule（`.list-shell-sticky thead th:first-child::before` / `th:last-child::after`）を削除 | SC17 |
