...
| SC5 自動展開の条件単位制限（UI-06a-D5） | guard 誤動作 | unit（`useStockInquiry.test.tsx` 拡張 + `StockInquiryPage.test.tsx` 既存 test 再検証） | SC5a: 検索条件が同一のまま `listItems.length === 1` で `selected` が `null` → `undefined`（手動クローズ相当）に変化しても `navigate` が再度呼ばれない。SC5b: `status`/`q`/`dept`/`page` のいずれかが変化した後、新しい条件で `listItems.length === 1` かつ `selected === null` なら `navigate` が呼ばれる（既存の自動展開が生きていることの回帰確認）。SC5c（Plan Review round 1 P2 追加）: `StockInquiryPage.test.tsx:135` の page-level 自動展開 test、`:264-270` の行クリック展開 test が guard 追加後も無変更で pass する。`:385` 付近のコメント「単一件自動展開の selected 更新 call も混ざる」前提を guard 追加後に再検証し、成立しなくなった場合は test 側のコメント・assertion を実態に合わせて更新する | SC5a: 手動クローズ後に同条件で自動再展開してしまう。SC5b: 条件変化後の自動展開が壊れる（既存 test `REQ-301: 結果 1 件で詳細カード自動展開` の回帰）。SC5c: `StockInquiryPage.test.tsx` の既存 3 箇所のいずれかが guard 追加で意図せず壊れる、または前提コメントが実態と乖離したまま放置される |
