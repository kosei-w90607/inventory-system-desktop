...
- SC2（UI-06a-D4）: `ProductListTable.tsx` に取引先列（商品コード/商品名/部門/**取引先**/状態/在庫数/売価の 7 列）が追加され、`supplier_name` が `null` のとき `—` を表示する。`ProductListTable` は `source` で条件分岐しない単一 component（`StockInquiryPage.tsx:235`、Lane 4 merge 後再実測・旧 `:224`）のため、`source="search"`（「すべて」）でも同じ列構成になる（Plan Review round 1 P2）
...
| SC3 colSpan（UI-06a-D4） | DOM 構造崩れ | unit（`ProductListTable.test.tsx` 拡張、fs literal） | SC3: `rg -Fc 'colSpan={7}' src/features/stock-inquiry/components/ProductListTable.tsx` = 1 かつ `rg -Fc 'colSpan={6}' src/features/stock-inquiry/components/ProductListTable.tsx` = 0（static oracle）。加えて展開行 `<TableCell colSpan={7}>` が実際に render されることを RTL で確認。**既存 test 更新必須（Plan Review round 1 P1）**: `:127` の `it("REQ-301: 選択行の nextElementSibling が colSpan 展開行（td[colspan=6]、旧下部固定の混入 guard）")` の it 名と `:141` `td[colspan="6"]` を `colspan=7`/`td[colspan="7"]` へ更新する（この test は SC8 の「無変更で pass」対象から除外する） | `colSpan` が 6 のまま残る、または 7 への更新が別箇所に漏れる。`:127` の test が更新されず `colspan="6"` を期待したまま残る |
...
| SC5 自動展開の条件単位制限（UI-06a-D5） | guard 誤動作 | unit（`useStockInquiry.test.tsx` 拡張 + `StockInquiryPage.test.tsx` 既存 test 再検証） | SC5a: 検索条件が同一のまま `listItems.length === 1` で `selected` が `null` → `undefined`（手動クローズ相当）に変化しても `navigate` が再度呼ばれない。SC5b: `status`/`q`/`dept`/`page` のいずれかが変化した後、新しい条件で `listItems.length === 1` かつ `selected === null` なら `navigate` が呼ばれる（既存の自動展開が生きていることの回帰確認）。SC5c（Plan Review round 1 P2 追加、行番号は Lane 4 merge 後再実測済み）: `StockInquiryPage.test.tsx:175`（旧 `:135`）の page-level 自動展開 test、`:305-311`（旧 `:264-270`）の行クリック展開 test が guard 追加後も無変更で pass する。`:425`（旧 `:385`）付近のコメント「単一件自動展開の selected 更新 call も混ざる」前提を guard 追加後に再検証し、成立しなくなった場合は test 側のコメント・assertion を実態に合わせて更新する | SC5a: 手動クローズ後に同条件で自動再展開してしまう。SC5b: 条件変化後の自動展開が壊れる（既存 test `REQ-301: 結果 1 件で詳細カード自動展開` の回帰）。SC5c: `StockInquiryPage.test.tsx` の既存 3 箇所のいずれかが guard 追加で意図せず壊れる、または前提コメントが実態と乖離したまま放置される |
| SC6 `StocktakeItemDetail.is_discontinued`（UI-10-D13） | 片方の経路だけ反映 / 既定値誤魔化し | unit（`stocktake_repo.rs` `#[cfg(test)]` 拡張） | SC6a: `test_list_stocktake_items_req205_includes_is_discontinued` — 廃番商品 1 件 + 非廃番商品 1 件を fixture に含む棚卸しで `list_stocktake_items` を呼び、返り値の `is_discontinued` が商品ごとの実値と一致する。SC6b: `test_find_stocktake_item_by_code_req205_includes_is_discontinued` — 同様に `find_stocktake_item_by_code` で廃番商品を検索し `is_discontinued: true` が返る。**fs literal oracle（Plan Review round 1 P1 是正、旧 `rg -c 'is_discontinued' ≥ 4` は起票時点で既に 10 件ヒットして vacuous）**: `rg -Fc 'p.is_discontinued' src-tauri/src/db/stocktake_repo.rs` = 4（baseline 2〈`:1314,1329` の無関係な既存 test assertion〉+ 新規 SQL 2）、`rg -Fc 'is_discontinued: row.get' src-tauri/src/db/stocktake_repo.rs` = 3（baseline 1〈`:470` の既存 `ProductForStocktake` 構築〉+ 新規構築 2） | いずれかの経路で `is_discontinued` が常に `false`（未反映）、または実際の商品状態と一致しない。fs literal が baseline のまま変化しない場合は SQL または構築側の追加漏れ |
...
| SC9 候補行 badge の是正（UI-10-D13） | 新旧 badge 混在 | unit（`StocktakePage.test.tsx` 拡張、fs literal） | SC9（Plan Review round 1 P3 是正: `-Fc '...border-border"'` は末尾クオート依存で脆いため引用符なしへ）: `rg -Fc 'border-border' src/features/stocktake/StocktakePage.tsx` = 2（新規箇所 + 候補行、正の oracle）かつ `rg -Fc 'border-border-strong' src/features/stocktake/StocktakePage.tsx` = 0（旧仕様混入防止、起票時点で既に 0 のため単独では非空 oracle にならず `= 2` と対で運用）。加えて候補行の既存 test（disabled 状態・「選択」button 等）が badge の class 変更以外は無変更で pass する | `border-border` の hit 数が 2 未満、`border-border-strong` が file 内に残る、または候補行の既存 test が回帰する |
...
- SC6 の fs literal は baseline（`p.is_discontinued` = 2、`is_discontinued: row.get` = 1）を明示した上での delta（+2/+2）で確認する。baseline を無視した絶対値 oracle（旧 `≥4`）は起票時点で既に真になっており vacuous だった（Plan Review round 1 P1）
- SC3 の `:127` test 更新漏れは、既存 test が古い `colspan="6"` を期待したまま残り CI で fail するため、機械的に検出される（サイレント regression にはならない）
