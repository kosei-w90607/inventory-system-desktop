...
| C4 | F4 | rg | AC4 `バリエーション: セクション見出し` = 1 / `SectionHeader` ≥ 1 / `awk ① \| rg -c "min-w-0 flex-1"` ≥ 2 / AC6 `1 ページ 1 個の h1` = 1 + `PageHeader で描かない` ≥ 1 | variation 欠落、契約 class 欠落、1 h1 の文言が変わる |
