...
| C5 | F5 / F7 | unit | `src/components/patterns/PageHeader.test.tsx`（`:53-59,70-71,87-92` を D-RT3 へ読み替え: `header` が `space-y-1`、見出し行が `flex flex-wrap items-start justify-between gap-3`、h1 に `min-w-0 flex-1`、subtitle / description `<p>` の親が `header`、actions の親が `shrink-0` / **新設**: actions 無し 2 variant〈title のみ / title + subtitle〉の `container.innerHTML` を `toMatchInlineSnapshot` で固定。repo に snapshot test は現状 0 件） | 左 group に残る / actions 無し分岐が変わる / `min-w-0 flex-1` 欠落 |
...
| SegmentedControl 使用 site（`rg -n '<SegmentedControl' src --glob '!*.test.tsx'` = 5） | `ProductListPage.tsx:135,143,179`（toolbar）/ `monthly-sales/components/ModeTabs.tsx:20` / `plu-export/PluExportPage.tsx:696`「書出しモード」（Card 内 mode 切替）。`components/sales/TabsHeader.tsx` は class 定数 + `<Link>` で component 非使用 | toolbar 3 site | mode 切替 2 site（catalog ⑤: 可視 Label を持たない）+ TabsHeader（非使用） | AC3 |
...
- `PageHeader` の actions 無し分岐を書き換えたら → S11 新設の inline snapshot 2 本が FAIL
