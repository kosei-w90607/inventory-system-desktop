...
- C3 catalog ⑨ D1 Checkbox 句: フィルタ toolbar 内の Checkbox は label 内包 + `h-9 items-center self-end`（GA3。toolbar 外の Checkbox は baseline のまま、GA4）
- C4 catalog ⑤ アクセシビリティ: toolbar 内 SegmentedControl は `showLabel` で `ariaLabel` を上置き可視 Label（`<span id>` + `aria-labelledby`、`aria-label` 属性なし）、mode 切替（ModeTabs / PluExport 書出しモード）は Label なし、`ariaLabel` prop は常に必須
...
- F3 SegmentedControl の `showLabel` 時に `aria-label` と `aria-labelledby` が併存し、accessible name の出どころが二重化する / span の文言が `ariaLabel` と一致しない
- F4 mode 切替 SegmentedControl（ModeTabs / PluExport 書出しモード）に `showLabel` が渡り可視 Label が付く
...
- F7 既存 assert の更新漏れ / 意図の消失: `StocktakePage.test.tsx:1171`（filter-row containment）/ `IntegrityCheckPage.test.tsx:549`（bg-card 不在）/ `PriceRevisionFilters.test.tsx:68`（GA2 群化）/ `PageHeader.test.tsx:53-59,70-71,87-92`（⑮ SC1）が旧 selector のまま FAIL する、または `:1185,1261` の「枠なし旧 frame 不在」literal を新 class へ書き換えて自己矛盾する
...
| C1 | F1 | regression | 既存 page test の `getByRole("combobox", { name: "表示件数" })` 等（`ProductListPage.test.tsx` / `StockInquiryPage.test.tsx` / `PriceRevisionFilters.test.tsx:73-77` / `StocktakePage.test.tsx` / `IntegrityCheckPage.test.tsx`。accessible name 依存の assert は無変更で PASS。構造 selector 依存の 3 file 4 箇所は F7 行で更新） | wrapper 変更で accessible name が壊れる |
| C1 | F1 | rg oracle | AC2 / AC4（wrapper が `flex items-center gap-2` でない、`font-normal` の有無） | 一部 site が旧配置のまま |
| C4 | F3 | unit | `src/components/ui/segmented-control.test.tsx`（追加: `showLabel` + `ariaLabel="廃番表示"` → `getByRole("group", { name: "廃番表示" })` 解決 + `getByText("廃番表示")` が `span` + group に `aria-label` 属性なし / `showLabel` なし → `aria-label={ariaLabel}` 現行どおり） | `aria-label` 併存 / span と group が id で結ばれていない |
| C4 | F3 | integration | `src/features/products/ProductListPage.test.tsx`（追加: 廃番表示 / PLU表示 / 並び順 の 3 group が name で解決し、同名の可視 text が存在） | `showLabel` が渡っていない |
| C4 | F4 | rg oracle | AC3（`ModeTabs.tsx` / `PluExportPage.tsx` / `TabsHeader.tsx` に `showLabel` 0 件） | mode 切替に Label が付く |
| C5 | F5 / F7 | unit | `src/components/patterns/PageHeader.test.tsx`（`:53-59,70-71,87-92` を D-RT3 へ読み替え: `header` が `space-y-1`、見出し行が `flex flex-wrap items-start justify-between gap-3`、h1 に `min-w-0 flex-1`、subtitle / description `<p>` の親が `header`、actions の親が `shrink-0` / **新設**: actions 無し 3 variant〈title のみ / title + subtitle / title + subtitle + description〉の `container.innerHTML` を `toMatchInlineSnapshot` で固定し、local で populate した literal を同 commit に含める。repo に snapshot test は現状 0 件） | 左 group に残る / actions 無し分岐が変わる / `min-w-0 flex-1` 欠落 |
| C3 | F6 | unit | `src/features/stocktake/StocktakePage.test.tsx`（既存 `getByLabelText("未入力のみ表示")` 相当 + 追加: `label` が `Checkbox` を内包し `h-9` / `items-center` / `self-end` を持つ） | label 内包に失敗 / `self-end` 欠落 / `h-9` 欠落 |
| C1 | F7 | unit | `src/features/stocktake/StocktakePage.test.tsx:1171` の `closest(".flex.flex-wrap.items-center.gap-4")` → `.flex.flex-wrap.items-end.gap-3`（containment 維持）。`:1185,1261` は触らない / `src/features/integrity-check/IntegrityCheckPage.test.tsx:549` と `src/features/products/components/PriceRevisionFilters.test.tsx:68` の `closest(".flex.items-center.gap-2")` → `.grid.gap-1`（bg-card 不在 / GA2 群化の意図維持） | 旧 selector のまま / `:1185,1261` を新 class に書き換える |
...
| C6 | F9 | rg oracle | AC8（`<h2[^>]*text-lg` から `font-medium` を除いて 0 件、`id` 付き h2 も対象。HomePage `text-lg font-medium` 3 件不変）+ `npm run typecheck` | 取りこぼし / 除外違反 |
| C7 | F9 | rg oracle | AC10（`src-tauri` / function-design / DSR / `components/sales` / ModeTabs / plu-export / home に diff 0） | 範囲外に触る |
...
| フィルタ toolbar 内 Checkbox（label 内包） | `PriceRevisionFilters.tsx:102,143`（内包）/ `StocktakePage.tsx:756`（sibling） | Stocktake 1 site + `h-9 items-center self-end` 2 site（toolbar 行内。GA4: `:143` は toolbar 外で対象外） | toolbar 外: `IntegrityCheckPage.tsx:399` / `PriceRevisionPage.tsx:65`（内包、toolbar 外）/ `ProductForm.tsx:226` / `BackupRestorePage.tsx:415,594`（sibling、form 内）/ `ProductImportPreview.tsx:132`（`aria-label` のみ） | AC6 |
| SegmentedControl 使用 site（`rg -n '<SegmentedControl' src --glob '!*.test.tsx'` = 5） | `ProductListPage.tsx:135,143,179`（toolbar）/ `monthly-sales/components/ModeTabs.tsx:19` / `plu-export/PluExportPage.tsx:695`「書出しモード」（Card 内 mode 切替）。`components/sales/TabsHeader.tsx` は class 定数 + `<Link>` で component 非使用 | toolbar 3 site | mode 切替 2 site（catalog ⑤: 可視 Label を持たない）+ TabsHeader（非使用） | AC3 |
...
| section h2 | `text-lg` を持つ h2 27 箇所（単一行 literal 25 + `id` 付き `MonthlySalesPage.tsx:157` / `DailySalesPage.tsx:165`）/ `text-xl font-semibold` 既存（IntegrityCheck / Stocktake / FormSection ④） | 27 箇所 | `HomePage.tsx:90,95,100`（`font-medium`、home lane）/ 非 h2 の `text-lg`（`IntegrityCheckPage.tsx:491` / `BackupRestorePage.tsx:577` / `ProductImportPreview.tsx:244` / `DailySalesPage.tsx:237`） | AC8 |
...
- missing input: `SegmentedControl` に `showLabel` 未指定 → `aria-label={ariaLabel}` 現行どおり（unit）
...
- optional field behavior: `showLabel?` 未指定で現行と同一 DOM
...
- helper connected to main path: `SegmentedControl` の `showLabel` が `ProductListPage` の 3 site に実際に渡る（`ProductListPage.test.tsx`）
...
- `SegmentedControl` が `showLabel` 時も `aria-label` を出力し続けたら → `segmented-control.test.tsx` の「`aria-label` 属性なし」assert が FAIL
...
- `PageHeader` の actions 無し分岐を書き換えたら → S11 新設の inline snapshot 3 本が FAIL
...
- mode 切替に `showLabel` を渡したら → AC3 rg が 1 件
- guard（`showLabel` 分岐）を外して常に wrapper を出したら → `showLabel` なし case の DOM 比較が FAIL
