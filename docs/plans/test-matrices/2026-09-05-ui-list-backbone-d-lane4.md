...
| GA1c scroll 解決 helper（Gated Amendment 1） | フォールバック不良 / 複数 scroller | unit（`page-scroll.test.ts` 新規/拡張、fake scroller + `app-router.test.tsx` 拡張） | GA1c: with a `[data-list-scroll-container]` element present, scrollPageToTop / the restoration resolver targets it; without one, falls back to main; querying `[data-list-scroll-container]` returns exactly 1 match on 商品一覧 and exactly 0 matches on a page without the box (paired oracle, round 2 是正 Opus P2 — a lone "at most 1" assertion is vacuously true when 0 boxes exist) | 属性ありでも `main` を対象にしてしまう、属性なしで例外を投げる、`商品一覧` で一致数が 1 でない、または箱を持たない画面で一致数が 0 でない |
| GA1d 商品一覧 scroll restoration（Gated Amendment 1） | 復元先が main のまま / 横位置が復元されない / scrollLeft mutant が happy-dom の無制限 raw property で生存する | unit（`ProductListPage.scroll-restoration.test.tsx` 新規、幾何 stub は `src/lib/app-router.test.tsx:127-158` の `installNativeScrollClamp` を**拡張**——round 2 是正 Opus P1: `scrollTop`/`scrollHeight`/`clientHeight` の clamp に加え、同型の `scrollLeft`/`scrollWidth`/`clientWidth` の clamp setter を追加する。happy-dom 20.10.6 の `scrollLeft`〈`node_modules/happy-dom/lib/nodes/element/Element.js:165-174`〉は無制限の raw property で router-core 内蔵の復元〈`scroll-restoration.js:147`〉が先に代入するため、幅の clamp が無いとアプリ層の `scrollLeft` 復元行を削除しても pass してしまう。round 1 P2-6: `OperationLogsPage.scroll-restoration.test.tsx` の `scrollTop===0` oracle は幾何 stub を持たず `applyWhenScrollable`〈`app-router.ts:105`〉を通過しないため参考にならない） | GA1d: box scrollTop and scrollLeft are both restored on 一覧→詳細→戻り for 商品一覧, verified against a width-clamped `scrollLeft` (not happy-dom's raw unclamped property) | 復元が `main.scrollTop` に対して行われ box の位置が戻らない、`scrollLeft` が復元されない、または `scrollLeft` 削除 mutant が幅 clamp なしの harness で生存する |
