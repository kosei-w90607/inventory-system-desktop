...
| GA1c scroll 解決 helper（Gated Amendment 1） | フォールバック不良 / 複数 scroller | unit（`page-scroll.test.ts` 新規/拡張、fake scroller + `app-router.test.tsx` 拡張） | GA1c: with a `[data-list-scroll-container]` element present, scrollPageToTop / the restoration resolver targets it; without one, falls back to main; at most one matching element exists in the document (round 1 P3-8, uniqueness guard) | 属性ありでも `main` を対象にしてしまう、属性なしで例外を投げる、または複数一致時に文書順先頭を無警告で採用する |
| GA1d 商品一覧 scroll restoration（Gated Amendment 1） | 復元先が main のまま / 横位置が復元されない | unit（`ProductListPage.scroll-restoration.test.tsx` 新規、幾何 stub は `src/lib/app-router.test.tsx:132-155` の `installNativeScrollClamp` を踏襲——round 1 P2-6: `OperationLogsPage.scroll-restoration.test.tsx` の `scrollTop===0` oracle は幾何 stub を持たず `applyWhenScrollable`〈`app-router.ts:105`〉を通過しないため参考にならない） | GA1d: box scrollTop and scrollLeft are both restored on 一覧→詳細→戻り for 商品一覧 | 復元が `main.scrollTop` に対して行われ box の位置が戻らない、または `scrollLeft` が復元されない |
...
- GA1c は fake scroller の有無で対照 oracle を作る（属性ありのときだけ helper がそれを対象にする、無いときは `main` にフォールバックする、の両方を同一 test 内で確認し「常に `main` を返す」「常に fake scroller を返す」のどちらの mutant も検出する）。加えて `[data-list-scroll-container]` に一致する要素が高々 1 個であることを確認し、2 個目が生まれても文書順先頭を無警告採用しないことを保証する（round 1 P3-8）
