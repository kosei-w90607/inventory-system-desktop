...
- SC21（**F1, Opus P1-1, CHANGE**）: catalog `:48`/`:191` の現在形 gap 文（「現状排他である」「計 5 画面が影響」「現状 truncate のみで title を欠くため是正対象」）が「何を・どの PR で・どう解消したか」を述べる反映済み文へ書き換えられる（追記ではなく書き換え）。**F13 追加**: `:191` の `OperationLogsPage.tsx:522` 引用も `:533` へ訂正される
...
| SC21 catalog gap 文書き換え（**F1, CHANGE**） | 追記のみで旧文残存 | docs review（`rg -Fc`、非 vitest） | SC21: `rg -Fc "現状排他である" docs/design-system/02-component-catalog.md` = 0 かつ `rg -Fc "計 5 画面が影響" docs/design-system/02-component-catalog.md` = 0 かつ `rg -Fo "は現状 truncate のみで" docs/design-system/02-component-catalog.md \| wc -l` = 0（起票時いずれも 1）。`rg -Fc "本 PR（⑮）で" docs/design-system/02-component-catalog.md` ≥ 2。**F13 追加**: `rg -Fc "OperationLogsPage.tsx:522" docs/design-system/02-component-catalog.md` = 0（起票時 1）かつ `rg -Fc "OperationLogsPage.tsx:533" docs/design-system/02-component-catalog.md` ≥ 1 | 旧 gap 文が残ったまま新文言だけ追記される（矛盾併存）、または `:522` の stale citation が残る |
| SC23 DSR-22 anchor 実測行更新（**F9, Opus P2-7 追加**） | 更新漏れ | docs review（`rg -Fc` + reviewer 実読、非 vitest） | SC23: `rg -Fc "InventoryRecordsPage.tsx:336-343" docs/design-system/01-decision-rules.md` = 0（起票時 2〈`:435`,`:443`〉）。新引用が実装後の実際の head 行番号と一致することは Final Review が `rg -n` 実測で確認する（pre-implementation では正確な新番号を確定できないため機械 oracle 化不能） | 旧 anchor が残る、または新 anchor が実装後の実測行と一致しない |
...
- `01-decision-rules.md:435`/`:443` の `InventoryRecordsPage.tsx:336-343` 引用を実装後も更新し忘れたら、SC23 の `rg -Fc` = 0 オラクルが fail するか（F9）
