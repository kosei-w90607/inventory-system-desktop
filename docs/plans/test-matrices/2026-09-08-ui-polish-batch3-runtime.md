...
- SC23（**G5, Opus P3-1 / Sonnet P2-1 追加 / F9 是正**）: `01-decision-rules.md:435`（裸の `` `:336-343` ``/`` `:339` ``/`` `:342` ``）と `:443`（file 名付き `InventoryRecordsPage.tsx:336-343`）の anchor が S4 の記録ID列撤去に伴い実装後の実測行へ更新される（decision 本体 (b) は変更しない）
...
| SC23 DSR-22 anchor 実測行更新（**G5 追加 / F9, G2 是正**） | 更新漏れ / 誤ったオラクル対象 | docs review（`rg -Fo … \| wc -l` + reviewer 実読、非 vitest） | SC23: **G2 是正**（`:435` は file 名を再掲しない裸表記のため file 名付き literal では `:443` の 1 件しか検出できない）— `rg -Fo ":336-343" docs/design-system/01-decision-rules.md \| wc -l` = 0（起票時 2〈`:435`,`:443`〉）かつ `` rg -Fo ':339`' docs/design-system/01-decision-rules.md \| wc -l `` = 0（起票時 1）かつ `` rg -Fo ':342`' docs/design-system/01-decision-rules.md \| wc -l `` = 0（起票時 1）。新引用が実装後の実際の head 行番号と一致することは Final Review が `rg -n` 実測で確認する（pre-implementation では正確な新番号を確定できないため機械 oracle 化不能） | 旧 anchor が残る、または新 anchor が実装後の実測行と一致しない |
...
- `01-decision-rules.md:435`/`:443` の裸 `` `:336-343` ``/`` `:339` ``/`` `:342` `` 引用を実装後も更新し忘れたら、SC23 の `rg -Fo | wc -l` = 0 オラクルが fail するか（F9/G2）
