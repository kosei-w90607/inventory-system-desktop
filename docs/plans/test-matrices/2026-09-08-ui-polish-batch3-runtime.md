...
- SC19（**F15, Opus P3-5, CHANGE**）: 9 箇所のローカル `formatQuantity`（+ `StocktakeRecordDetailPage.tsx` の signed/optional 変種）が削除される。Gated Amendment 2 → **Gated Amendment 3 で置換**: 入力行テーブルの `columnheader` 配列に「単位」が **無い**（`ManualSalePage` = [「商品コード」,「商品名」,「部門」,「現在庫」,「数量」,「販売金額」,「操作」]、`DisposalPage` = [「商品コード」,「商品名」,「部門」,「現在庫」,「種別」,「数量」,「原価」,「理由」,「操作」]、`ReturnExchangePage` = [「商品コード」,「商品名」,「部門」,「現在庫」,「方向」,「数量」,「操作」]）。数量 cell 内に input と同居する「個」/「cm」の text（`within(cell)` で `getByRole("spinbutton"|"textbox")` と `getByText("個")` の両方が解決）、現在庫 cell が「4 個」形（`formatStockDisplay`）、金額 input cell 内に「円」。単位列が別途ある 3 site（`ManualSalePage.tsx:610`/`DisposalPage.tsx:514`/`ReturnExchangePage.tsx:834`）は数値のみ（`toLocaleString("ja-JP")`）になり、それ以外の 9 site は `formatStockDisplay`/`formatStockUnitLabel` を使い unit code（`pcs`/`cm`）が日本語ラベルへ翻訳される
