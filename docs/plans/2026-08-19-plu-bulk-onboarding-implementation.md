...
- PR #85 merge（squash `014c064`）後の base 付け替えは、packet 計画の rebase 方式を破棄して **origin/main の単段 merge 方式**へ切替した（PR #85 packet「main drift 吸収の merge 記録」と同じ D-055 判断）。根拠 2 点:
...
- merge commit = `e72c1ce`（parents = 本 branch 先端 `2fbb3af` + origin/main `3bcdf45`）。squash 由来の旧 merge-base に落ちるため見かけ衝突が広域に出るが、解消は次のとおり検分済み:
