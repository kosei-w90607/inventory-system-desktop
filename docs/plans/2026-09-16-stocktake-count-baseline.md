...
- **D-D4 Z004 取込みの「カウント時点境界」（STK-2）**: BIZ-03 commit step 6b で `pos_stock_sync` 行の在庫減算の前に、その商品の最新 `counted_at`（進行中・完了済みを問わず `stocktake_items` の最新 1 件、未カウント = NULL）を引き、CSV 行の `settlement_date` が「カウント日より前」（Q2 の回答で同日の扱いを決める）なら在庫減算をスキップする。`sale_records` は作る（売上は正）。movement は作らない。境界判定は実測カウント（`counted_at` NOT NULL）だけを基準にし、force_fill / 廃番自動入力の明細（`counted_at` NULL）は基準にしない。判定形: 対象 product_code 群について `stocktake_items` から `MAX(counted_at)` を 1 回の query で map として引き、`settlement_date`（`YYYY-MM-DD`）と `substr(counted_at, 1, 10)` を比較する（Opus F14 / F17）。operator への通知は preview / parse 段階の既存 `warnings`（`mod.rs:83`）に「棚卸しで計上済みのため在庫を変更しない行 N 件」を載せ、commit 結果 `ImportResult` の wire 型は変えない（Opus F3。preview と commit の間にカウントが入れば件数は変わり得るが、在庫を守る規則は commit 時の判定が正）。不変条件（32 §15.8 / 35 §20.8 に置く）: 「カウント前に発生した販売の movement は、カウント後に作られない」。これにより D-D4（日付比較）と D-D5（時刻比較）が整合する（Opus F16）。棄却: 取込みを拒否（売上記録まで失う）/ movement を quantity 0 で残す（整合性チェックと履歴を汚す）/ Z004 運用開始まで境界を設計しない（UI-10-D2 の再訪条件がまさに Z004 運用開始で、設計は今しておく方が実装が 1 回で済む）
...
三つまとめて「既定でよい」とはしない。Q2 が店の聞き取り待ちのため Phase は design のまま。
...
- **Q2 同日の POS 販売の扱い（STK-2 の境界、店の手順で決まる、未決）**: Z004 は日付しか持たないため、カウント日と同じ日付の販売を「カウント後（在庫を減らす、`settlement_date < カウント日` だけスキップ）」とみなすか「カウント前（減らさない、`settlement_date <= カウント日` をスキップ）」とみなすかを決める。前者 = 開店前に数えてその場で保存する運用、後者 = 閉店後に数える運用。営業中に数える運用ならどちらも正確にならない。**店への聞き取り（owner）**: (1) 棚卸しのカウントは開店前・営業中・閉店後のどれか (2) 数えたその場で保存するか、翌日にまとめて入力することがあるか（後者なら「カウント日」を保存日時と別に入力する設計が要る。聞き取り結果まで設計しない）。前日分の Z004 を取り込んでからカウントすることは正しさの必要条件ではない（未取込みでも snapshot が過大な分を補正が引き、後着分は境界でスキップされる。Opus F15）ので前提にしない
...
- **S2 `docs/function-design/35-biz-stocktake-service.md`**: §20.4 step 3 に `system_stock = 現在の stock_quantity` の書込みを追加、step 4 を `current_difference = system_stock − actual_count` に、`:218-219` の設計判断を D-D1 / D-D3 へ書き換え / §20.5 step 3a（force_fill）を `system_stock = actual_count = 現在庫` に、step 4 の `StocktakeItemForComplete` に `system_stock` を追加、step 5 f/g を D-D2（`adjustment = actual_count − system_stock`、`apply_stock_change` 経由、`AdjustedItem` の意味）に、`:317` の「`apply_stock_change` を使わない理由」を撤回し理由を残す / §20.6a `:425` 設計ノートを「詳細画面の差異 = 補正 movement の quantity = `actual_count − system_stock`（D-090 で snapshot 差と一致）」へ / §20.7 `:435` を「棚卸し中も CSV 取込みは許可。実測カウント済み商品はカウント時点境界で在庫減算をスキップ（BIZ-03 §15.4、D-090）」に / §20.8 INV-2 行を「`apply_stock_change` の通常式」に、不変条件「カウント前に発生した販売の movement はカウント後に作られない」と「fix_integrity は対象外」を各 1 行追加、確定後在庫が負になり得る方針（D-D2）を明記 / 更新履歴 1 行。`BIZ-06-D1`〜`D3` の子 ID を D-D1〜D-D3 に対応させて置く
...
- **AC9** 一貫性: packet の D-D1〜D-D5 の数値例（10 / 8 / 12 の例）が 35 / 32 の本文に例として写され、Q2 の回答（`<` か `<=`）が D-090 / 32 §15.4 / 73 UI-10-D14 の 3 箇所で同じ記号で書かれている（`rg -c 'settlement_date <=' <3 file>` と `rg -c 'settlement_date <[^=]' <3 file>` の 2 本で、Q2 の回答に応じて片方が各 file ≥ 1、もう片方が 0。Q2 未決のうちは本 AC は保留）
