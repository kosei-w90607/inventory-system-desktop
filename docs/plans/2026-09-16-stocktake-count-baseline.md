...
- Writer: Sonnet（subagent、worktree 分離、Plan Reviewer とは別 fresh context。設計判断は本 packet の D-D1〜D-D6 と owner 回答で確定済みのものを source docs へ書く役）
...
owner の設計判断 3 問（下記「owner への設問」Q1〜Q3）。これは Phase `design → plan-draft` の遷移条件（未解決の設計質問なし）であり、tracked の Human Gate 欄には置かない。回答が揃い、Coordinator 既定案と異なる回答は packet と D-D1〜D-D6 に反映してから plan-draft へ進む。manual（L3）は docs-only のため不要。runtime lane ㉘ の L3 で実機確認する。

遷移記録（append-only）:
- kickoff → spec-check → design（本 commit）: Risk R2（docs-only。BIZ-06 / BIZ-03 / UI-10 の設計正本と decision-log を更新し、runtime 契約はこの lane では変えない）。Design Phase = 本 packet で方式を比較し Coordinator 既定案を置く。owner 回答待ちのため plan-draft へは進めない（DEV_WORKFLOW「design → plan-draft: no unresolved design questions」）。
...
Goal Invariant: 棚卸しの「基準時点」が source docs で 1 つに定まり、(a) カウント後の入出庫を確定が打ち消さない（STK-1）、(b) 確定後に届く過去販売の Z004 取込みが在庫を二重に減らさない（STK-2）、(c) `stocktake_items.system_stock` の意味が DB / BIZ / UI の文書で一致する（DOC-2）状態を、実装者が chat や本 packet を読まずに source docs だけで実装・テストできる。
...
- docs: 35 `:203` 「counted_at = 現在日時」/ `:218-219` 設計判断「差異は `system_stock` ではなく現在の `stock_quantity`」「`system_stock` は『開始時点の参考値』」/ `:283` step 5f / `:317` 「`apply_stock_change` を使わない理由」/ `:435` §20.7「差異は動的計算で吸収」/ `:450` §20.8 INV-2「stock_after = actual_count を直接使用」/ `:512` 更新履歴。`rg -c '差異は動的計算'` = 1、`rg -c '開始時点の参考値'` = 1、`rg -c 'apply_stock_change を使わない理由'` = 1、`rg -c 'BIZ-06-D'` = 0
- docs: 73 `:41` UI-10-D2 Rejected「BIZ 確定ロジック側で `counted_at` 以降の在庫変動を自動織り込む（現運用では織り込むべき商品単位データ自体が存在しないため実装不能。将来 Z004/PLU 運用が始まったら再検討）」+ `:44` Revisit trigger / `:97` UI-10-D10（差異 = `current_stock - actual_count`、「システム在庫」列を「現在在庫」列へ改称）/ `:125` UI-10-D13（最終 ID）/ `:378` §73.14 の out-of-scope「BIZ-06 確定ロジックの変更（… UI-10-D2）」/ `:408-409` 変更履歴。`rg -c 'UI-10-D14'` = 0、`rg -c '実装不能'` = 1
...
- **D-D4 Z004 取込みの「カウント時点境界」（STK-2）**: BIZ-03 commit step 6b で `pos_stock_sync` 行の在庫減算の前に、その商品の最新 `counted_at`（進行中・完了済みを問わず `stocktake_items` の最新 1 件、未カウント = NULL）を引き、CSV 行の `settlement_date` が「カウント日より前」（Q2 の回答で同日の扱いを決める）なら在庫減算をスキップする。`sale_records` は作る（売上は正）。movement は作らない。取込み結果に「棚卸しで計上済みのため在庫を変更しなかった行 N 件」を warning として返す（既存の warning 経路を使う）。棄却: 取込みを拒否（売上記録まで失う）/ movement を quantity 0 で残す（整合性チェックと履歴を汚す）/ Z004 運用開始まで境界を設計しない（UI-10-D2 の再訪条件がまさに Z004 運用開始で、設計は今しておく方が実装が 1 回で済む）
- **D-D5 取込み取消（rollback）時の snapshot 補正**: `rollback_csv_import` が void する movement のうち、進行中棚卸しでカウント済み（`counted_at` が movement の `created_at` より後）の明細があれば、その `system_stock` から void した quantity 分を戻す（`system_stock -= movement.quantity`。販売 −2 を void → snapshot +2）。理由: snapshot は「取り込まれた誤った販売」を含んでおり、取消で現在庫が戻ると D-D2 の補正が二重になる（例: 開始 10、誤 CSV −2 で 8、カウント 10 / snapshot 8 → 補正 +2 予定 → 取消で現在庫 10 → 確定で 12 になる）。完了済み棚卸しは既に補正済みのため対象外。日報取込み（Z001/Z002/Z005）は在庫を動かさないため対象外。手動記録の取消機能は現行に無い（DOC-1）ため対象外、将来追加時に同じ規則を適用する。棄却: 棚卸し中の rollback を禁止（UI-10-D1 の「偽の状態を作らない」に反し、誤取込みの是正手段を 3 か月止める）
- **D-D6 決定の置き場**: durable な横断判断は decision-log D-090（D-025 / SP-205-09 / UI-10-D2 を引く）。BIZ 契約は 35（BIZ-06）と 32（BIZ-03）、repo 契約は 20 §2.11、CMD は 42、DB 意味は tracking、UI は 73 UI-10-D14。UI-10-D2 の Rejected 本文は書き換えず、行末に「→ 2026-09-16 D-090 で再訪（UI-10-D14）」を追記する（非遡及）。biz-task-specs.md `:465` / `:482` は要約層として同期

## owner への設問（design → plan-draft の遷移条件）

回答は「Q1 = a / Q2 = b」の形で足りる。既定案のまま進めてよければ「既定でよい」の一言でよい。

- **Q1 方式**: (a) **既定** = D-D1〜D-D3（カウント時に snapshot、確定はカウント時点差異で補正、差異表示もカウント時点差異）。(b) 再カウント必須方式（カウント後に動いた商品を確定前に数え直させる。運用負荷が大きいため非推奨）。(c) 現状維持 + 運用ルール（確定直前に全件再カウント。長期棚卸しでは守れないため非推奨）
- **Q2 同日の POS 販売の扱い（STK-2 の境界、店の手順で決まる）**: Z004 は日付しか持たないため、カウントした日と同じ日付の販売を「カウント後（在庫を減らす）」と「カウント前（減らさない）」のどちらとみなすかを決める。(a) **既定** = カウント後扱い（`settlement_date < カウント日` だけスキップ）。前提 = 開店前にカウントする / 前日分の Z004 を取り込んでからカウントする。(b) カウント前扱い（`settlement_date <= カウント日` をスキップ）。前提 = 閉店後にカウントする。店が「営業中にも数える」なら、その日の販売分だけ誤差が出ることを承知の上で (a) を選び、73 に運用ルール（「数える商品は開店前に、前日分の取込み後に」）を書く
- **Q3 STK-2 の境界（D-D4 / D-D5）を runtime lane ㉘ に含めるか**: (a) **既定** = 含める（境界判定は commit 6b の 1 query + 日付比較、取消側は 1 UPDATE で小さい。Z004 運用開始時に再設計しなくて済む）。(b) 設計だけ確定し、実装は Z004 本運用開始（UI-10-D2 の再訪条件）まで先送り

## Scope

docs-only。Writer は Sonnet subagent。

- **S1 `docs/decision-log.md`**: D-090「棚卸しの基準時点は各商品の最終カウント時点とし、確定はカウント時点差異で補正する（2026-09-16）」= Decision / Status accepted / Why（STK-1 / STK-2 / DOC-2、UI-10-D2 の根拠が手動 movement に当てはまらない）/ Impact（BIZ-06 / BIZ-03 / UI-10 / IO stocktake_repo、runtime lane ㉘）/ Alternatives（D-D2 / D-D4 / D-D5 の棄却案）/ Revisit（`inventory_movements` に業務日付を持つ設計へ移る場合、POS が時刻付き明細を出せるようになった場合）。Q2 の回答（同日規則）を Decision 本文に含める
- **S2 `docs/function-design/35-biz-stocktake-service.md`**: §20.4 step 3 に `system_stock = 現在の stock_quantity` の書込みを追加、step 4 を `current_difference = system_stock − actual_count` に、`:218-219` の設計判断を D-D1 / D-D3 へ書き換え / §20.5 step 3a（force_fill）を `system_stock = actual_count = 現在庫` に、step 4 の `StocktakeItemForComplete` に `system_stock` を追加、step 5 f/g を D-D2（`adjustment = actual_count − system_stock`、`apply_stock_change` 経由、`AdjustedItem` の意味）に、`:317` の「`apply_stock_change` を使わない理由」を撤回し理由を残す / §20.7 `:435` を「棚卸し中も CSV 取込みは許可。カウント済み商品はカウント時点境界で在庫減算をスキップ（BIZ-03 §15.4、D-090）」に / §20.8 INV-2 行を「`apply_stock_change` の通常式」に / 更新履歴 1 行。`BIZ-06-D1`〜`D3` の子 ID を D-D1〜D-D3 に対応させて置く
- **S3 `docs/function-design/32-biz-csv-import-service.md`**: §15.4 step 6b にカウント時点境界（D-D4、Q2 の規則、warning の文言と件数の返し方）/ §15.5 に snapshot 補正（D-D5）/ §15.8 に不変条件 1 行（「カウント済み商品の在庫は、カウント日より前の販売を取り込んでも減らない」）/ 更新履歴 1 行。`BIZ-03-D` の子 ID を置く
- **S4 `docs/function-design/73-ui-stocktake.md`**: `:41` UI-10-D2 Rejected 行末に再訪注記（D-D6）/ UI-10-D14 新設（差異列 = `system_stock − actual_count`、「現在在庫」列は維持、カウント入力欄の選択商品情報も同じ、確定結果の `adjusted_items` の意味、運用ルール = Q2 の回答、Why / Rejected / Revisit）/ `:378` §73.14 の out-of-scope 行を「D-090 で再訪、runtime lane ㉘」に / 変更履歴 1 行
- **S5 `docs/db-design/tracking-system-tables.md`**: `:110` `system_stock` = 「最終カウント時点のシステム在庫（未カウントは開始時点の値、force_fill は確定時点の値）」/ `:116` 設計意図を D-D1 / D-D2 / D-D5 に合わせて書き換え
- **S6 `docs/architecture/biz-task-specs.md`**: `:465` / `:482` の「動的計算」を D-D2 / D-D3 の要約に
- **S7 `docs/function-design/42-cmd-sales-stocktake.md`** `:224` `update_count` の `current_difference` の意味 / **`docs/function-design/20-io-product-repo.md`** §2.11 `update_stocktake_item_count`（`system_stock` 引数追加）、`get_stocktake_items_for_complete`（`system_stock` を返す）、新規 `find_latest_counted_at_by_product`（D-D4 用、商品ごとの最新 `counted_at`）、rollback 用 `adjust_counted_system_stock`（D-D5 用）の契約
...
- **AC6** `rg -c 'D-090' docs/function-design/42-cmd-sales-stocktake.md docs/function-design/20-io-product-repo.md docs/diagrams/cross-feature-verification.md` の各 file ≥ 1（baseline 0）
- **AC7**（負の oracle）`git diff --name-only origin/main..HEAD -- src-tauri src docs/research docs/spec docs/function-design/37-biz-daily-report-import-service.md | wc -l` = 0
- **AC8** `bash scripts/doc-consistency-check.sh --target plan` ERROR 0 / `bash scripts/check-workflow-git.sh` PASS / `bash scripts/doc-consistency-check.sh`（full、docs 変更のため）ERROR 0。`src-tauri/tests/design_compliance_test.rs` が 35 / 32 の必須セクション（シグネチャ / 処理ステップ / エラーハンドリング）を要求するため、見出し構造を壊さない（`cargo test --offline --test design_compliance_test` PASS）
- **AC9** 一貫性: packet の D-D1〜D-D5 の数値例（10 / 8 / 12 の例）が 35 / 32 の本文に例として写され、Q2 の回答（`<` か `<=`）が D-090 / 32 §15.4 / 73 UI-10-D14 の 3 箇所で同じ記号で書かれている（`rg -n 'settlement_date <' docs/decision-log.md docs/function-design/32-biz-csv-import-service.md docs/function-design/73-ui-stocktake.md` の記号が一致）
...
- Absolute guarantee / escape hatch self-check completed, with every exception checked and compatibility stated: 「カウント後の入出庫を打ち消さない」の例外 = 取消（rollback、D-D5 で補正）と force_fill（snapshot = 現在庫で差異 0）。「過去販売を二重に減らさない」の例外 = 同日販売（Q2 の規則、運用ルール）と未カウント商品（境界なし、通常減算）。整合性チェック（BIZ-07）は `stock_quantity = SUM(movement)` を維持する（補正は movement 経由、スキップは movement を作らない）
...
- D-D2 の恒等式と数値例が正しいか（開始 10 → カウント 10 → 販売 2 → 確定で 8 / 誤 CSV −2 → カウント 10 → 取消 → 確定で 10 / POS 2 販売未取込み → カウント 8 → 確定 8 → 遅い取込みでスキップ → 8）。D-D5 の補正方向（`system_stock -= voided.quantity`、販売 −2 の void で +2）。Q2 の記号が 3 箇所で揃うか。UI-10-D2 の再訪を非遡及で書けているか。35 の必須セクション構造。runtime lane ㉘ への申し送りが file:line で足りるか

## 後続 runtime lane ㉘ への申し送り（本 lane の成果物の一部、file:line は `c6167c4d`）

- `src-tauri/src/db/stocktake_repo.rs:268-279` `update_stocktake_item_count` に `system_stock` 引数 / `:406-427` `get_stocktake_items_for_complete` に `system_stock` 列 / 新規 `find_latest_counted_at_by_product(conn, product_code) -> Option<String>` / 新規 `adjust_counted_system_stock(tx, product_code, delta, before_counted_at)`。IO 設計 20 §2.11 に契約（本 lane S7）
...
- docs 同期: 35 / 32 の疑似コードを実装に合わせて最終化、`cross-feature-verification.md` の期待値、73 §73.9 の warning 文言（取込み結果）
