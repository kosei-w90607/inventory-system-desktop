...
- R9（売上と返品の相殺への対策、TD-024）: 以下が現行の規則（Gated Amendment 3 の補足 3 までを反映。ADR D4「売上と返品の相殺」と D5 に置く）。置き換えた旧案と理由は末尾の「経緯」にまとめる。
  ...
  - 「前回」: 同じ machine_no の直前の settlement_no の Z004 で、EJ の区間の開始と連続（D5）が証明できるものに限り、受領順では選ばない。最初の区間と未受領の見分け方: その machine_no の Z004 をアプリが一度も受領していなければ最初の区間（前回が存在しない扱い）、受領したことがあり直前の settlement_no の Z004 だけが無ければ未受領とする。最初の区間の判定は EJ の有無より先に行い、Z004 の取込みの時点で登録の変化にする（EJ の取込みを案内しない）。settlement_no の戻り: 同じ machine_no で番号の戻りは、同じ machine_no で受領済みの settlement_no と同じ番号の Z004 を、別の精算（精算の同一性が一致しない）として受領したときに限って判定する。そのときは前回を証明できない扱い（登録の変化）とし、以後の「前回」の候補は戻った後に受領した Z004 に限る（レジ入替・番号 reset の後に、戻る前の Z004 を前回と取り違えないため。F3）。受領済みの最小と最大のあいだの欠番を埋める Z004 は後追いの通常の受領として扱い、前回の判定と再評価（F1・前回未受領）に使う。受領済みの最小より小さい番号の Z004 は前回を証明できない扱い（登録の変化）とする（Coordinator の裁定 2026-09-24。Writer の反映で、最大より小さい番号を一律に戻りとすると欠番の後追い取込みまで登録の変化になり、前回の Z004 の取込みで解消する経路を弱めると分かったため）。
