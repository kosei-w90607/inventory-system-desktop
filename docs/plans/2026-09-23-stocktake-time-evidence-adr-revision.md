...
- 介入回数上限: 10（当初 4〈起票中の owner 確認 1、Codex の Final Review relay 1、Ready 1、merge 1。既定 3 から 1 増やす理由 = owner にしか答えられない店の運用と受容 risk の確認を 1 回含むため〉。2026-09-25 に owner 承認で引き上げ「引き上げしていいよ」「その数で記録していいよ」: Codex broad の差し戻しで GA4 が入り、owner の承認・判断と取り直しの relay が増えた。消費 9〈起票中の確認〈2026-09-24〉、GA3 の判断〈TD-027〉、relay 80、GA4 の承認、Consequences の 1 文の判断、引き上げの承認、数え直しの確認、relay 80b、Ready・merge の承認〈owner 2026-09-25「Ready から締めまで」〉〉、残り 1 = relay 80c）
...
- AC9（相殺への対策）: ADR D4 / D5、`32-biz-csv-import-service.md` の分類 step と外部 probe 表、`tracking-system-tables.md` の flag の理由の列に、R9 の分岐（完全な EJ に候補の行なし → flag なし、行あり → 相殺の行あり、EJ なし・不完全・前回の Z004 が未受領 → 相殺の確認待ち〈EJ待ち〉、帰属できない行・前回を証明できない → 相殺の確認待ち〈登録の変化〉。確認待ちは 2 種とも確定を止める）と、後から取り込む EJ か前回の Z004 による EJ待ちの再評価（帰属できない行と前回の証明を先に判定する順序）、直前の settlement_no の Z004 の受領による、直前の Z004 が未受領の間の登録の変化（最初の区間・受領済みの最小より小さい番号の区間・番号の戻りの区間）の再評価、前回の code が分からない区間（直前の settlement_no の Z004 を受領していない区間。最初の区間・受領済みの最小より小さい番号・前回の未受領・番号の戻り）で F5 の外部前提が真のとき今回の Z004 に行のない数えた在庫連動商品を確認待ちの対象に含めること（Gated Amendment 4、語は Gated Amendment 5）が書かれていることを Contract Audit が確認する。`rg -n '相殺の確認待ち' docs/adr/2026-09-18-stocktake-time-evidence.md docs/function-design/32-biz-csv-import-service.md docs/db-design/tracking-system-tables.md` が 3 file とも一致する（改訂前 0 件）。
...
- negative tests: 合成モデルへの mutation（AC5 (i)〜(xxiv)）。撤去語の再混入を AC2 の 3 つの検索で検出。
...
- R9（売上と返品の相殺への対策、TD-024）: 以下が現行の規則（Gated Amendment 4 までを反映。ADR D4「売上と返品の相殺」と D5 に置く）。置き換えた旧案と理由は末尾の「経緯」にまとめる。
  ...
  - 区間全体の確認待ち（次の 2 種に限る。対象はその区間で数量・金額とも 0 の数えた商品の全て。今回の Z004 に行がない商品〈PLU 未登録の商品〉は含まない。F5 の前回の Z004 にはある code の商品だけは例外として含む。前回の code が分からない区間〈直前の settlement_no の Z004 を受領していない区間。最初の区間・受領済みの最小より小さい番号の区間・前回の未受領の区間・番号の戻りの区間〉では、F5 の外部前提が真のとき、数えた在庫連動商品のうち今回の Z004 に行がないものを全て対象に含め、区間の理由〈EJ待ちか登録の変化〉と同じ flag を付ける。直前の settlement_no の Z004 の受領時の再評価で、前回にその code があれば D4 の順で判定し〈EJ にその名称の行があれば相殺の行あり〉、前回にも無ければ解消する。flag は取込みの TX で作るので、解消は他の flag と同じく新しい実測の保存か当該 import の取消でも起きる〈Gated Amendment 4、Codex broad #2。JAN のない在庫連動商品にも前回の受領までの間は付く。語は Gated Amendment 5 で、1 種の意味の「前回の Z004 の未受領」と区別するため改名した、Opus closure N2〉）:
    ...
    - 相殺の確認待ち（登録の変化、`offset_mapping_changed`）: 区間の EJ に帰属できない行がある場合と、前回を証明できない場合（最初の区間と、前回の Z004 はあるが区間の開始と連続を証明できない場合）。前回を証明できないとは、EJ に区間の開始の証拠はあるが受領済みの前回の Z004 と一致しない場合をいう。開始の証拠が EJ に無い場合は不完全（EJ待ち）とする（F4）。EJ を取り込み直しても解消しないので、EJ の再評価の対象にしない。ただし直前の Z004 が未受領の間の登録の変化（前回の候補になる直前の settlement_no の Z004 がまだ無いことによるもの。最初の区間・受領済みの最小より小さい番号の区間・番号の戻りの区間。番号の戻りの後は、戻った後に受領した直前の番号の Z004 に限る）は、直前の settlement_no の Z004 を初めて受領した時点で EJ待ちと同じ順で全て再評価する（今日の Z004 と EJ を先に取り込み、古い番号の Z004 を新しい方から後追いで取り込んだ場合にも、受領の順で判定を固定しないため。F1 を Gated Amendment 4 で一般化、Codex broad #1）。前回が未受領の区間で登録の変化になる原因はこの 3 つだけなので（帰属できない行の判定と開始の証拠の照合は前回の受領が要る）、この再評価で残る登録の変化（帰属できない行がある区間と、EJ の開始の証拠が受領した前回と一致しない区間）は証拠の矛盾として凍結し、数え直しで解消する。flag の理由の 5 値は変えない（直前の Z004 が未受領の間かは受領の記録から分かる）。文言は、直前の Z004 が未受領の間の登録の変化には、EJ の取込み機能が入った後は直前の精算の商品別 CSV の取込みも案内し、それ以外の登録の変化には数え直しを案内する（R7。Coordinator の裁定 2026-09-24 で最初の区間に置き、Gated Amendment 4 で広げた。数え直しだけを案内すると、取込みで解消する区間で余分な数え直しを招くため）。
