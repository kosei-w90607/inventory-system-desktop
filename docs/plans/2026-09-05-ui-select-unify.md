...
- **S1 DisposalPage 1 箇所**: `:509` の種別 per-row native `<select>` を `Select`/`SelectTrigger`/`SelectContent`/`SelectItem` へ（`aria-label` 維持、`DisposalType` 3 値、sentinel 不要）。`disabled={isFormLocked}`（`:511`）を `SelectTrigger` の `disabled` として保持する（**Opus Plan Review round 2 P2-2**）。完了条件: `rg -c "<select" src/features/disposal/DisposalPage.tsx` = 0（起票時 1）かつ `rg -c "SelectTrigger" src/features/disposal/DisposalPage.tsx` ≥ 1
...
| L8-D6 ReturnExchangePage 条件描画撤去（round 1 P1-3） | `ReturnExchangePage.tsx` | `ReturnExchangePage.test.tsx`（S10） | non-scope（disabled 側は L3-only） |
