...
- **Lane 4 drift 注記（Plan Review round 1 P2）**: Lane 4（`agent/ui-list-backbone-d-lane4`、S3a、`docs/plans/2026-09-05-ui-list-backbone-d-lane4.md:179,460`）が `StocktakePage.tsx:821-852` の `else` 分岐（現状 `<Table>` を裸で return）を `PaginationSummary` 追加のため `<>...</>`（Fragment）または `<div>` で包み直す。本 lane は merge train で Lane 4 の**後**（Lane 5 → ⑧ → Lane 4 → ⑦ → 本 lane）に base 付け替えするため、Lane 4 merge 後は `:838`（商品名セル、badge 挿入先）の行番号が数行分ずれる可能性が高い。**Writer は Lane 4 merge 後の base 付け替え直後に `StocktakePage.tsx` の該当セルを再実測してから S6 を実装すること**（`:624` の候補行 badge は `:604-649` の別 JSX ブロックにあり、この wrap の影響範囲〈`:821-852`〉外なので再実測不要）
...
- **S3 展開行トグルクローズ + 自動展開の検索条件単位制限（R2-3）**: `ProductListTable.tsx` の行 `onClick` を、選択中行の再クリックで `onSelect(null)` を呼ぶよう変更（`onSelect` の型を `(productCode: string | null) => void` へ）。`StockInquiryPage.tsx` の `onSelect` wiring を `selected: code ?? undefined` へ。`useStockInquiry.ts` の自動展開 `useEffect`（`:126-132`）に、検索条件（`status`/`q`/`dept`/`page`）から導出した key を `useRef` で保持し、同じ key で一度自動展開したら再展開しない guard を追加する（UI-06a-D5、下記 Design Intent Trace）。**`StockInquiryPage.test.tsx` の既存 test 再検証必須（Plan Review round 1 P2）**: `:135` に page-level の 2 個目の自動展開 test（hook 単体の `useStockInquiry.test.tsx` とは別に page 結線を検証）、`:264-270` 付近に行クリックで展開する test があり、いずれも guard 追加後に無変更で pass することを確認する。`:385` 付近のコメント「単一件自動展開の selected 更新 call も混ざるため、q を変更した call を明示的に待つ」は、guard 追加で自動展開の発火回数自体が変わり得るため、この前提（複数 call が `onSearchChange.mock.calls` に混在する）が guard 追加後も成立するか再検証する。完了条件: `ProductListTable.test.tsx` に再クリックで選択解除される test、`useStockInquiry.test.tsx` に「同一条件で手動クローズ後は再自動展開しない」+「条件変化後は再度自動展開する」の対 test、`StockInquiryPage.test.tsx` の既存 3 箇所（`:135`/`:264-270`/`:385`）が無変更または前提再検証込みで pass する
