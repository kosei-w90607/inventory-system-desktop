...
**Stacked train（D-074、DEV_WORKFLOW「Stacked train」節）**: Lane 4 human-confirm tip `05ece3f` に単段 merge `495e42b` で積み直し済み（旧起点 `1a8ba62`、2026-09-06）。merge train 順序: Lane 5 → ⑧（native select 統一）→ Lane 4 → **⑦**（`agent/ui-conventions-batch`、PR #39、Phase: human-confirm。DSR-22 の badge 枠要件を interactive 操作枠へ narrow 化し、②分類 badge の枠を `--border-strong`〈3:1〉から `--border`〈tone 固有色可、3:1 対象外〉へ改定する）→ 本 lane（Plans.md ⑦/⑧/④ の記載を正とする）。本 lane は ⑦ merge 後にもう一度 main へ base 付け替えするため、実装時点で ⑦ の改定後 design-system 文書が canonical になっている前提で S6 の badge 仕様を書く（下記「起票時実測」参照）。Lane 4 merge で `docs/Plans.md` は auto-merge（④ 節と ⑨ bullet が非重複行のため conflict markers なし、両方の記載を維持）、`StocktakePage.tsx`/`StockInquiryPage.tsx` 等の行番号 drift は再実測済み（下記「起票時実測」Lane 4 drift 節）。今後の先行 lane squash merge 後は、旧 tip を保存してから最新 `origin/main` を 1 回だけ merge する単段 merge で base を付け替える（rebase しない）。付け替え後の merge delta が実装 file に触れる場合は、その delta を独立に再検証してから Phase を進める。
...
- 在庫照会の一覧テーブル（`ProductListTable`、「すべて」/「在庫少」/「在庫切れ」共通の単一 component、`StockInquiryPage.tsx:235（Lane 4 merge 後再実測、旧 `:224`）`）に取引先列が表示される。並び順は在庫少・在庫切れ（`status=stockout|low_stock`、`list_low_stock` 経由）でのみ 取引先名 昇順（取引先なしは最後）→ 同名内は在庫数 昇順 → 商品名 昇順に変わり、「すべて」（`search_products` 経由）は既存の `ORDER BY p.product_code` のまま変更しない
...
## 起票時実測（2026-09-06 起票、2026-09-06 Lane 4 human-confirm tip `05ece3f` へ単段 merge `495e42b` 後に行番号再実測。旧 worktree base `1a8ba62`。すべて本 packet 起草者が rg/bat/sed で再確認）
...
- **S1 filter-low-stock-list のリネーム + ソート追加**: `filter-low-stock-list.ts` の `filterLowStockList` を `filterAndSortLowStockList` へリネームし、フィルタ後に `supplier_name`（null 最後、非 null は昇順）→ `stock_quantity` 昇順 → `name` 昇順の安定ソートを追加する。呼び出し元 `useStockInquiry.ts:78` を更新。完了条件: `rg -c "filterAndSortLowStockList" src/features/stock-inquiry/hooks/useStockInquiry.ts` ≥ 1、かつリネーム漏れなし（Plan Review round 1 P3: `rg -c` はディレクトリ対象で 0 件時に無出力・exit 1 を返すため `=0` を直接比較しない。`rg -c "filterLowStockList" src/features/stock-inquiry ; echo "exit=$?"` を実行し `exit=1`〈無出力〉であることを確認する形にする）
- **S2 ProductListTable に取引先列を追加（両 view 共通、無条件）**: `ProductListTable.tsx` の列を 商品コード / 商品名 / 部門 / **取引先** / 状態 / 在庫数 / 売価（7 列）へ。取引先 null は `—`。展開行 `colSpan` を `6` → `7` へ更新。`ProductListTable` は「すべて」/「在庫少」/「在庫切れ」で条件分岐しない単一 component（`StockInquiryPage.tsx:235（Lane 4 merge 後再実測、旧 `:224`）`）のため、取引先列は `source` に関わらず両 view に無条件で出る。並び順は S1 のソートが `source === "low_stock"` の呼び出し経路（`useStockInquiry.ts` の `filterAndSortLowStockList`）にのみ適用され、「すべて」（`source === "search"`）は既存の `ORDER BY p.product_code` のまま。**既存 test 更新必須（Plan Review round 1 P1）**: `ProductListTable.test.tsx:127` の `it("REQ-301: 選択行の nextElementSibling が colSpan 展開行（td[colspan=6]、旧下部固定の混入 guard）")`（`:141` で `td[colspan="6"]` を assert）は `colSpan=7`化で壊れる。it 名の `colspan=6` 表記と `:141` の `td[colspan="6"]` を両方 `7` へ更新する（SC8 の「無変更で pass」対象からはこの test を明示的に除外する、下記 Matrix SC8 参照）。完了条件: `rg -Fc 'colSpan={7}' src/features/stock-inquiry/components/ProductListTable.tsx` ≥ 1 かつ `rg -Fc 'colSpan={6}' src/features/stock-inquiry/components/ProductListTable.tsx` = 0
- **S3 展開行トグルクローズ + 自動展開の検索条件単位制限（R2-3）**: `ProductListTable.tsx` の行 `onClick` を、選択中行の再クリックで `onSelect(null)` を呼ぶよう変更（`onSelect` の型を `(productCode: string | null) => void` へ）。`StockInquiryPage.tsx` の `onSelect` wiring を `selected: code ?? undefined` へ。`useStockInquiry.ts` の自動展開 `useEffect`（`:126-132`）に、検索条件（`status`/`q`/`dept`/`page`）から導出した key を `useRef` で保持し、同じ key で一度自動展開したら再展開しない guard を追加する（UI-06a-D5、下記 Design Intent Trace）。**`StockInquiryPage.test.tsx` の既存 test 再検証必須（Plan Review round 1 P2、Lane 4 merge 後に行番号再実測）**: `:175`（旧 `:135`）に page-level の 2 個目の自動展開 test（hook 単体の `useStockInquiry.test.tsx` とは別に page 結線を検証）、`:305-311`（旧 `:264-270`）付近に行クリックで展開する test があり、いずれも guard 追加後に無変更で pass することを確認する。`:425`（旧 `:385`）付近のコメント「単一件自動展開の selected 更新 call も混ざるため、q を変更した call を明示的に待つ」は、guard 追加で自動展開の発火回数自体が変わり得るため、この前提（複数 call が `onSearchChange.mock.calls` に混在する）が guard 追加後も成立するか再検証する。完了条件: `ProductListTable.test.tsx` に再クリックで選択解除される test、`useStockInquiry.test.tsx` に「同一条件で手動クローズ後は再自動展開しない」+「条件変化後は再度自動展開する」の対 test、`StockInquiryPage.test.tsx` の既存 3 箇所（`:175`/`:305-311`/`:425`）が無変更または前提再検証込みで pass する
- **S4 `StocktakeItemDetail` に `is_discontinued` を追加**: `stocktake_repo.rs:64` の struct へ `pub is_discontinued: bool` を追加し、`find_stocktake_item_by_code`（`:348-349,360-370`）と `list_stocktake_items`（`:537-539,548-558`）の両 SQL に `p.is_discontinued` を追加、struct 構築に `is_discontinued: row.get(N)?` を追加する（新規 JOIN 不要、SELECT 列追加のみ）。完了条件（Plan Review round 1 P1、旧オラクルを是正）: 旧オラクル `rg -c 'is_discontinued' stocktake_repo.rs ≥ 4` は起票時点で既に **10** 件ヒットしており vacuous（`ProductForStocktake` の既存 field 等、無関係な既存箇所を含むため）。実測ベースの baseline+delta 形に置き換える: (a) `rg -Fc 'p.is_discontinued' src-tauri/src/db/stocktake_repo.rs` は起票時点で **2**（`:1314,1329` の `assert!(p.is_discontinued)` 等、SQL とは無関係な既存 test 内のローカル変数 `p` へのフィールドアクセス。**Coordinator が実測して是正**: 発注書ドラフトの「= 2」は SQL 側 2 箇所のみを指すつもりだったが、この baseline 2 件と衝突するため、実装後の期待値は baseline 2 + 新規 SQL 2 = **4**）(b) `rg -Fc 'is_discontinued: row.get' src-tauri/src/db/stocktake_repo.rs` は起票時点で **1**（`:470` の `ProductForStocktake` 構築、既存）、実装後の期待値は既存 1 + 新規構築 2 = **3**
- **S5 bindings 再生成**: `cd src-tauri && cargo run --bin generate_bindings` を実行し `src/lib/bindings.ts` を再生成する。完了条件は AC6（機械 oracle: `git diff --numstat` の追加 1/削除 0 + インデント非依存の `is_discontinued: boolean,` fs literal、round 2 是正）参照。予期しない diff が出た場合は原因を特定してから進める
- **S6 StocktakePage.tsx に廃番 badge を追加 + 候補行 badge を同じ形へ揃える**: `StocktakePage.tsx:847`〈Lane 4 merge 後に再実測済み、旧 `:838`。下記「起票時実測」参照〉商品名セルへ `{item.is_discontinued ? <Badge variant="secondary" className="border-border">廃番</Badge> : null}` を追加する。icon は付けない（⑦ 改定後ルールで②分類の icon は任意、既存候補行の前例〈icon なし〉に揃える）。同じ commit で `StocktakePage.tsx:624` の既存候補行 badge（`<Badge variant="secondary" className="ml-2">廃番</Badge>`）にも `border-border` を追加し（`className="ml-2 border-border"` 等）、同一 file 内の 2 箇所を同じ枠仕様に揃える（見た目の不一致を残さない）。完了条件（Plan Review round 1 P3、`-Fc '...border-border"'` は末尾クオート依存で脆いため引用符なしの `-F 'border-border'` へ）: `rg -Fc 'border-border' src/features/stocktake/StocktakePage.tsx` = 2（新規箇所 + 候補行の是正箇所、`border-border-strong` はこの literal にも部分一致するが本 lane はどちらの箇所にも `-strong` を使わないため実質 2 で確定する）、かつ `rg -Fc 'border-border-strong' src/features/stocktake/StocktakePage.tsx` = 0（旧仕様の混入なし。起票時点でも既に 0 のため単独では非空 oracle にならない点に注意 — 上記 `= 2` の正の確認と対で運用する）
- **S7 docs 同期**: `58-ui-stock-inquiry.md` に UI-06a-D4（取引先列 + 並び順）/ UI-06a-D5（展開行トグルクローズ + 自動展開の検索条件単位制限）を §58.10 に追加し、§58.7 ProductListTable 節の列挙・colSpan 記述・§58.4 selected ライフサイクル節を更新する。**Plan Review round 1 P2 で追加実測、drift 箇所 3 件を明示**: `:255` の疑似コード `filterLowStockList(rows, params.q, params.dept, params.status)` を `filterAndSortLowStockList` へリネーム、`:498`「状態列追加後のインライン展開は `colSpan=6`。旧 `colSpan=5` は table 列数と不一致になるため regression test で防ぐ」を `colSpan=7`（旧 `colSpan=6` は不一致）へ、`:543` の test 一覧「nextElementSibling colSpan=6 guard（旧下部固定・旧 5 列混入検出）」を `colSpan=7` へ、それぞれ更新する（放置すると doc-consistency-check の drift 検出対象になる）。`73-ui-stocktake.md` に UI-10-D13（棚卸しリストの廃番 badge）を §73.3 に追加し、§73.6 一覧・フィルタ表へ「廃番 badge」行を追加する。**`docs/function-design/20-io-product-repo.md:751`（Plan Review round 1 P1）**: `StocktakeItemDetail構造体` の 9 field 列挙に `is_discontinued: bool（products.is_discontinued）` を追加する（列挙漏れは doc-consistency-check の DTO drift 検出対象）。両 file（58-ui/73-ui）の更新履歴に本 PR の 1 行を追加
...
- AC9: `StocktakePage.tsx:624` の既存候補行 badge も `border-border` を持つ（is_discontinued 表示 2 箇所が同じ枠仕様になる） — **Plan Review round 1 P2 是正**: `rg -c 'border-border-strong' ... = 0` は起票時点で既に 0（`--border-strong` sweep 未着手のため）で vacuous。正の oracle `rg -Fc 'border-border' src/features/stocktake/StocktakePage.tsx` = 2（新規箇所 + 候補行）を主とし、`border-border-strong` = 0 は旧仕様混入防止の補助チェックとして併記する
...
- Function / command / DTO: `docs/function-design/20-io-product-repo.md` §2.11 `list_stocktake_items`（`:751`、Plan Review round 1 P1 実測で `StocktakeItemDetail構造体` の 9 field 列挙を確認済み。`is_discontinued: bool` の追加が必須、S7 参照）
...
| UI-06a-D5 展開行トグルクローズ + 自動展開の検索条件単位制限（R2-3） | `ProductListTable.tsx` / `StockInquiryPage.tsx` / `useStockInquiry.ts` | `ProductListTable.test.tsx` / `useStockInquiry.test.tsx` / `StockInquiryPage.test.tsx` | AC-L3-3 |
...
- targeted tests: `filter-low-stock-list.test.ts`（ソート追加）/ `ProductListTable.test.tsx`（取引先列・colSpan・トグルクローズ）/ `useStockInquiry.test.tsx`（自動展開の検索条件単位制限）/ `StockInquiryPage.test.tsx`（`:175`/`:305-311`/`:425` の既存 page-level test 再検証、Plan Review round 1 P2、Lane 4 merge 後再実測済み）/ `stocktake_repo.rs` `#[cfg(test)]`（`is_discontinued` 両経路）/ `StocktakePage.test.tsx`（badge 表示）
...
| SPEC-STKSUP-D1 | S3 | `ProductListTable.test.tsx` / `useStockInquiry.test.tsx` / `StockInquiryPage.test.tsx` | トグルクローズ・自動展開制限 | vitest |
...
2026-09-06: **owner L3 = AC-L3-1 / AC-L3-2 / AC-L3-3 すべて PASS**（原文 = `docs/design-system/reference/2026-09-04-owner-l3-feedback-raw.md`「(d) PR #41 L3 結果 原文」、介入 2/3 消費）。AC-L3-3 は owner が自動展開 → 手動クローズまで確認、「勝手に開くことなければいい」= 同条件で再展開しない契約の確認として PASS。所感 1 件（展開行 detail の商品名上・ボタン下の余白が窮屈）は scope 外 → Plans.md Backlog へ起票（在庫照会 detail 余白）。Human Gate 完了、残り = Codex ロジックレビュー 1 回。
