...
- S10 `docs/backlog.md`: 後続 lane 3 件（runtime lane A / B、棚卸し画面 D1）を D11 の題と内容で起票し、41 行目の「`04-backbone.md` の token 表 min-height 40px」の参照を新しい置き場所へ直す。旧 04 の未了の作業（処分表 U12・U13）を 1 行で起票する。
...
- AC9 原則の統合: 04 の原則が 10 本前後（8〜12 本）で、旧番号対応表が旧 04 原則 1〜16 の 16 行を持つ。S2 の処分表の U1〜U13 が、それぞれの行き先（00・01・02・04・review-checklist・backlog）で引け、U9 の撤回が旧番号対応表の「撤回・置換」列にある。03 が原則を持たない出典の一覧になっている（`rg -n '核心4本柱|補助3原則' docs/design-system/03-philosophy.md docs/UI_TECH_STACK.md` が 0 件）。
...
| 最新・上書き件数の強調（③強調の badge） | 注意・確認（③強調の琥珀 pill、owner 採用 2026-08-20、枠色は owner 決定 2026-09-05） | 段 2 に数える（badge の塗りの pill で押すボタンでない。lane A の L3 までの例外、D2） | 琥珀の pill（`variant="default"` と `border-warning`） | lane A の L3 で試し（stone の pill と太字 = ふつう・補足の段 2 へ移す案。「最新」と②分類の「手動」を言い分けられるか。採れば 00 を直す） |
