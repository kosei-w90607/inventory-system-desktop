...
Backlog（`docs/Plans.md:135,148,164`）記載の 3 件（I-G1 sweep test の gitignore 非尊重 / T10 source 文字列 test の formatter 脆弱性 / PK4 section 抽出の `###` 打ち切り）を機械的是正として 1 lane にまとめる。`docs/Plans.md:136`（STATECAP 検査の stacked train 継承除外）は Backlog 自身が「設計非自明のため将来判断」と記す通り、本 packet は候補案 2 つとトレードオフのみ提示し owner / Coordinator 判断待ちとして Scope から切り離す（詳細は「STATECAP 継承除外 — 判断保留」節）。
...
- S1: `src-tauri/tests/import_internal_contract_test.rs` の `sweep_dir_for_tokens` が、swept root（`src-tauri/src` / `src-tauri/tests` / `src`）配下に出現しうる既知の gitignore 対象生成物（`target` ディレクトリ、`mutants.out*` ディレクトリ、`*.rs.bk` file、`src/routeTree.gen.ts`）を走査対象から除外し、それらの内部に禁止 token があっても偽陽性 fail しない。
...
- S1: 修正後も `target` / `mutants.out*` / `*.rs.bk` / `src/routeTree.gen.ts` のいずれかの内部 token が sweep 対象に残る、または既存の active surface（`src-tauri/src` 等の tracked file）への走査が誤って除外される（既存 defect 検出力の劣化）。
...
- STATECAP 検査の stacked train 継承除外（`docs/Plans.md:136`）: 下記「STATECAP 継承除外 — 判断保留」節の通り Scope 外。
- `.gitignore` の汎用パーサ導入（`ignore` crate 等の新規依存追加）: 既知の 4 パターンのみを明示 skip する最小対応に留める（下記 S1 参照、ponytail: 汎用パーサへの upgrade は swept root 配下に未知の gitignore パターンが実際に出現した時点で判断）。
...
## STATECAP 継承除外 — 判断保留（Scope 外、`docs/Plans.md:136`）
...
- **案 A: 計数の起点を `merge-base(origin/main, HEAD)` から自 lane の `Plan Commit`（Workflow State 記載 SHA）へ変更する**。Plan Commit は PK5 により「全実装 commit の祖先」であることが既に保証されており、stacked train でも自 lane の分岐点に一致する。
  ...
  - 欠点: `check-workflow-git.sh` は現在 git 履歴のみで完結し packet 内容（Markdown）を一切読まない。Plan Commit をパケット file から抽出する新しいパースパスが要る（`extract_workflow_field` 相当のロジックを bash 側に複製 or 共有化）。`Plan Commit` が `pending`（plan-gate 未到達）の間は計数不能になるため fallback 定義が必要。単一 lane（非 stacked）の既存計算結果と一致するかの回帰検証が要る。
...
owner / Coordinator の判断待ち事項: (1) 案 A・B のどちらを採用するか（あるいは現状の運用〈content commit 同乗〉のまま是正を見送るか）、(2) 案 A を採る場合、bash からの Markdown field 抽出をどこまで許容するか（新規結合の是非）。優先度は owner 判断（Backlog 記載を維持）。

## Scope

- **S1 `sweep_dir_for_tokens` に既知 gitignore パターンの skip を追加**（`src-tauri/tests/import_internal_contract_test.rs:252-276`）: `fs::read_dir` の再帰中に、ディレクトリ名が `target` または `mutants.out` 始まりの場合はそのディレクトリを再帰対象から除外し、file 名が `routeTree.gen.ts` または `.rs.bk` で終わる場合はその file を token 検索対象から除外する（`ignore` crate 等の新規依存は追加しない、既知 4 パターンの明示 skip のみ。ponytail: 汎用 `.gitignore` parser への upgrade は swept root 配下に未知パターンが実際に出現した時点で判断）。既存 dev-dependency `tempfile`（`Cargo.toml:53`）を使い、新規 test `test_sweep_dir_for_tokens_skips_known_generated_paths` を追加: tempdir 直下に (a) `target/` ディレクトリ内に禁止 token を含む file、(b) `mutants.out-tmp/` ディレクトリ内に禁止 token を含む file、(c) `routeTree.gen.ts` file に禁止 token を直接書き込み、(d) 通常の tracked-相当 file に禁止 token を書き込み、の 4 パターンを配置し `sweep_dir_for_tokens` を直接呼び出して `hits` を検証する（(a)(b)(c) は検出されない = 空、(d) は検出される = 非空、を同一 test 内で対にする。空集合 oracle 単独は使わない）。完了条件: `rg -Fc 'fn test_sweep_dir_for_tokens_skips_known_generated_paths' src-tauri/tests/import_internal_contract_test.rs` = 1（baseline 0）、既存 `test_active_sales_import_vocabulary_sweep_i_g1` は無変更で pass する
...
- `.gitignore` 汎用パーサの導入（S1 は既知 4 パターンの明示 skip のみ）
- `useUnsavedChangesWarning.test.tsx` の T10 以外の test、`UnsavedChangesDialog.tsx` 本体の実装変更（S2 は test file のみ）
- `extract_markdown_section` / `extract_markdown_h2_section` の一般契約変更、他 7 箇所の呼び出し元の切替（S3 は「次の行動」1 箇所のみ）
- `check-workflow-git.sh` の STATECAP 範囲判定そのものの実装（Scope 外、上記判断保留節）

## Acceptance Criteria

- AC1（S1）: `test_sweep_dir_for_tokens_skips_known_generated_paths` が `target/` / `mutants.out*/` / `routeTree.gen.ts` 内の禁止 token を検出せず、同一 test 内の通常 file 内の禁止 token は検出する（対で確認、空集合 oracle を避ける）— `cargo test --test import_internal_contract_test test_sweep_dir_for_tokens_skips_known_generated_paths` PASS
...
- compatibility checks: S1 既存 test 無変更 PASS（AC2）、S3 他 7 箇所の `extract_markdown_section` 呼び出し元が無変更で PASS（既存 doc-consistency-plan-packet.test.sh 全体 PASS で担保）
...
- S1: skip 対象が既知 4 パターン（`target`/`mutants.out*`/`*.rs.bk`/`routeTree.gen.ts`）に限定され、それ以外の tracked file への走査が誤って除外されていないこと（AC1(d) の非除外確認）
...
- S3: `extract_markdown_section "$plans_md" "次の行動"` の呼び出しが 1 箇所のみ存在し、それが `extract_markdown_h2_section` に置き換わっていること。他 7 箇所の呼び出し元（Trace Matrix / Acceptance Criteria / Test Plan 等）が無変更であること
...
- Non-scope に列挙した項目（STATECAP 継承除外、`.gitignore` 汎用パーサ、T10 以外の test、他 7 箇所の呼び出し元切替）が変更されていないこと
...
- 具体的な適用: S1 は `ignore` crate 等の新規依存を追加しない（既知 4 パターンの明示 skip のみ、ponytail rung 5「導入済み依存で済むか」を優先し既存 `tempfile` のみ使う）。S2 は正規化 1 行 + assertion 統合のみで、readFileSync 自体の置換や新規 helper 関数化はしない（呼び出し元 1 箇所のみで rule of three 未達）。S3 は既存 `extract_markdown_h2_section` を再利用し新規関数を書かない（ponytail rung 2「既に codebase にある」を最優先）
