...
- **S8 docs 同期**:
  - **(F1, Opus P1-1, CHANGE)** catalog `:48` の component gap 記述のうち現在形の gap 文「`actions` と `subtitle`（および説明セクション）は現状排他である」「…の計 5 画面が影響を受けている」は**単なる状態注記の追記ではなく書き換え対象**とする（反映済み・「何を・どの PR で・どう解消したか」の文へ全面改稿）。書き換え例: 「`actions` と `subtitle`（および説明セクション）の排他は本 PR（⑮）で解消済み: `PageHeader.tsx` の `actions` 分岐に `<h1>` + 条件付き `subtitle`/`description` を `<div className="space-y-1">` にまとめる root-cause fix を適用し、`SupplierManagementPage.tsx` の外側 sibling `<p>` 移行を含む計 5 画面で説明文/副題が正しく描画されるようになった（外側 `<header>` の class は不変のため `PageHeader.test.tsx:61-73` は green のまま）。呼び出し側で wrapper を都度書く使用パターンは不要になった。」同様に catalog `:191` の「`MovementTable.tsx:94` は現状 truncate のみで `title` を欠くため是正対象」を「`MovementTable.tsx:93-95` は本 PR（⑮）で truncate から折り返し（`whitespace-normal break-words`）+ `title` 属性へ是正済み、`MovementTable` 自身が全文確認手段を持つ」へ書き換える。オラクル（pair、更新履歴 `:994` 以降には対象文言自体が出現しないため素の `rg -Fc` で足りる、実測確認済み）: 旧文言 `rg -Fc "現状排他である" docs/design-system/02-component-catalog.md` = 0 かつ `rg -Fc "計 5 画面が影響" docs/design-system/02-component-catalog.md` = 0 かつ `rg -F "は現状 truncate のみで" docs/design-system/02-component-catalog.md | wc -l` = 0（起票時いずれも 1）。新文言側は `rg -Fc "本 PR（⑮）で解消済み" docs/design-system/02-component-catalog.md` ≥ 1 かつ `rg -Fc "本 PR（⑮）で" docs/design-system/02-component-catalog.md` ≥ 2（`:48`/`:191` 双方）
  - **(F13, Opus P3-3)** catalog `:191` はさらに独立した stale citation を持つ: `OperationLogsPage.tsx:522`（truncate+title precedent）は実測 `:533` に drift している（本 packet「起票時実測」S5 節で確認済み）。この citation も `:522`→`:533` へ訂正する。オラクル: `rg -Fc "OperationLogsPage.tsx:522" docs/design-system/02-component-catalog.md` = 0（起票時 1）かつ `rg -Fc "OperationLogsPage.tsx:533" docs/design-system/02-component-catalog.md` ≥ 1
  ...
  - **(F9, Opus P2-7)** `01-decision-rules.md:435`（`InventoryRecordsPage.tsx:336-343`/`:342`〈記録日時〉/`:339`〈代表商品〉）と `:443`（`InventoryRecordsPage.tsx:336-343`）の anchor は既に実測 `:349-359`/`:357`/`:354` へ drift しており（本 packet「起票時実測」S4 節）、S4 の列撤去でさらに 1 列分ずれる（記録ID列撤去後は後続列の head 行番号がそれぞれ 1 つ若返る）。Writer は実装完了後の実測行番号（列撤去後の実際の head 行、`rg -n` で再実測）へこれらの引用を更新する（正確な新番号は実装 diff 確定後にのみ判明するため plan 段階では確定させない）。オラクル: `rg -Fc "InventoryRecordsPage.tsx:336-343" docs/design-system/01-decision-rules.md` = 0（起票時 2〈`:435`,`:443`〉、更新履歴 `:481` 以降には出現しないため素の `rg -Fc` で足りる）。新引用が実装後の実際の行番号と一致することは Final Review で `rg -n` 実測により reviewer が確認する（pre-implementation の時点で正確な新番号を確定できないため機械 oracle 化できず、Writer 自己検証 + Final Review 実測に依存、Residual Test Gap）
  ...
  - 完了条件: 上記各オラクルに加え、`rg -Fc "記録IDを含む 8 列" docs/design-system/01-decision-rules.md` = 0（是正後）、`rg -Fc "toLocaleString" docs/function-design/58-ui-stock-inquiry.md` ≥ 1（起票時 0）
...
- AC9: docs 側 — `rg -Fc "記録IDを含む 8 列" docs/design-system/01-decision-rules.md` = 0（是正後、起票時 1）。**F1（Opus P1-1）是正、pair oracle**: catalog `:48`/`:191` の現在形 gap 文が書き換えられている — `rg -Fc "現状排他である" docs/design-system/02-component-catalog.md` = 0 かつ `rg -Fc "計 5 画面が影響" docs/design-system/02-component-catalog.md` = 0 かつ `rg -Fo "は現状 truncate のみで" docs/design-system/02-component-catalog.md | wc -l` = 0（起票時いずれも 1、**F11 是正**: scalar count）。かつ `rg -Fc "本 PR（⑮）で" docs/design-system/02-component-catalog.md` ≥ 2（反映済み文言が :48/:191 双方に追加されている）。**F13（Opus P3-3）追加**: `rg -Fc "OperationLogsPage.tsx:522" docs/design-system/02-component-catalog.md` = 0（起票時 1）かつ `rg -Fc "OperationLogsPage.tsx:533" docs/design-system/02-component-catalog.md` ≥ 1。**F9（Opus P2-7）追加**: `rg -Fc "InventoryRecordsPage.tsx:336-343" docs/design-system/01-decision-rules.md` = 0（起票時 2）— 新引用の実装後実測行との一致は Final Review の `rg -n` 実読で確認（machine oracle 化不能、Residual Test Gap）。`doc-consistency-check.sh` の ERROR 0
