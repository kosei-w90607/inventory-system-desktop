...
- AC-L3-6（owner Windows native L3、**F15 追加**）: `ManualSalePage.tsx`/`DisposalPage.tsx`/`ReturnExchangePage.tsx` の入力行テーブルで「現在庫」列（数値のみ）と「単位」列（個/cm）が重複表示にならず、単位列が数量列の直後にあって金額・理由の後ろに来ない（**Gated Amendment 2**）ように自然に見える
