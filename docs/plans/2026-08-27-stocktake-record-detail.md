...
4. bindings: `cargo run --bin generate_bindings` 再生成（`getStocktakeRecord` / `StocktakeRecordDetail` / `StocktakeRecordDetailItem` / `StocktakeStatus` union。既存 `Stocktake` 型と既存 stocktake command 4 本の wire は不変 = diff 追加のみ）
5. route: **`stocktake.tsx` の layout route 化**（`<Outlet />` のみを render、`csv-import.tsx` と同型）+ **`src/routes/stocktake/index.tsx` 新設**（`StocktakePage` を index route へ移設）+ `src/routes/stocktake.records.$stocktakeId.tsx` 新設 + `npm run generate:routes`。既存 `/stocktake` 作業画面は index route で従来どおり描画されることを runtime route test で固定する（T16。4b gated Amendment 1 の教訓を最初から Scope 化）。`validateSearch` の `returnTo` pattern（`z.string().max(500).optional().catch(undefined)`）は既存 5 詳細 route と同形で踏襲
...
8. D-052 契約変更（C23 として採番予約。実装時に live `invalidation-contract.ts` の最終 C 番号を再実測して確定 — PR #86 WER の番号衝突教訓）: `stocktakeComplete` と `stocktakeCountUpdate` へ `queryKeys.inventoryRecords.stocktakeDetailRoot()` を追加。導出（UI_TECH_STACK §2.5 table.column）: 両 mutation は本 query が読む stocktakes / stocktake_items / inventory_movements を書く。`stocktakeStart` は新規 header/items の作成のみで既存詳細の読取り対象を変更しないため追加しない。**広域 prefix `inventoryRecords.root()`（4b の csvImportRollback 方式）は不採用** — `stocktakeCountUpdate` は商品 1 件入力ごとの高頻度 mutation で、無関係 list/detail への refetch collateral が過大（D-052 過剰禁止原則。rollback のような低頻度 mutation とは条件が異なる）。独立転記 oracle test（`src/test/invalidation-oracle.ts`）の追随 + production-only mutation 感度の再実測込み
...
- 既存 stocktake command 5 本（get_active / start / get_items / update_count / complete）の wire 変更（なし）
...
- AC4: `src/lib/bindings.ts` diff に `getStocktakeRecord` / `StocktakeRecordDetail` / `StocktakeStatus` が追加され（`rg -c "getStocktakeRecord" src/lib/bindings.ts` ≥ 1）、`cargo run --bin generate_bindings` 再実行で clean diff。既存 `Stocktake` 型と既存 stocktake command 4 本の wire 行は不変（diff が追加のみ）
...
- AC8: `/stocktake` 直接進入で既存棚卸し作業画面（`StocktakePage`）が従来どおり描画される runtime route test（layout + index 再構成の回帰防止）
...
- AC10: invalidation 独立転記 oracle test が `stocktakeComplete` / `stocktakeCountUpdate` の新集合と順序非依存・重複検出付き完全一致
...
| D-052 C23（予約） | UI_TECH_STACK §2.5 | D-052 | `stocktakeComplete` / `stocktakeCountUpdate` へ `stocktakeDetailRoot()` prefix。広域 `inventoryRecords.root()` は countUpdate の高頻度性から collateral 過大で不採用。`stocktakeStart` は読取り対象を変更せず追加しない | `invalidation-contract.ts` + oracle test | AC10 / T14 |
...
| Lifecycle / retry | in_progress → completed の状態遷移表示、count 更新中の詳細 cache 鮮度（D-052 C23）を State Lifecycle Matrix でカバー | Test Matrix |
...
| Scope 5: `stocktake.tsx` layout 化 + `stocktake/index.tsx` 移設で既存作業画面が従来どおり `/stocktake` で描画される | route files（layout + index） | T16 / AC8（runtime route test） | L3 で作業画面到達も一瞥確認 |
...
| D-052 C23（予約）: stocktakeComplete / stocktakeCountUpdate へ stocktakeDetailRoot() 追加 + 独立転記 oracle 完全一致 | `invalidation-contract.ts` + oracle test | T14 / AC10 | — |
| D-052: stocktakeStart 非追加の導出記録 | PR body | — | 導出根拠を PR body 記録 |
...
- compatibility checks: 既存 stocktake command 5 本の wire 不変（bindings diff が追加のみであること）、既存 5 詳細画面 test green、既存 `/stocktake` 作業画面の runtime 描画維持
...
- D-052 C23 導出の正確さ（欠落と過剰の両方向。stocktakeStart 非追加と root() 不採用の根拠を含む）
...
- stocktakeComplete / stocktakeCountUpdate 成功時、本詳細 query は D-052 SSOT 経由で invalidate される
...
| D-052 C23 | Scope 8 | T14 / AC10 | 導出正確さ | PR body（stocktakeStart 非追加含む） |
