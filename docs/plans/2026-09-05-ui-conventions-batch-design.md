...
- AC14: `git diff --name-only 07302b5..HEAD -- src` の出力が `src/styles/globals.css` の 1 行のみ、かつ `git diff 07302b5..HEAD -- src/styles/globals.css` の追加行が `:root` の `--success-border` / `--success-strong` 2 行 + `@theme inline` の `--color-success-border` / `--color-success-strong` 2 行の計 4 行（削除行 0）に限られる（Gated Amendment 1 で「src 0 件」から改訂、Final Review Opus P2-1 で alias 2 行を追加: Tailwind 4 は `@theme` からのみ utility を生成するため alias が無いと `text-success-strong` 等が無効になる。既存消費者 `HomePage.tsx:66` / `BackupRestorePage.tsx:358` / `PluExportPage.tsx:521,567` はこの欠落で現在無効）。
