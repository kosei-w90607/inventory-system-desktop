...
- **`stickyHeader` と page root の高さ連鎖はセット（Plan Review round 1 P2-7、Opus）**: この box構造は `ProductListPage.tsx` 側が `PageShell` へ `flex h-full min-h-0 flex-col` を渡すことに依存する。今後 `stickyHeader` を他画面へ配線する際（Non-scope、後続 lane）、page root がこの高さ連鎖を渡さないまま `stickyHeader` だけ付けると、`h-full` が auto に落ちて box の `overflow-auto` が無効化され、sticky も pager も静かに壊れる。Writer は `ListShell.tsx` 冒頭に「`stickyHeader` は page root の高さ連鎖（`flex h-full min-h-0 flex-col` を `PageShell` へ渡すこと）とセット」である旨のコメントを追加し、既存の fs scan test `src/test/page-root-pageshell-sweep.test.ts` に「`stickyHeader` を渡す Page は `PageShell` に `h-full`/`flex-col` 系 class を渡している」ことを確認する 1 ケースを足す（新規 prop は増やさない）。
...
- **適用側（`app-router.ts` を次の 6 箇所すべてで、`main` 固定から動的解決へ変更する）**: `document.querySelector('[data-list-scroll-container]')` の有無で「箱があれば id `"products-list"`・無ければ `"main"`」を返す resolver を新設し（`MAIN_SCROLL_SELECTOR` 定数〈`:10`〉は resolver の fallback 値として残す）、次に適用する:
  - `:28` `pruneScrollRestorationEntries` の allowlist を `[data-scroll-restoration-id="main"]` 単独から `main`/`products-list` の 2 selector へ拡張する（削除対象から両方を除外——DSR-17 (j) 同期）
  - `:39` `applyMainNavScroll`（分類④、主ナビゲーション遷移の先頭表示）の対象 selector を resolver の返す値へ（分類④が箱を対象にする、round 1 P2-3）
  - `:50` `scrollToTopSelectors` を `[MAIN_SCROLL_SELECTOR, '[data-scroll-restoration-id="products-list"]']` の 2 selector へ（cache miss 時の先頭 scroll fallback を両対象に効かせる）
  - `:74` の `const main = document.querySelector<HTMLElement>(MAIN_SCROLL_SELECTOR)` を resolver の返す selector へ
  - `:80-83` `getElementScrollRestorationEntry(appRouter, { id: "main", ... })` の `id` を resolver の返す id（`"main"` | `"products-list"`）へ
  - `:84` `const savedScrollTop = entry?.scrollY` に加え `const savedScrollLeft = entry?.scrollX` を読む（保存側は `scroll-restoration.js:85` で `scrollX`/`scrollY` を両方 cache するため、横スクロールも復元対象にする——round 1 P2-4、AC-L3-1「同じ位置に戻る」を縦横とも満たす）。`:107` の `main.scrollTop = savedScrollTop` の直後に `if (Number.isFinite(savedScrollLeft)) main.scrollLeft = savedScrollLeft;` を追加する
  - `:89`（`savedScrollTop <= 0` 早期 return）・`:91`（`main.scrollTop >= savedScrollTop`）・`:105`（`main.scrollHeight < savedScrollTop + main.clientHeight`、**`scrollTop` ではなく `scrollHeight`/`clientHeight` を参照する行**——round 1 P2-1、Sonnet: `:233` 旧文言が `:105` を「`scrollTop` 参照箇所」と誤記していたため訂正、`scrollTop` 参照は `:91`/`:107` の 2 箇所のみ）は縦方向の可視性ゲートのみを行い、横方向専用のゲートは追加しない（table の列幅は初回描画で確定し、`MutationObserver` が監視する非同期の行追加〈縦方向〉と異なり幅が事後に変化しない前提のため）
...
- `app-router.test.tsx`（既存拡張）: 箱基準の resolver が `pruneScrollRestorationEntries`/`applyMainNavScroll`/`scrollToTopSelectors`/`getElementScrollRestorationEntry` の 4 箇所すべてで一貫して「箱があれば `products-list`・無ければ `main`」を返すこと、復元時に `scrollTop` と `scrollLeft` の両方が適用されること
- `ProductListPage.scroll-restoration.test.tsx`（新規）: 本番 `createAppRouter` + 実 routeTree で、商品一覧の箱の scroll 位置（縦・横）が「一覧→詳細→戻り」で復元されることを確認。幾何 stub は `src/lib/app-router.test.tsx:132-155` の `installNativeScrollClamp`（`Object.defineProperty` で `clientHeight`/`scrollHeight`/`scrollTop` を stub）を踏襲する——`OperationLogsPage.scroll-restoration.test.tsx` の `main.scrollTop===0` oracle は幾何 stub を持たず、happy-dom 既定の `scrollHeight`/`clientHeight` = 0 のままでは `applyWhenScrollable`（`app-router.ts:105`）を永久に通過しないため参考にならない（round 1 P2-6、Opus）
- Matrix mutants: box の class が落ちる／`data-list-scroll-container`/`data-scroll-restoration-id` 属性が落ちる／解決 helper が属性ありでも `main` にフォールバックしてしまう／`Pagination` が box の中に入ってしまう／横方向の復元（`scrollLeft`）が適用されない
