...
- **適用側（`app-router.ts` の 6 箇所のうち、起動時に一度だけ評価される 2 箇所は静的 2-selector 配列のまま、`onRendered` 発火のたびに評価される 3 箇所だけを resolver 呼び出しへ変更する——round 2 是正、Opus P2、旧文言「6 箇所すべてを動的解決へ」は誤り）**: `document.querySelector('[data-list-scroll-container]')` の有無で「箱があれば id `"products-list"`・無ければ `"main"`」を返す resolver を新設し（`MAIN_SCROLL_SELECTOR` 定数〈`:10`〉は resolver の fallback 値として残す）:
  ...
  - **forced-top 経路にも横方向のリセットを揃える（round 2 是正、Opus P3）**: `:76` の `main.scrollTop = 0`（`forceScrollTop` flag 経路）を `main.scrollTo({ top: 0, left: 0 })` へ変更し、`page-scroll.ts:1` の `PAGE_TOP_SCROLL_OPTIONS = { top: 0, behavior: "smooth" }`（`page-scroll.ts:47` の `main.scrollTo(PAGE_TOP_SCROLL_OPTIONS)` で使用）に `left: 0` を追加する。理由: perPage 変更時に箱を横スクロールしたまま先頭へ戻すと、縦だけ戻り横位置が残ったままになる
...
- Matrix mutants: box の class が落ちる／`data-list-scroll-container`/`data-scroll-restoration-id` 属性が落ちる／解決 helper が属性ありでも `main` にフォールバックしてしまう／`Pagination` が box の中に入ってしまう／横方向の復元（`scrollLeft`）が適用されない（幅 clamp 付き harness で検出、round 2 是正）／`:76`/`page-scroll.ts:47` の forced-top が `left: 0` を含まない（round 2 是正）
...
- `min-w-36`（th/td 双方、無害な belt）は残す——div wrapper が主機構として幅を確定するため必須ではないが、`min-w`/`max-w` 単体の不確実性を懸念する読み手向けの保険として残し、削除しない（コストゼロ、削除の動機も無い）。`max-w-36` は div wrapper と機能が重複し `td`/`th` 自体への直接効果が不確実なため付けない。
...
- **行高の近似値（round 1 是正、Sonnet P2 + Opus P2。ponytail: 実測ではなく class からの概算、L3 run 3 で実データとズレていれば数値のみ補正する）**: thead th の高さは `table.tsx:61` `h-10` = 2.5rem。tbody 行（JAN あり、2 行セル）は `text-sm` 商品名 1 行（line-height 1.25rem）+ `text-xs` JAN code 1 行（line-height 1rem）+ `table.tsx:74` `p-2`（上下 0.5rem×2 = 1rem）+ border 1px（≈0.0625rem）= **1 行 ≈ 3.31rem** と近似する。箱の `max-h` = 2.5rem（thead）+ 50 × 3.31rem（tbody 50 行）≈ 168rem、`max-h-[168rem]` とする。**この「50 行」は目安であり固定境界ではない（Opus P2）**: JAN を持たない商品（JAN code 行が無く 1 行セル、商品名 1 行 + p-2 + border ≈ 2.25rem）が混在すると 1 画面に収まる実際の行数は 65〜90 行程度まで伸び得る。この数値は近似であり、L3 run 3 で「50 行より早く/遅く箱が scroll し始める」所感があれば構造を変えず定数のみ補正する。
- **CSS 制約（Human Gate に明記、owner が run 3 で判断する材料）**: 箱が横 scroll 容器（`overflow-auto`）である限り、CSS の overflow 計算規則により縦方向の computed 値も `auto` になる（起票時実測「識別列固定の実装方式」節で確認済みの一般規則の再適用）。50 行以下（箱が内部縦 scroll を必要としない状態）では箱自身は縦に scroll しないため、`sticky top-*` の thead は「箱の scrollport に対しては」常に先頭にあり続けるが、`<main>`（ページ全体）を縦に scroll したときは箱ごと画面外へ流れる。したがって GA1 で PASS した「縦に滑らせても見出しが箱の上に残り、下のページ送りは常に見える」という owner 期待像は、**51 行超（箱が内部 scroll する状態）でのみ再現され、50 行以下（`<main>` が page scroll する状態）では見出しごと流れる**。この trade-off を L3 run 3 で owner に明示し、(a) このまま採用 / (b) GA1 の「箱が常に唯一の scroller」へ差し戻す / (c) 折衷（perPage が閾値以下のときだけ箱を clamp しない等）のいずれかを owner が選ぶ。本 Gated Amendment はこの試作を Scope とし、owner 判定確定前に DSR-17/DSR-22/catalog ⑯ の正本記述は改訂しない（上記 GA1/GA2 節に付した置換予定注記のみ、下記 Registration Obligations 参照）。
- **Scroll restoration の再設計（round 1 是正、Opus P2。GA1 の単一 target 設計を 2 target 化する契約を明記——実コードは Writer が GA3b 実装時に書く）**: GA1 の `resolveScrollTarget()`（`app-router.ts:20-25`）は「箱があれば箱・無ければ main」という択一で、商品一覧では常に箱だけを縦横の復元・先頭化対象にしていた。GA3b では商品一覧は「`<main>`（縦、51 行以下で主 scroller）」と「箱（横は常に、51 行超のときは縦も）」の**両方**を独立に復元・先頭化する必要がある。保存側（TanStack Router 内蔵の cache）は変更不要——`data-scroll-restoration-id="main"`/`"products-list"` の 2 entry は GA1 で既に両方 cache 対象、`SCROLL_RESTORATION_ALLOWLIST`（`app-router.ts:18`）も不変。
  ...
  - **(ii) armed/disarm 条件表（round 1 是正、Opus P2 の指摘「配線先が不明瞭」への対応）**:
    ...
    |---|---|---|---|---|
    | `<main>` | 縦 | 保存 `scrollY > 0` かつ `main.scrollTop < scrollY`（GA1 の既存 armed 判定と同型、対象を箱から main へ戻す） | `applyWhenScrollable` で `main.scrollHeight >= scrollY + main.clientHeight` を満たしたとき `main.scrollTop = scrollY` | 51 行以下では main が主 scroller、51 行超では箱が縦を担うため main の縦は動かない想定（`main` 自体の高さが箱の `max-h` 分しか伸びないため） |
...
- `ListShell.test.tsx` 拡張（GA3b-3）: 箱要素が `max-h-[168rem]`（最終決定値）と `overflow-auto` を持ち、`min-h-[12rem]`/`flex-1` を持たない
...
**AC-GA3b（run 3、owner Windows native L3、判定材料）**: 箱が既定で 50 行分の高さまで伸び、それ以下では `<main>` の page scroll で一覧を見渡せる。51 行以上（perPage 100 等）では箱が内部縦 scroll に切り替わる。ページ scroll 中に列見出しが流れる／箱 scroll 中は残る、という trade-off を確認したうえで owner が (a)/(b)/(c) のいずれかを選ぶ。

**Registration Obligations 追加**: `PRODUCT_CODE_COLUMN_WIDTH_CLASSES`（`ListShell.tsx`、GA3a）は新規 DOM 属性ではなく既存パターン（`IDENTITY_COLUMN_CLASSES` と同型のリテラル class 配列）の追加 export のため、新規登録義務は無い。GA3b は新規属性を追加しない（既存 `data-list-scroll-container`/`data-scroll-restoration-id` を再利用する）。DSR-17/DSR-22/catalog ⑯ の正本改訂は owner 判定（run 3）確定後に別途行う（本 Gated Amendment の Scope には含めない、上記 GA1/GA2 節への置換予定注記のみ本 commit で反映済み）。
...
- AC-L3-4（owner Windows native L3、**run 3 新設、Lane 4 Gated Amendment 3 GA3b、判定材料——PASS/FAIL の合否条件ではなく owner が方針を選ぶための所見**）: 箱が既定で 50 行分の高さまで伸び、それ以下では `<main>` の page scroll で一覧を見渡せる。51 行以上（perPage 100 等）では箱が内部縦 scroll に切り替わる。ページ scroll 中に列見出しが流れる／箱 scroll 中は残る、という trade-off を確認したうえで owner が (a) このまま採用 / (b) GA1 の箱内 scroll 常時へ差し戻す / (c) 折衷、のいずれかを選ぶ。**（round 1 是正、Opus P1-1）**: 商品一覧の既定 perPage は 100（`search.ts:133`）のため、page scroll 側を見るには表示件数を 50 に切替える操作が要る。既定 100 では常に箱 scroll 側のみを見ることになる——owner へこの手順を案内し、既定値を 50 へ変更するかどうかは run 3 で owner が判断する別選択肢として提示する
