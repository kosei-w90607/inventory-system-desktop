...
- **S3 上部 `PaginationSummary` の 7 画面ロールアウト + 既存重複文言 test の是正**（`ListShell` を使わない全画面、L4-D5。各画面の「結果あり」分岐内、`<Table>` 直前に 1 行追加するだけの機械的変更。Plan Review round 2 で既存 test 破綻の是正を追加）:
  ...
  - **既存 test 是正（round 2/3、必須。round 3 で S2 の `text-base`→`text-sm` 変更が壊す箇所を追加、対象は 5 file 9 箇所へ拡大）**:
    - `IntegrityCheckPage.test.tsx:414`、`OperationLogsPage.test.tsx:280,292,419,431`、`StockInquiryPage.test.tsx:526` の `screen.getByText(<件数文言>)`（6 箇所、round 2 是正）を、上部/下部の重複を許容する形（`screen.getAllByText(...)` で `length` 検査、または `within(<下部 Pagination の container>)` で片方に限定）へ書き換える
...
- AC6: 表示件数 `Select` が 8 画面すべてで filter/toolbar 枠内の最後尾 — `StocktakePage.test.tsx:1055`（SC10 rewrite）、`PriceRevisionFilters.tsx`/`PriceRevisionPage.tsx`（配線先の変更 + `PriceRevisionPage.test.tsx:660-669` SC9a の pass 維持）。他 5 画面は既存 DOM 順が変わらないことを既存 test の pass 維持で確認（回帰）
...
- compatibility checks: `ListShell.test.tsx` 既存 17 it のうち S9 と無関係な it が pass のまま（AC9、`:96,98,107,119` は S2 の影響で是正対象）、`Pagination.test.tsx` の「0 件」表示 test は仕様変更として更新（削除ではない）、`StocktakePage.test.tsx:1042`/`:1055`、`PriceRevisionPage.test.tsx:660-669`、`IntegrityCheckPage.test.tsx:414`、`OperationLogsPage.test.tsx:280,292,419,431`、`StockInquiryPage.test.tsx:526`、`ProductListPage.test.tsx:632-637`、`Pagination.test.tsx:71` の既存 test 是正
