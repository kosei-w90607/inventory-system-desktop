...
- 撤去（round 1 是正、Opus P2、コメントの是正漏れ）: `ListShell.tsx:12-18`（`stickyHeader` は page root の高さ連鎖とセット、と説明する GA1 冒頭コメント）と `ProductListPage.tsx:259-265`（`overflow-hidden` を付けない理由 + `PageHeader` の `shrink-0` 前提を説明する GA1 コメント）は、上記のクラス撤去後は事実と矛盾する説明文になるため、内容を GA3b の実装（page root の高さ連鎖は不要になった旨）に合わせて書き換えるか削除する。完了条件: `rg -Fc "高さ連鎖" src/` = 0（起票時点の baseline = 4: `ListShell.tsx:12`〈コメント〉、`ProductListPage.tsx:259`〈コメント文中の「GA1 高さ連鎖の前提」〉、`src/test/page-root-pageshell-sweep.test.ts` 2 箇所〈GA1e の説明コメント、下記 GA3b-5 で反転/削除する同じ節）
- **行高の近似値（round 1 是正、Sonnet P2 + Opus P2。round 2 是正、Opus P3: 箱内 summary 帯を計算に追加。ponytail: 実測ではなく class からの概算、L3 run 3 で実データとズレていれば数値のみ補正する）**: thead th の高さは `table.tsx:61` `h-10` = 2.5rem。箱内の summary 帯（`ListShell.tsx` の `sticky top-0 h-10`、`topSummary` 使用時、`ProductListPage.tsx:289` 相当で活性化）も同じ `h-10` = 2.5rem。tbody 行（JAN あり、2 行セル）は `text-sm` 商品名 1 行（line-height 1.25rem）+ `text-xs` JAN code 1 行（line-height 1rem）+ `table.tsx:74` `p-2`（上下 0.5rem×2 = 1rem）+ border 1px（≈0.0625rem）= **1 行 ≈ 3.31rem** と近似する。箱の `max-h` = 2.5rem（summary 帯）+ 2.5rem（thead）+ 50 × 3.31rem（tbody 50 行）≈ 170.5rem、`max-h-[170rem]` とする。**この「50 行」は目安であり固定境界ではない（Opus P2）**: JAN を持たない商品（JAN code 行が無く 1 行セル、商品名 1 行 + p-2 + border ≈ 2.25rem）が混在すると 1 画面に収まる実際の行数は 65〜90 行程度まで伸び得る。この数値は近似であり、L3 run 3 で「50 行より早く/遅く箱が scroll し始める」所感があれば構造を変えず定数のみ補正する。
- **CSS 制約 + Human Gate semantics（round 2 是正、Opus P1。round 1 是正時点の「51 行超では GA1 の期待像が再現される」は誤りだったため撤回する）**: `max-h-[170rem]`（≈2720px、下記「行高の近似値」参照）は `<main>` の実 viewport（通常 800〜1200px 程度）の 2 倍以上あり、箱が実際に 50 行近く埋まった状態では常に viewport よりはるかに高くなる——これは行数に依らない一般論ではなく、「50 行分の高さ」という owner 要求そのものが必然的にもたらす帰結である。箱が横 scroll 容器（`overflow-auto`）である限り、CSS の overflow 計算規則により縦方向の computed 値も `auto` になる（起票時実測「識別列固定の実装方式」節で確認済みの一般規則の再適用）ため、`sticky top-*` の thead は「箱自身の scrollport」を基準に固定される。箱の高さが viewport を超える状況では、`<main>` を縦に scroll するとその過程で箱の上端が viewport の外へ出て行き、thead は箱と一緒に画面外へ流れる。**これは 51 行超（箱が内部 scroll に切り替わった状態）でも変わらない**——箱自身の高さは常に `max-h` 分（≈2720px）で固定されており、内部 scroll が発生していても箱の上端が viewport の外に出ていれば thead は見えない。したがって「51 行超では GA1 の期待像（見出しが常に見える）が再現される」という主張は成立しない（判定対象外とする）。同様に、下部 `Pagination` は箱の**外**（sibling）に置かれているため、箱が viewport よりはるかに高い間は、一覧の下端（Pagination）へ到達するには箱の高さ分（≈2720px）を page scroll する必要がある——これは 50 行以下・51 行超のどちらでも共通の帰結である。**Human Gate の判定材料（正直な帰結、owner が run 3 で確認する）**: (1) 列見出し（thead）は page scroll 中は視界から流れる（箱の scrollport 基準の sticky は `<main>` の scroll には追従しない）。(2) 下部ページ送りへは一覧の下端まで page scroll して到達する（箱が sibling のため）。(3) 箱自身の内部 scroll は実データが 50 行（近似値、行の内容により 65〜90 行相当まで前後）を超えたときのみ発生する。**owner の 3 択（run 3）**: (a) 本試作のままこの帰結を許容する / (b) GA1 の「箱が常に唯一の scroller」（見出し・ページ送りが常に見える）へ差し戻す / (c) 箱の `max-h` を viewport 基準にする（例 `max-h-[calc(100vh-14rem)]`、GA1 に近い体験——箱が常に画面内に収まるため見出し・ページ送りが page scroll 中も視認しやすくなるが、50 行を常時表示しきる owner の当初要望とは trade-off する）。本 Gated Amendment はこの試作を Scope とし、owner 判定確定前に DSR-17/DSR-22/catalog ⑯ の正本記述は改訂しない（上記 GA1/GA2 節に付した置換予定注記のみ、下記 Registration Obligations 参照）。
- **Scroll restoration の再設計（round 1 是正、Opus P2。GA1 の単一 target 設計を 2 target 化する契約を明記——実コードは Writer が GA3b 実装時に書く）**: GA1 の `resolveScrollTarget()`（`app-router.ts:20-25`）は「箱があれば箱・無ければ main」という択一で、商品一覧では常に箱だけを縦横の復元・先頭化対象にしていた。GA3b では商品一覧は「`<main>`（縦、51 行以下で主 scroller）」と「箱（横は常に、51 行超のときは縦も）」の**両方**を独立に復元・先頭化する必要がある。保存側（TanStack Router 内蔵の cache）は変更不要——`data-scroll-restoration-id="main"`/`"products-list"` の 2 entry は GA1 で既に両方 cache 対象、`SCROLL_RESTORATION_ALLOWLIST`（`app-router.ts:18`）も不変。
  ...
  - **(iii) 完了条件**: `page-scroll.test.ts` に 2 target ケース（main と箱の両方が存在するとき `scrollPageToTop()` が両方へ `scrollTo` を呼ぶ）を追加、`app-router.test.tsx` に main が縦へ・箱が横（+ 縦、51 行超）へそれぞれ独立に scroll された状態から両方が復元されるケースを追加する（下記 GA3b-7 参照）。
...
- `ListShell.test.tsx` 拡張（GA3b-3）: 箱要素が `max-h-[170rem]`（最終決定値）と `overflow-auto` を持ち、`min-h-[12rem]`/`flex-1` を持たない
