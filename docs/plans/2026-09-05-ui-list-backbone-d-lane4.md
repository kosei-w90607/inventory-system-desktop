...
- Human Gate: owner Windows native L3 run 3（run 1 は 2026-09-06 に AC-L3-1/AC-L3-3 所感で FAIL、Gated Amendment 1/2 起票・run 2 で FAIL 1 件 + 設計所感、Gated Amendment 3 起票済み）— AC-L3-1（run 2、Gated Amendment 1、**run 3 で GA3a の是正を確認**）: 窓を狭めて表が枠より広い状態で横に滑らせると検索ツール・商品コード・商品名・見出しは動かず部門から右だけが滑る、縦に滑らせても見出しが箱の上に残り下のページ送りは常に見える、「修正」から戻ると同じ位置に戻る、**最狭幅×2 ページ目以降でも部門列の頭が固定列の下に隠れない、逆方向（横 scroll 中）でも商品名列が商品コード列の右端を覆わない**（GA3a、下記 Gated Amendment 3 参照）, AC-L3-2 ページ送り上下（複数ページの画面 1 つ + 単一ページの画面 1 つ）, AC-L3-3（run 2、Gated Amendment 2 追加）表示件数 Select の位置と枠の地色が 6 画面で揃い、一括価格改定の取引先追加ボタンが Select の隣にある（整合性チェックの単独 Select は対象外、下記 S1 参照）, AC-L3-4（run 3 新設、Gated Amendment 3 GA3b、判定材料——PASS/FAIL の合否条件ではなく owner が (a)/(b)/(c) を選ぶための所見）: 箱が既定で 50 行分の高さまで伸び、それ以下では `<main>` の page scroll で一覧を見渡せる。51 行以上（perPage 100 等）では箱が内部縦 scroll に切り替わる。ページ scroll 中に列見出しが流れる／箱 scroll 中は残る、という trade-off を確認したうえで owner が方針を選ぶ
...
- **介入 3/3 消費済み（2026-09-06 owner L3 run 2）。Gated Amendment 3 は run 3 を要するため 4/3 = 予算超過**——owner の「まず回答としては箱の高さ方針だとaだと思う、まずは見てみてどうなるかというところ」（raw file「Lane 4 L3 run 2 追記」節、2026-09-06）を、L3 run 3 に向けた延長の了承として記録する。GA3a（識別列固定の是正）は owner が run 2 で明示した FAIL の直接是正であり追加了承を要さないが、GA3b（箱の高さ方針、owner 決定 (a)「まずは見てみて」）は owner 自身が求めた試作のため、この 1 行を了承の根拠として引用する
...
### Gated Amendment 3（owner Windows native L3 run 2 起源、2026-09-06〜07。GA3a = 識別列固定 FAIL の機序確定 + 是正、GA3b = 箱の高さ方針の試作）
...
**Design（Coordinator 決定、単一 source で幅と offset の一致を担保）**:
- 商品コード列を固定幅 `w-36 min-w-36 max-w-36`（9rem = 144px、JAN 13 桁 ≈ 125px が収まる）にし、`ListShell.tsx` `IDENTITY_COLUMN_CLASSES[2]` の `left-[7rem]`（2 箇所、`thead th:nth-child(2)`/`tbody td:nth-child(2)`）を `left-[9rem]` へ改める。`min-w-36` が「短いコードで列が sticky offset より狭くなる」（93 < 112 の型）を、`max-w-36` + 折返しが「長いコードで列が offset を超えて広がる」（116 > 112 の型）を、それぞれ防ぐ——`w-28` 単体（round 3 是正で既に指摘済みの trade-off）と同型の問題を、`min`/`max` の両側から塞ぐ。
- **`max-w-36` を実効化するには折返しを許可する必要がある**（round 3 是正の教訓の再確認）: `table.tsx:74` の `TableCell` 既定は `whitespace-nowrap` であり、`max-width` を付けても `nowrap` のままでは長い内容が折り返さず max-width を突破して列を押し広げる（`w-28` が実効しなかったのと同じ機序）。したがって商品コード td（`ProductTable.tsx:52`）は `whitespace-nowrap` を打ち消し `break-all` を追加する（`whitespace-normal break-all` の組合せ、Tailwind の一般的な「長い連続文字列を強制折返し」パターン）。商品コード th（`ProductTable.tsx:36`、見出しラベル「商品コード」5 文字で折返し不要）は `whitespace-nowrap` のまま、幅 3 class のみ揃える。
- **単一 source（ponytail: 新しい config/theme 拡張は作らない）**: Tailwind JIT は実行時テンプレートリテラルの class を拾えないため、幅（`w-36 min-w-36 max-w-36`）と offset（`left-[9rem]`）を実行時に同じ変数から生成することはできない。したがって両方 literal class のまま、幅 3 class を `ListShell.tsx`（`IDENTITY_COLUMN_CLASSES` の直前）に `export const PRODUCT_CODE_COLUMN_WIDTH_CLASSES = ["w-36", "min-w-36", "max-w-36"] as const;` として co-locate し、`ProductTable.tsx` の th（`:36`）・td（`:52`）の両方がこれを import して適用する（幅の実体は 1 箇所に集約、th/td 間のズレを構造的に排除）。offset の `left-[9rem]`（`IDENTITY_COLUMN_CLASSES[2]` 内、2 箇所）は幅とは別の literal のままだが、両者のコメントで相互参照し、**幅と offset の rem 値が一致することを新規 parity test で機械的に保証する**（`w-36` → 9rem、`left-[9rem]` → 9rem をそれぞれ文字列から抽出し数値比較。片方だけ変更する mutant を検出する対の oracle）。
...
- `ProductTable.test.tsx` 拡張: 商品コード td が `w-36`/`min-w-36`/`max-w-36`/`break-all` を持つ（`whitespace-nowrap` は持たない）；商品コード th が `w-36`/`min-w-36`/`max-w-36` を持つ（既存 `:114-118` の `w-28` assertion を置換）
- `ListShell.test.tsx` 拡張: `IDENTITY_COLUMN_CLASSES[2]` 由来の root token に `left-[9rem]`（`thead th:nth-child(2)`/`tbody td:nth-child(2)` 双方）が含まれ、`left-[7rem]` は 0 件（既存 `:462,465` の `left-[7rem]` assertion を置換）
- `ListShell.test.tsx` 新規（parity oracle）: `PRODUCT_CODE_COLUMN_WIDTH_CLASSES` の `w-36` から抽出した rem 値（9）と `IDENTITY_COLUMN_CLASSES[2]` の `left-[9rem]` から抽出した rem 値（9）が一致する（片方だけ書き換える mutant を検出）
- Matrix mutants: 幅 3 class のいずれかが欠落／`break-all` 欠落で `max-w-36` が無力化される／offset だけ変更され parity test が検出しない／`w-28` 消滅チェックが `ProductTable.tsx` 以外にも波及し他 4 file の正当な `w-28` を誤検出する

**AC-GA3a（run 3、owner Windows native L3）**: 最狭幅 × 2 ページ目以降で部門列の頭が固定 2 列の下に隠れない。横 scroll 中に商品名列の左端が商品コード列の右端を覆わない。
...
- **行高の近似値（ponytail: 実測ではなく class からの概算、L3 run 3 で実データとズレていれば数値のみ補正する）**: thead th の高さは `table.tsx:61` `h-10` = 2.5rem。tbody 行は `table.tsx:74` `p-2`（上下 0.5rem×2 = 1rem）+ `ProductTable.tsx` 商品名セルの 2 行想定（商品名 1 行 ≈ 1.25rem + JAN code `text-xs` 1 行 ≈ 1rem）+ border 1px 分の余裕を丸めて、**1 行 ≈ 3.5rem** と近似する。箱の `max-h` = 2.5rem（thead）+ 50 × 3.5rem（tbody 50 行）= 177.5rem、`max-h-[177.5rem]` とする。この数値は近似であり、L3 run 3 で「50 行より早く/遅く箱が scroll し始める」所感があれば構造を変えず定数のみ補正する。
- **CSS 制約（Human Gate に明記、owner が run 3 で判断する材料）**: 箱が横 scroll 容器（`overflow-auto`）である限り、CSS の overflow 計算規則により縦方向の computed 値も `auto` になる（起票時実測「識別列固定の実装方式」節で確認済みの一般規則の再適用）。50 行以下（箱が内部縦 scroll を必要としない状態）では箱自身は縦に scroll しないため、`sticky top-*` の thead は「箱の scrollport に対しては」常に先頭にあり続けるが、`<main>`（ページ全体）を縦に scroll したときは箱ごと画面外へ流れる。したがって GA1 で PASS した「縦に滑らせても見出しが箱の上に残り、下のページ送りは常に見える」という owner 期待像は、**51 行超（箱が内部 scroll する状態）でのみ再現され、50 行以下（`<main>` が page scroll する状態）では見出しごと流れる**。この trade-off を L3 run 3 で owner に明示し、(a) このまま採用 / (b) GA1 の「箱が常に唯一の scroller」へ差し戻す / (c) 折衷（perPage が閾値以下のときだけ箱を clamp しない等）のいずれかを owner が選ぶ。本 Gated Amendment はこの試作を Scope とし、owner 判定確定前に DSR-17/DSR-22/catalog ⑯ の正本記述は改訂しない（上記「GA3b で置換予定」注記のみ、下記 Registration Obligations 参照）。
- **Scroll restoration の再設計（契約のみ規定、実装は Writer が GA3b 実装時に設計する）**: GA1 の `resolveScrollTarget()`（`app-router.ts:20-25`）は「箱があれば箱・無ければ main」という択一で、商品一覧では常に箱だけを縦横の復元・先頭化対象にしていた。GA3b では商品一覧は「`<main>`（縦、51 行以下で主 scroller）」と「箱（横は常に、51 行超のときは縦も）」の**両方**を独立に復元・先頭化する必要がある。保存側（TanStack Router 内蔵の cache）は変更不要——`data-scroll-restoration-id="main"`/`"products-list"` の 2 entry は GA1 で既に両方 cache 対象、`SCROLL_RESTORATION_ALLOWLIST`（`app-router.ts:18`）も不変。適用側（`onRendered`、`app-router.ts:81-167`）は現在 `resolveScrollTarget()` が返す単一要素に対して縦横 armed/適用を行う構造だが、GA3b では main（縦のみ）と箱（縦・横）の 2 要素それぞれに同型の armed/適用ロジックを回す設計へ拡張する。forced-top（`app-router.ts:76`・`page-scroll.ts:47` `PAGE_TOP_SCROLL_OPTIONS`）も main の縦リセットと箱の横リセット（+ 51 行超時の縦リセット）の両方を行う。
- **GA1e fs scan の反転**: `src/test/page-root-pageshell-sweep.test.ts:87-115` の GA1e は「`stickyHeader` を渡す Page は同一 file 内で `PageShell` に高さ連鎖 class を渡している」ことを期待する。GA3b 実装後は `ProductListPage.tsx` が高さ連鎖 class を渡さなくなるため、GA1e はそのままでは誤って violation を報告する。GA3b 実装時に GA1e を「`stickyHeader` を持つ Page は高さ連鎖 class を**持たない**」ことを期待する逆方向のテストへ書き換えるか、`stickyHeader`/page root 高さ連鎖の契約自体が GA1 限定の過去仕様になるため describe ごと削除する（Writer が実装時に選ぶ、本 packet ではどちらかに固定しない）。
...
- `ListShell.test.tsx` 拡張（GA3b-3）: 箱要素が `max-h-[177.5rem]`（最終決定値）と `overflow-auto` を持ち、`min-h-[12rem]`/`flex-1` を持たない
- `ListShell.test.tsx` 拡張（GA3b-4、補助）: toolbar 要素が `shrink-0` を持たない（死んだ class 除去の回帰確認）
- `page-root-pageshell-sweep.test.ts` 拡張（GA3b-5）: GA1e の反転または削除（上記「GA1e fs scan の反転」参照）
...
- AC-L3-1（owner Windows native L3、**run 2、Gated Amendment 1 で全面差し替え。run 3 で GA3a の是正項目を追加**）: 窓を狭めて表が枠より広い状態で横に滑らせる → 検索ツールと商品コード・商品名と見出しは動かず、部門から右だけが滑る。縦に滑らせても見出しが箱の上に残り、下のページ送りは常に見える。「修正」から戻ると同じ位置に戻る。**旧 run 1 の AC-L3-1**（`<main>` 基準 sticky-left モデル、toolbar は流れる前提）は FAIL となったため本文を差し替える——旧文の内容は Gated Amendment 1 の起票時実測節・`## Review Response` に経緯として残す。**run 3 追加項目（Gated Amendment 3 GA3a）**: 最狭幅 × 2 ページ目以降でも部門列の頭が固定 2 列の下に隠れない。横 scroll 中に商品名列の左端が商品コード列の右端を覆わない
...
- AC-L3-4（owner Windows native L3、**run 3 新設、Gated Amendment 3 GA3b、判定材料——PASS/FAIL の合否条件ではなく owner が方針を選ぶための所見**）: 箱が既定で 50 行分の高さまで伸び、それ以下では `<main>` の page scroll で一覧を見渡せる。51 行以上（perPage 100 等）では箱が内部縦 scroll に切り替わる。ページ scroll 中に列見出しが流れる／箱 scroll 中は残る、という trade-off を確認したうえで owner が (a) このまま採用 / (b) GA1 の箱内 scroll 常時へ差し戻す / (c) 折衷、のいずれかを選ぶ
...
| Gated Amendment 3 GA3a: 商品コード列固定幅 + sticky offset 一致（単一 source + parity test） | `ListShell.tsx`、`ProductTable.tsx` | GA3a 新規 assertion | AC-L3-1（run 3） |
| Gated Amendment 3 GA3b: 箱の高さ方針試作（行数基準 max-h、GA1 高さ連鎖の撤去、scroll 復元の main/箱 二元化） | `ProductListPage.tsx`、`ListShell.tsx`、`page-scroll.ts`、`app-router.ts`、`page-root-pageshell-sweep.test.ts` | GA3b 新規 assertion | AC-L3-4（run 3、判定材料） |
