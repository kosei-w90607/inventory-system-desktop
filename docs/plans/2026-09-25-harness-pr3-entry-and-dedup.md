...
- review の依頼・出力の規範（依頼形式・重大度・出力形式）は `docs/code_review.md` の 1 箇所で決まり、subagent 発注の入力欄は `docs/templates/subagent-review-packet.md`（PR2 の所有。規範を複製しない）が持ち、薄い wrapper（`pr-review` / `review-only-subagent` Skill、`pr-review-prompt` template、`.claude/rules` 2 本、`.claude/commands` 3 本）が消えても、残る参照がすべて実在の file を指す。
...
| Codex が review-only 発注を受ける | 発注書が読む範囲（packet / Matrix / 差分 / 観点）を指定し、Codex は `AGENTS.md` `Session Start` の初回レビュー行を読む | 発注書が指定した範囲から始め、Session Start の全体の読書経路は踏まない。R3/R4 の Contract Audit が求める設計正本は発注の範囲に関わらず読み、具体的な欠陥の根拠があれば他の範囲外も読んで理由を報告に書く。`docs/code_review.md` の重大度と出力形式で返す | 初回レビュー行に、指定範囲から始める経路と、範囲に関わらず設計正本を読む条件がある（AC4） | 実際の token 削減量は未実測（backlog の 2026-09-07 実測は旧経路のもの） |
...
行番号は base `85b18a04`（本 branch の merge-base）のもの。起票時の base `8bd2bc9a` から本 PR の対象 file で変わったのは `docs/quality/review-checklist.md` と `docs/backlog.md` だけで、backlog は行番号でなく entry の文で指す。各 file の変更は該当行・節に限り、他の節を書き換えない。
...
- S1 `AGENTS.md`
  ...
  - `:15`（初回レビュー）: 「発注書が読む範囲を指定した review（例: Codex の review-only 発注）は、その範囲から始め、Session Start の全体の読書経路は踏まない。R3/R4 の Contract Audit が求める設計正本は、発注の範囲に関わらず読む。具体的な欠陥の根拠があれば他の範囲外も読み、理由を報告に書く」を足す（backlog の「Codex review-only 発注向けの Session Start fast path」entry の最短経路。発注書 template 側の宣言行は `.local/` の運用で tracked の対象外）。
  ...
  - `## Decision and Approval Boundaries`（`:38-45`）: 問い合わせの行き先を 2 項目で足す。(i) owner へ送り中継を省かないもの = 店の事実・実機でしか確認できない挙動（具体的な質問か短い PASS / FAIL）、目的・製品の振舞い・受容リスク・予算・優先順位・範囲の変更・不可逆な操作、Windows L3・R4・Ready・merge、PR の label・thread 状態・close・issue comment といった外部書込み（`AGENTS.md:45` が別承認とする操作）。判断は owner。現行の明示承認と有効な委任（push・Draft PR 作成、owner の承認後の helper `ready` / `merge` の実行）の範囲では Coordinator が実行を代行できる（委任の範囲の文はここだけに置き、PR2 の MANUAL の Human Gate 行はここを参照する）。複数に当たり owner 側を含む場合と、裁定権の所在が争点の場合は owner へ送る。(ii) agent 側で解くもの = 発注だけの誤り（正本が一意で発注が食い違う）・環境の不備（許可済みの環境・範囲で解消でき、追加の費用・権限・正本の変更を伴わない場合）・技術的な前提（調査・Contract Probe）・正本の意味の争いの事実確認。owner に技術的な正しさの承認を求めず、証跡の編集や発注訂正の伝言を頼まない。出典は `docs/DEV_WORKFLOW.md:282-296` の表と注記 (a)〜(c)、行 7 の穴は backlog の「問い合わせの行き先の表の行 7 に `AGENTS.md` が別の明示承認とする PR 操作が挙がっていない」entry。`:45` の文は残す。
...
- S5 workflow Skill（`.agents/skills/**`。`.claude/skills` の symlink は sandbox で書込み不可のため触らず、`pr-review` / `review-only-subagent` には symlink が無いことを `ls -la .claude/skills` で確認済み）
  - `inventory-workflow-start/SKILL.md`: `:13` の MANUAL への link を anchor なしの file link にし、同じ行の語「Capacity-degraded」を「担当が使えないとき」に替える（PR2 が §3 の見出しを変え、§3.3 を `## 担当が使えないとき` に書き直すため）。`:20` から `pr-review` を消す。`:24` の定型文を消す。
...
- S6 `docs/code_review.md`
  ...
  - `## Output Shape` に、finding ごとに確信度を添える要求を足す（現行 `docs/templates/subagent-review-packet.md` の `## Output` の要求を移す。PR2 の S4 がそちらを外して code_review を参照にする。確信度の移し先はここだけ）。
...
- S14 `docs/decision-log.md` の末尾に `## D-093` を追記する（番号は予約済み。D-091 は PR #98、D-092 は PR2）: 入口文書の単一所有。(1) npm 供給網ガード（D-030）の行動規則の所在を `CLAUDE.md` から `AGENTS.md` へ移す（D-030 の Impact の「CLAUDE.md の節」を部分的に置き換える。規則の中身は変えない）。(2) 問い合わせの行き先の所有を `AGENTS.md` Decision and Approval Boundaries にする（D-090 の SPEC-WF-LIGHT1A-D4 を部分的に置き換える。`docs/DEV_WORKFLOW.md` の表は PR4 が link に置き換える）。(3) review の依頼・出力の規範（依頼形式・重大度・出力形式・確信度）を `docs/code_review.md` に統合し、`pr-review` / `review-only-subagent` Skill と `pr-review-prompt` template を廃止する。subagent 発注の入力欄は `docs/templates/subagent-review-packet.md`（D-092）が持ち、規範を複製しない。`pr-review-prompt` は本 PR で code_review へ統合済みで、PR4 での移入は要らない。(4) Final Review の固定観点「保守者として読めるか」（owner 2026-09-07）と店の事実の照合を code_review へ置く。
...
- PR2 に求めること（Coordinator が PR2 の起草役へ伝える）: MANUAL `:124` の `pr-review-prompt.md` への link を消すか `docs/code_review.md` へ向ける。`docs/agent-guidance/model-notes.md:9` の `CLAUDE.md#sonnet--opus-の-effort` への link を座組表へ向ける（本 PR がその見出しを消すため。anchor は検査されないので黙って死ぬ）。`docs/templates/subagent-review-packet.md:26`、`docs/agent-guidance/README.md:8`、`docs/agent-guidance/shared.md:9` の legacy / Evidence Mode の文は PR2 が消す（PR1 の Non-scope が PR3 と書いたが、所有に合わせ PR2 へ移す）。`docs/templates/subagent-review-packet.md` の `## Output` の出力の規範（確信度を含む）を外して `docs/code_review.md` を参照にする（確信度は本 PR の S6 が code_review へ足す）。
...
- PR2 が止まった場合: `CLAUDE.md` の `## Sonnet / Opus の effort` と `AGENTS.md:21` の profile の文は PR2 の merge まで現行のまま残す（S2 のその部分と AC11 の `xhigh` = 0 を保留し、Gated Amendment で S7 と合わせて PR4 へ移す）。link だけ外して effort の規則が tracked から消える形にしない。

### helper と PK5 の前提（capture の前に満たす）

- `scripts/pr-gate.py:231` は PR head の `docs/plans/` に active packet がちょうど 1 つであることを要求する。base `85b18a04` では EJ parser core の packet は archive 済みで、active な他 packet は並走 PR2 の branch にだけある（main には無い）。PR2 の packet は PR2 の merge から closeout まで main に残るので、本 PR の capture の前に PR2 の closeout が origin/main に入り、それを本 branch へ merge してあることを確認する。
...
- `docs/project-profile.md:257`（S10 で消す `## Open Questions` の中の、tracked の POS sample / reference data を synthetic-only にするかという未決の data safety の問い）: closeout で backlog へ移す。
...
| SPEC-WF-HARNESS3 | `docs/code_review.md`、pr-review-prompt、subagent-review-packet | D4 | review の依頼・出力の規範（重大度・出力形式・確信度）の正本を code_review 1 つにし、subagent 発注の入力欄は review packet（PR2）が持つ。却下: review packet に出力形式を残す（2 つの規範を読むことになる）、規範ごと review packet へ統合（PR2 の所有で、外部 reviewer への依頼も code_review が持つ） | S6、S7 | AC2、AC8 |
...
- Assumptions and constraints: PR2 が `## 座組` の見出しを置き、MANUAL `:124` の link を消し、review packet の出力の規範を外す（「PR2 との境界と merge 順」）。PR2 の closeout が capture 前に main に入る。
...
- 「helper は packet がちょうど 1 つでないと拒否する」: `scripts/pr-gate.py:228-231` → 成立。起票時の base `8bd2bc9a` には EJ の packet が残っていた（現 base `85b18a04` では archive 済み。「helper と PK5 の前提」）。
...
| D4 review の依頼・出力の規範の code_review への統合 | S6、S7 | AC2、AC8 | subagent 発注の入力欄は review packet（PR2 の S4） |
...
- D1: npm 供給網ガード（D-030）の行動規則と、問い合わせの行き先（PR 操作と、判断は owner・委任の範囲では Coordinator が実行を代行できることを含む）の正本は `AGENTS.md`。`CLAUDE.md` は Claude 固有の補助だけを持ち、座組と effort は `docs/AGENT_OPERATING_MANUAL.md#座組` を参照する。
...
- D4: review の依頼・出力の規範（依頼形式・重大度・出力形式・確信度）の正本は `docs/code_review.md`。subagent 発注の入力欄は `docs/templates/subagent-review-packet.md`（PR2 の所有）が持ち、規範を複製しない。`docs/templates/pr-review-prompt.md` は削除する。
