...
- AC-8: `git diff --stat` の変更 file が Scope 列挙の docs 10 点（Scope 1〜7 の distinct 8 doc — Scope 4 は 65/74 の 2 file — + Scope 8 で新規に加わる 69-ui / 53-ui の 2 doc）+ `docs/Plans.md`（entry 同期）のみ（src / src-tauri / 設定 file の変更 0。gated amendment 2 で「9 点」の算術誤りを 10 点へ訂正）。
