...
- **Amendment 4 closure round 1 是正（Codex Final Review round 1 P1-1〜P1-5 / P2-1〜P2-2 起源、2026-09-04、Coordinator 起票、Writer `e529e05` 以降 worktree で実装）**: round 1 Final Review は「reference の画面本体が owner 方針 A に戻り切っていない」と判定し PASS を保留した（`cf56d18..e529e05 -- src src-tauri` 差分 0、DSR-22 差分 0 は確認済み）。以下 S28〜S34 で是正する。
- **S28 後続 Lane の ListShell / sticky / 上部 pager を screen 本体から除去**: forms-b（棚卸し）/ history（入出庫履歴・在庫変動履歴・操作ログ）/ home-sales-admin（日次商品明細・月次ランキング・Backup 一覧・整合性チェック）/ import-export（部門別集計・商品プレビュー・PLU 書出し対象）の画面本体から `.pager.top` 帯を除去する（`mockup-d-lists.html` の pilot 帯化は不変）。識別列固定（`id`/`id2` sticky class）は、runtime で実際に sticky 描画するのは商品一覧 `ListShell` pilot だけであることを根拠に、forms-b の棚卸し・home-sales-admin の 4 一覧からも除去し、末尾 note の「後続候補（本 mockup へ描かない）」へ一本化する。例外は history の 3 テーブル（記録日時+代表商品／日時+種別×2）のみとし、DSR-22 mapping・AC-L3-5 の owner 最終確定対象として `id`/`id2` sticky class を維持する（Codex round 1 の「DSR-22 が例外的に要求する history の固定列 mapping自体は残す」を踏襲）。
- **S29 history を runtime contract へ再同期**: 入出庫履歴の filter row を `InventoryRecordsPage.tsx:150-267` どおり 記録種別 / 開始日 / 終了日 / `SearchBar`（live、ボタンなし）/ 記録ID / 部門 / 状態 の label 付き input・select に置換し（SegmentedControl・並び替え select は現実装に無いため削除）、列は `:301-346` の実列順（種別 / 記録ID / 業務日付 / 代表商品 / 明細数 / 状態 / 操作）のうち記録日時・代表商品のみ DSR-22 mapping で先頭 2 列へ移設し残りは相対順を保つ。行内 detail 展開 JS を削除し、各行の操作は `詳細を見る` link 1 個（`Eye` icon）に統一。操作ログの subtitle を runtime `OperationLogsPage.tsx:331` の「システムの操作履歴を期間・種別で確認します」へ、在庫変動履歴 filter の「期間」結合 label を `StockMovementsPage.tsx:124-171` どおり 開始日 / 終了日 / 種別 の分離 label へ揃える。
- **S30 home-sales-admin を runtime contract へ再同期**: Home に `HomePage.tsx:79` の前日未取込み alert（`前日分が未取込みです` + 取込み精算日、destructive/bad トーン）を PLU warning の下へ追加する。日次商品明細を `ProductTable.tsx` の列 商品コード / 商品名 / 部門 / 数量 / 単価 / 金額 へ揃える（上部件数先出しは S28 で除去済み）。Backup 復元を `BackupRestorePage.tsx:550,619,657-675` の 3 段（一覧行「この控えに戻す」→ 選択後の警告 alert「復元すると今の記録は戻せません」+「復元の確認へ進む」→ 最終確認 dialog「元に戻せません」/「キャンセル」/「{日時} の控えに戻す」）へ書き換える。整合性チェック table を `IntegrityCheckPage.tsx:307-364` の 6 列（商品コード / 名前 / システム在庫 / 入出庫の合計 / 差異 / 操作）へ揃え、差異列に `differenceLabel` outline badge を添え、checkbox・「補正する」ラベル・「補正済み」badge を「操作」列内へ統合する（S27 で確定した primary button `補正を確定` は維持）。
- **S31 import-export を runtime contract へ再同期**: 日報 preview（`t-nippo`、状態 `2/3 プレビュー`）から「結果（前回の取込み）」block を削除する（`DailyReportImportPage.tsx:26-63` の状態機械で preview と result は排他）。商品一括インポートを `ProductImportPreview.tsx` の 4 summary（登録対象 / 新規候補 / 重複 / エラー）+ 3 section（新規登録候補: 行/商品コード/商品名/部門ID/売価/PLU対象、既存商品との重複: 上書き checkbox を行ごとに持つ table + 警告 alert、エラー行: 行/内容の 2 列プレーン table）へ置換する。PLU書出しを `PluExportPage.tsx` から同期: 復帰バナーに `:462-467` の「この書出しを未反映から外す」primary button + 「破棄して再書出し」outline button を持たせ、要修正一覧を `:594-613` の列 商品コード / JANコード / 商品名 / 理由（`formatExcludedReason`）へ、未反映商品を `:652-657` の列 商品コード / JANコード / 商品名 / 売価 / 在庫（`memory No.` / `状態` 列は削除）へ揃え、`:686-738` の「書出し設定」card（差分/全件 SegmentedControl、警告 alert、差分対象件数、書出し button）を新設する。runtime に実装のない「Full書出しの前に既存 PLU バックアップを確認」alert（67-ui REQ-402/UI-08-D5 要求、`PluExportPage.tsx` 未実装）は画面から除去し note の機能 gap 記録へ移す。
- **S32 forms-a の先行 UI を除去**: 入庫記録の明細行にあった `cur` row（左バー + 淡背景）と「入力中」badge（一覧画面 D-7 の現在行表現の転用提案、61-ui に明記なし）を screen 本体・JS 生成データから削除し、末尾 note の「後続候補（本 mockup へ描かない）」へ移す。
- **S33 SC12 / SC14a / SC14b の scope 分離と再実行性の是正**: SC14a（画面 markup 範囲）と SC14b（末尾 note 範囲）を明確な `sed` 範囲（`<main>`〜`</main>` 相当 / 末尾 `<div class="note">` 以降）で分離し、absence 判定は `rg -v 'class="note"'` を通す。presence 判定を含む全 command を `for w in …; do rg -c -F -- "$w" <file>; done` 型に統一し、no-match は exit 1 のまま個別件数を明示する（合算 `\|` alternation は使わない）。forbidden signature（`class="pager top"` 全画面本体・`<label>期間 `・`詳細ページを開く`・`関連 movements を見る`・`結果（前回の取込み）`・`memory No.`・受入記録の `入力中`・バックアップの `の状態に戻す`）を absence 対象に追加し、Matrix SC12 / SC14a / SC14b 実測表へ command と件数を転記する（下表参照、全 green）。
- **S34 Plans.md の owner disposition を 4 区分へ分解**: `Plans.md:50 (vi)` を「採用 / 現状維持 / 後続候補 / 明示不採用」の 4 labeled sub-bullet へ分け、`docs/plans/2026-09-03-ui-list-backbone-d-lane2.md:642-648`（owner run 3 evidence）の理由（operator 理解、専用 layout / semantic detail の優位、DTO・native 確認・紙面設計の未充足、架空 field / 誤誘導 / 意味を壊す共通化）まで転記し、5 mockup の末尾 note と同じ字面で照合する。
...
- AC-L3-5（S7 + Gated Amendment 4、mockup 視認）: `docs/design-system/reference/mockup-d-{forms-a,forms-b,history,home-sales-admin,import-export}.html` 5 file を順に開き、画面に描かれた情報・操作が現実装に存在し、Lane 2 の差分と後続候補が混ざっていないことを確認する。回答は番号ではなく file / 画面名 + 気になる箇所でよい。history の固定列は DSR-22 `:435` mapping（記録日時 + 代表商品）どおりに描かれていることを確認し、owner が異論なしなら run 3 所感と合わせて mapping 最終確定の evidence とする（`:441` の暫定文言撤去は L3 PASS 後の docs commit = ready-hosted-final 同乗 vehicle、従来計画どおり）。
...
- AC10（mutation）: Matrix「必須 mutation 注入」X1〜X34 を Final Review で Sonnet subagent（隔離 worktree、clean tree）が `npm run test` で独立再実測し全 kill（survivor 0、Gated Amendment 2 で X17〜X20、Gated Amendment 3 で X21〜X29、追補で X30〜X34 追加）。
