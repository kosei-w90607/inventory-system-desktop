...
- Plan Commit: 244a5dd
- Amendments: 4401112 b318240 357941c 75d5e30 ec56d20 cf56d18 29091d4 9005270 cdd82d9 9e9e76f a823df8 eeeca9c 4b7eecf
...
### Gated Amendment 7（owner L3 run 6 部分 PASS 起源、2026-09-04、Coordinator 起票、owner escalation）

run 6 の PASS 2 点（在庫数「18 個」、角丸）は維持し、要修正 3 点を束ねる。Plan Review round は天井到達済みのため Fable 裁定で確定。runtime 変更は token 値 1 / ListShell の帯 class 1 / thead の seam 対策 1。

- **S46 操作面の色を #FAFAF9 へ（owner run 6「検索欄と Select の色は FAFAF9 で」）**: `globals.css` の `--control-surface` を `#ffffff` → `#fafaf9`（`--background` と同値だが独立 token として維持。将来 page 地と操作面を分けられるように）。`input.tsx` / `select.tsx` の `bg-control-surface` は不変（検索欄 `SearchBar.tsx` は `Input` 経由で同 token を使うため両者は必ず一致する）。00-foundations 表の HEX を #fafaf9、根拠を「owner run 6 指定。対 `--card` #f5f5f4 1.02:1（面の差は僅少、枠 `--border-strong` 3.53:1 が操作対象の主信号）」へ。SC15 の literal を `--control-surface: #fafaf9` へ更新（X37 不変）。`mockup-d-lists.html` の `input` / `.ctl` の background が `#fff` なら `var(--bg)` へ同期。
- **S47 件数行の下線を外す（owner run 6「上端の線だけ外そう」）**: `ListShell.tsx:101` の帯から `border-b border-border` を削除（帯 = `bg-background` のみ。列見出し帯の上端に線が無い状態。forced-colors の下端線は owner が mandatory を撤回済みのため不要）。SC8 / SC10 の `border-b` / `border-border` 期待を「含まない」へ更新。Matrix X33 を「帯に `border-b border-border` を追加」（kill = SC8 not.toContain）へ書き換え。catalog ⑯ 第 3 項の「件数行（`bg-background` + 1px `--border` 下線）」を「件数行（`bg-background`、線なし）」へ、`mockup-d-lists.html` `.pager.top` の `border-bottom` を削除（SC12 lists oracle の `border-bottom` presence を absence へ）。
- **S48 列見出し帯の縦 seam 対策（owner run 6「最小幅で 部門 と 売価 の間に白い細線」）**: 原因 = S43 で `[&_thead_tr]:bg-list-head` を外したため、`border-separate` + 最小幅の fractional layout で th 同士の境界に subpixel の隙間が生じ page 地が透ける（Amendment 2 S11 の tr 背景が塞いでいた）。是正 = (i) `[&_thead_tr]:bg-list-head` を復活（seam を塞ぐ）。(ii) 角丸を保つため、tr 背景が th の角の外側に四角く覗く分を page 色で覆う corner mask を追加: ListShell root（`stickyHeader` 時）に hook class `list-shell-sticky` を付け、`globals.css` に unlayered で `.list-shell-sticky thead th:first-child::before` / `.list-shell-sticky thead th:last-child::after`（`content:""; position:absolute; top:0; left:0（after は right:0）; width:0.375rem; height:0.375rem; pointer-events:none; background: radial-gradient(circle at 100% 100%（after は 0 100%）, transparent 0.375rem, var(--background) 0.375rem)`）を定義（th は `position: sticky` で absolute の containing block になる。0.375rem = `rounded-md`）。children に className を渡さない D-2 / D-9 は維持（root の class と globals の descendant selector）。Spec（SC17、fs literal + DOM）: `globals.css` に `.list-shell-sticky thead th:first-child::before` と `th:last-child::after` の 2 rule があり `radial-gradient` と `var(--background)` を含み `@layer` の外にある、ListShell root の classList に `stickyHeader` 時のみ `list-shell-sticky` を含む、root に `[&_thead_tr]:bg-list-head` を含む（SC4d / SC8 の tr 不在期待を反転）。Matrix X36 を「`[&_thead_tr]:bg-list-head` を削除」へ書き換え、X40「root から `list-shell-sticky` を削除」、X41「globals の corner mask 2 rule を削除」を追加（kill = SC17）。docs: catalog ⑯ 第 3 項に「th cell 背景 + tr 背景（seam 対策）+ corner mask（角丸維持）」を 1 文、`mockup-d-lists.html` は `th` の radius 指定に加え `tr{background:var(--d-head)}` を同期。**予約経路**: L3 run 7 で角に artifact が出る場合は corner mask をやめ、tr 背景なし + `[&_thead_th]:shadow-[1px_0_0_0_var(--list-head)]`（右隣へ 1px の同色帯で seam を覆う）+ `[&_thead_th:last-child]:shadow-none` へ切り替える Gated Amendment 経路。
- **Matrix**: SC15 更新（#fafaf9）、SC8 / SC10 更新（`border-b` 不在、tr 背景あり）、SC12 lists 更新、SC17 新設、X33 / X36 書き換え、X40 / X41 追加。AC10 = X1〜X41。
- **AC-L3 更新**: AC-L3-1 の「面が白く」を「面が #fafaf9 で toolbar 箱 #f5f5f4 と区別できる」へ。AC-L3-2 に「列見出し帯の上端に線が無い」「window を左右限界まで縮めても列見出しの間に縦線が出ない」「角丸は維持」を追加。L3 run 7 = この 3 点（+ 角の見た目維持）。
- **遷移**: Writer（Sonnet、TDD）→ Sonnet 独立 closure（X40 / X41 新注入 + X33 / X36 / X35 / X37 / X19 再測、SC15 / SC17、fs scan test を含む file 集合で実行）→ Fable 裁定 → 同乗 commit（Ledger S46〜S48 行 + 結果記入 + Workflow State）→ L3 run 7 → PASS で DSR-22 `:441` 確定 docs commit に `human-confirm -> ready-hosted-final` 同乗 → Ready → merge。
...
- AC10（mutation）: Matrix「必須 mutation 注入」X1〜X41 を Final Review で Sonnet subagent（隔離 worktree、clean tree）が `npm run test` で独立再実測し全 kill（survivor 0、Gated Amendment 2 で X17〜X20、Gated Amendment 3 で X21〜X29、追補で X30〜X34、Gated Amendment 6 で X35〜X39、Gated Amendment 7 で X40〜X41 追加）。
...
| Amendment 7 S46 `--control-surface` #fafaf9 | globals.css / 00-foundations / mockup-d-lists.html | SC15（更新）、X37 | AC-L3-1（run 7） |
| Amendment 7 S47 件数行の下線撤去 | ListShell.tsx / catalog ⑯ / mockup-d-lists.html | SC8 / SC10（更新）、X33 | AC-L3-2（run 7） |
| Amendment 7 S48 tr 背景復活 + `list-shell-sticky` corner mask | ListShell.tsx / globals.css / catalog ⑯ / mockup-d-lists.html | SC17、X36 / X40 / X41 | AC-L3-2（run 7、最小幅の seam と角） |
| DSR-22 低視力 | — | — | AC-L3-4 |
...
### Gated Amendment 7 対応（2026-09-04、owner run 6 要修正 3 点、S46〜S48）

**Writer content commit**: `8b68d03`（S46）/ `632eb4d`（S47）/ `12667a0`（S48、content candidate = Reviewed Content HEAD）。各 commit は revert-and-reapply の 3 wave で S-step ごとに 1:1。`git diff 4b7eecf..12667a0 --stat -- src` = ListShell.tsx / test / globals.css / test の 4 file、`src-tauri` / packet 差分 0。

**着地**: S46 = `globals.css:70` `--control-surface: #fafaf9`、00-foundations 行、mockup `select.ctl` を `var(--bg)`（`.search input` は background 未指定のため不変）。S47 = `ListShell.tsx:106` 帯から `border-b border-border` 撤去、catalog ⑯「線なし」、mockup `.pager.top` の border-bottom 削除、SC12 lists を absence へ。S48 = `ListShell.tsx:53` `[&_thead_tr]:bg-list-head` 復活、`:82` root hook `list-shell-sticky`、`globals.css:114-136` に unlayered corner mask 2 rule（`radial-gradient` + `var(--background)`、0.375rem）、catalog ⑯「corner mask」、mockup `thead tr{background:var(--d-head)}`。SC17 の fs literal は selector が 2 回現れる（共通宣言 + 個別 background）ため `lastIndexOf` で個別 block を検査。TDD で 3 wave とも red → green、DS3 = 25 不変。

**独立 closure**: Sonnet mutation（`12667a0`、fs scan test を含む 3 file を毎回実行）= X40 / X41 kill、再測 X33 / X36 / X35 / X37 / X19 kill、追加 3 件（token を #fff に戻す / corner mask の `var(--background)` を `var(--card)` に / corner mask を `@layer base` 内へ移す）kill、survivor 0。doc oracle 6/6、DS3 = 25、既存 test 削除なし、gate 全 green、findings 0。P1/P2 = 0。

**遷移**: post-impl state-only 2/2 到達のため、`implementing -> local-verified -> independent-review -> human-confirm` を本 commit（Contract Coverage Ledger S46〜S48 行 + 本結果記入 + Workflow State）に同乗して materialize。根拠 = content candidate `12667a0`、L1 evidence は PR body。L3 run 7 = 商品一覧の 3 点（操作面 #fafaf9 / 上端の線なし / 最小幅の seam なし + 角丸維持）。
