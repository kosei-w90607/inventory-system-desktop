...
- **S10 PLU 一括操作の作用範囲と確認予告（FAIL-2）**: `ProductListPage.tsx` の `toolbarSecondary` 内で `PLU 対象にする` / `PLU 対象から外す` の pair を 1 つの group（`role="group"` + `aria-labelledby`）に包み、直上に可視 caption「**PLU 一括操作**（表示中の商品すべてが対象・確認画面あり）」を `text-sm`（label 部は `font-medium text-foreground`、括弧部は `text-muted-foreground`）で置く。button の label・variant・色は不変（既存 test の `getByRole("button", { name })` を壊さない）。Spec: caption text が `toolbarSecondary` 内に存在し、2 button が同じ group の子である。
...
- AC-L3-6（Gated Amendment 2 S10 / S12、render oracle）: 商品一覧の PLU 一括操作 caption で「表示中の商品すべてが対象・確認画面あり」が押す前に読めること。`PLU 対象から外す` を押して dialog を cancel で閉じたとき、閉じる途中に「PLU対象にしますか」系の反対文言が出ないこと。
...
- AC10（mutation）: Matrix「必須 mutation 注入」X1〜X20 を Final Review で Sonnet subagent（隔離 worktree、clean tree）が `npm run test` で独立再実測し全 kill（survivor 0、Gated Amendment 2 で X17〜X20 追加）。
...
**S10 PLU 一括 caption**: `ProductListPage.tsx` の `toolbarSecondary` 内、PLU 一括 2 button を `role="group"` + `aria-labelledby="plu-bulk-caption"` で包み、直上に `<p id="plu-bulk-caption">` で caption「**PLU 一括操作**（表示中の商品すべてが対象・確認画面あり）」を配置（label = `font-medium text-foreground`、括弧部 = `text-muted-foreground`、全体 `text-sm`）。button の label・variant・color は無変更。`ProductListPage.test.tsx` に SC7 を追加（caption class + 2 button が同一 group）。
