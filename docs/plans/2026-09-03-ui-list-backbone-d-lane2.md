...
- AC10（mutation）: Matrix「必須 mutation 注入」X1〜X29 を Final Review で Sonnet subagent（隔離 worktree、clean tree）が `npm run test` で独立再実測し全 kill（survivor 0、Gated Amendment 2 で X17〜X20、Gated Amendment 3 で X21〜X29 追加）。
