...
- **docs**: S8 に (ix)「帯文言の自然文化（『全 n 件のうち a〜b 件を表示（p/t ページ）』、owner L3 run 2 所感、P3）は DSR-22 範囲付き統一形の改訂で 8 caller + mockup + Matrix に波及するため Lane 3〜5」、(x)「帯の `truncate` は flex container 側にあり溢れは ellipsis でなく hard clip になる。`PaginationSummary` root へ移す案（Opus P3-5）」を追記。
...
- **遷移**: 追補の Writer 是正（content commit）→ Sonnet mutation 再実測（X30〜X33 + X21〜X29 再測）+ Opus デザイン面軽確認（追補差分のみ）→ P1/P2 = 0 で Coordinator の同乗 commit（Ledger SC6〜SC13 行 + 結果記入 + Workflow State）。content candidate = 追補の最終 content commit。
...
- AC-L3-4（DSR-22 低視力 L3 (a)(b)、`ProductListPage.tsx` の render oracle）: 商品一覧を Windows ハイコントラスト（forced-colors）で開き、toolbar 枠・入力枠・focus（追補 S16 の `Highlight` outline が検索欄 Tab で見えること）・sticky 帯とその下端境界線（追補 S17 の `forced-colors:border-b`）が消えないこと。in-app 表示 scale「特大」1.3 × OS DPI 125% の組合せを 1 回見ること（低視力 operator の実運用、Opus closure P3-3）。DPI 125% / 150% で toolbar 枠・sticky 帯（1 行のまま）・pager（`min-w-20` に「10 / 13 ページ」が収まる）が崩れないこと。検索欄に Tab で focus したとき静止時の枠との違いが一目で分かること。入力の多い画面（商品登録 or 入庫登録）を 1 つ 150% で開き、3.53:1 の入力枠が密集しても「線だらけ」に見えないこと。
...
- AC10（mutation）: Matrix「必須 mutation 注入」X1〜X33 を Final Review で Sonnet subagent（隔離 worktree、clean tree）が `npm run test` で独立再実測し全 kill（survivor 0、Gated Amendment 2 で X17〜X20、Gated Amendment 3 で X21〜X29、追補で X30〜X33 追加）。
