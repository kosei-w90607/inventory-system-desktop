...
- **Plan Review round 1 追加実測（F3, Sonnet P1-3）**: `daily-sales/components/ProductTable.tsx:133`（`src/features/daily-sales/components/ProductTable.tsx`、`<Badge variant="secondary" className="bg-warning-soft text-warning-strong">手動</Badge>`、`item.source === "manual"` 時のみ表示）。⑦ 起票時実測はこれを owner culling 対象（②分類か①状態か未裁定）として記録したのみだった。Coordinator adjudication（2026-09-08、決定 A）: ②分類（手動入力という「出どころ」の恒常的な区別であり、遷移する状態ではない）。原則 4「4 種目を作らない」+ 手動販売は正規の記録であり注意を要する状態ではない + owner の「色は意味がある時だけ」（catalog `:830` tone family 規約と同根）を根拠に、amber 系 className（`bg-warning-soft text-warning-strong`）を撤去し `variant="secondary"` のみ（S1 の `border-border` 付与を受ける）にする。owner 確認は pending — owner が決定 B（①状態 warning 側に残す）を選んだ場合は Coordinator が amend する
- **Plan Review round 1 追加実測（F4, Sonnet P2-1）**: `ReturnExchangePage.tsx:603-607`（`src/features/return-exchange/ReturnExchangePage.tsx`、`<Badge variant="outline" className="border-warning-border bg-warning-soft text-warning-strong">{formatStockEffectBadge(false)}</Badge>`「レジ未処理」）は既に warning 3 点セットを手書き済みだが tone prop 化されておらず、icon も無い（S2 の他サイトと同様 icon 追加が要る）。⑦ 起票時実測の 44 件走査で見落とされていたサイト（本 packet 起票時にも見落としていた）。機械的な `tone="warning"` 移行 + icon 追加の対象に含める。隣接 `:580`（`<Badge variant="outline" className="border-stone-200 bg-stone-50 text-stone-700">{formatStockEffectBadge(true)}</Badge>`「レジ戻し済み」）は無彩色の中立 badge のため変更しない
- **Badge 総数の再集計（F3、Sonnet P1-3）**: `rg -c "<Badge" src/features src/components --glob '!*.test.*'` を合計すると **45 件**（⑦ 起票時実測の「44 件」は `ReturnExchangePage.tsx:603` を含む前の走査値、本 packet が rg で再実行し 1 件差分を確認）
...
- **DOM 構造（F8, Opus P2-7, Coordinator decision）**: live 型は `<div className="flex items-center gap-2"><Label htmlFor={inputId} className="shrink-0 text-muted-foreground">{inputLabel}</Label><Input ... className="max-w-md" /></div>` の横並び wrapper にする（`Input` は既存の `max-w-md` を維持、`Label` は `CommitSearchBar` と同じ `shrink-0 text-muted-foreground` を流用）
...
- **F16（Opus P2-6）**: catalog `:836-839` の①状態 tone family マッピング表は `CsvImportRecordDetailPage.tsx:41,140` を destructive 行（取消済み）にのみ記載しているが、S2 の Coordinator adjudication（Q1）により同サイトは success（成功）/warning（部分成功）行にも tone を持つ。success 行・warning 行それぞれに `CsvImportRecordDetailPage.tsx:140` の該当分岐（「成功」「部分成功」）を追加する
...
- AC4（**F10, Opus P2-1 是正、pair oracle**）: (a) `rg -n 'border-warning bg-warning-soft text-warning-strong' src --glob '!*.test.*'` = 0（`alert.tsx` の定義文字列は語順が異なる `bg-warning-soft border-warning ...` のため、この pattern はそもそも `alert.tsx` にヒットしない — 例外なしの完全 sweep）。(b) `rg -c 'variant="warning"' src --glob '!*.test.*'` = 17（14 箇所の手書き Alert + `PriceRevisionPage.tsx` + `DailySalesPage.tsx` + `MonthlySalesPage.tsx`、変換完了の positive 側）
- AC5: `rg -c "aria-label" src/components/patterns/SearchBar.tsx` の `LiveSearchBar` 関数範囲内で 0。`SearchBar.test.tsx` に live 型の negative oracle（`input.getAttribute("aria-label")` が `null`）を追加する。`InventoryRecordsPage.test.tsx:744-745` の negative assertion 2 件（label 不在・id 不在）を positive assertion へ反転する（**F7 是正**、`:745` も対象）。**F5（Opus P1-1）是正**: live 型を参照する既存 query を `"商品を検索"` へ書き換える — `rg -c '"商品を検索"' src/features/products/ProductListPage.test.tsx` ≥ 9、`rg -c '"商品を検索"' src/features/inventory-records/InventoryRecordsPage.test.tsx` ≥ 6、`rg -c '"商品を検索"' src/components/patterns/SearchBar.test.tsx` ≥ 4（live 型分）、`rg -c 'getByLabelText("商品検索")' src/components/patterns/SearchBar.test.tsx` = 4（commit 型のみ残る、live 型は書き換え済み）
...
- AC8: docs 側 — `rg -c "^## DSR-23" docs/design-system/01-decision-rules.md` = 1（起票時 2）。`rg -Fc "PriceRevisionPage.tsx:79-83" docs/design-system/02-component-catalog.md` ≥ 1 かつ `rg -Fc "PriceRevisionPage.tsx:112-116" docs/design-system/02-component-catalog.md` = 0（**F15** citation 訂正）。catalog `:836-839` の success/warning 行に `CsvImportRecordDetailPage.tsx:140` の言及が追加される（**F16**）。`doc-consistency-check.sh` の ERROR 0（DS3 token check 含む、トークンは既に foundations/globals.css 両方に登録済みのため差分なしで通過見込み）
...
- Deferred design gaps, risk, and follow-up target: `daily-sales/components/ProductTable.tsx:133`「手動」badge の②分類/①状態 owner 確認（Coordinator 仮裁定 A、pending）
...
- Design gaps intentionally deferred: `daily-sales/components/ProductTable.tsx:133`「手動」badge の②分類/①状態 owner 確認（Coordinator 仮裁定 A、pending）
...
- `daily-sales/components/ProductTable.tsx:133`「手動」badge が amber className を失い `variant="secondary"` のみになっていること（Coordinator decision A、owner 確認 pending）
- `ReturnExchangePage.tsx:603-607`「レジ未処理」が `tone="warning"` + icon へ移行し、`:580`「レジ戻し済み」が無変更であること
