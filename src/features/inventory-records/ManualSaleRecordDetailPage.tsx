// src/features/inventory-records/ManualSaleRecordDetailPage.tsx
//
// REQ-203 / REQ-206: 手動販売記録詳細。
...
export function ManualSaleRecordDetailPage({
  ...
  const detailQuery = useQuery({
    ...
    queryFn: () =>
      unwrapResult(commands.getManualSaleRecord(recordId), {
        source: "commands",
        cmd: "get_manual_sale_record",
      }),
    staleTime: 30_000,
    gcTime: 5 * 60_000,
    retry: 0,
    ...
    <div className="space-y-5 p-6">
      ...
      <section className="rounded-md border p-4">
        <div className="grid gap-3 text-sm sm:grid-cols-6">
          <div>
            <span className="text-muted-foreground">販売日</span>
            <div className="font-medium">{detail.sale_date}</div>
          </div>
          <div>
            <span className="text-muted-foreground">理由</span>
            <div className="font-medium">{REASON_LABELS[detail.reason] ?? detail.reason}</div>
          </div>
          <div>
          ...
          <div>
            <span className="text-muted-foreground">販売金額合計</span>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-right">販売金額</TableHead>
